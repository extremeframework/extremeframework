...
/**
 * Usage: <{module class="WidgetListAdminView" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminview.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminView extends AdminViewController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminview.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('adminview')) {
            $vars = array_keys(get_object_vars(new AdminViewModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminview', $preset, $presetvalue));
