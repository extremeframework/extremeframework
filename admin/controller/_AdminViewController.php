...
class _AdminViewController extends __AppController
{
    var $module = 'adminview';
    var $type = 'controller';

    public function __construct() {
        parent::__construct();

        PluginManager::do_action('adminview_init');
    }
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('adminview', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminview_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminview_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminview_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('adminview');

        ContextStack::check2redirect('adminview');

        AclController::checkPermission('adminview', 'list');

		ContextStack::register(APPLICATION_URL.'/adminview/list/');
    ...
    public function updateOrderAction() {
        ...
        AclController::checkPermission('adminview', 'edit');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_VIEW.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
		...
		$model = new AdminViewModel();
		if (!empty($term)) {
            $model->whereAdd("REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%'");
		}
		$model->find();
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminview');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminview_before_delete', $_model);
        }

		$model = new AdminViewModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminview', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('adminview', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('adminview_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminview', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminviewlist_selection_selectall']) && $_REQUEST['adminviewlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminview.list.model']) && is_object($_SESSION['adminview.list.model'])) {
					$model = clone $_SESSION['adminview.list.model'];
				} else {
		        	$model = new AdminViewModel();
				}
    			...
    			$selection = isset($_REQUEST['adminviewlist_selection'])? $_REQUEST['adminviewlist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminview' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminview/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminview', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminViewModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminview', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminview/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminview/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminview_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminview_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminview');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'TITLE', 'CODE', 'MODULE', 'SCREEN', 'CONDITION_FIELD', 'CONDITION_VALUE', 'TEMPLATE', 'IS_PRINT_VIEW', 'ORDERING');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminview_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'TITLE', 'CODE', 'MODULE', 'SCREEN', 'CONDITION_FIELD', 'CONDITION_VALUE', 'TEMPLATE', 'IS_PRINT_VIEW', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminview'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminview_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminview_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminview_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            CustomFieldHelper::updateCustomFieldValues('adminview', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('adminview_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminViewModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminview_before_update', $model, $old);

        		if (!AclController::hasPermission('adminview', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_VIEW.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminview_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminview_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminview_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminview');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminview', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminview.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.adminview.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.adminview.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.adminview.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminview.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminview.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminview.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminview.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminview.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminview.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminview.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminview.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('TITLE', 'CODE', 'MODULE', 'SCREEN', 'CONDITION_FIELD', 'CONDITION_VALUE', 'ORDERING');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminview.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminview.customview'])? $_SESSION['adminview.customview'] : '';
    ...
    private function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminview', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminview', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('adminview');
        ...
        $_SESSION['adminview.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminview'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminview', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminview', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminview', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminview.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.adminview.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminview');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminview', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminview', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminview', 'new');
        }
        ...
        $ids = isset($_SESSION['adminview.list.ids'])? $_SESSION['adminview.list.ids'] : array();
        ...
        PluginManager::do_action('adminview_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminview', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminview', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminview', 'view'));
        ...
        PluginManager::do_action('adminview_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.adminview.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminview');
        ...
        $roweditablecolumns = array('TITLE', 'CODE', 'MODULE', 'SCREEN', 'CONDITION_FIELD', 'CONDITION_VALUE', 'TEMPLATE', 'IS_PRINT_VIEW', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminview', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminview_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminview', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminview_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminview', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminview'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminview'));

	    $this->display($smarty, $templatecode);
	}

    private function getLayoutColumns() {
        return array('TITLE', 'CODE', 'MODULE', 'SCREEN', 'CONDITION_FIELD', 'CONDITION_VALUE', 'TEMPLATE', 'IS_PRINT_VIEW');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('adminview', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.UDID = 0 OR ".TABLE_PREFIX."ADMIN_VIEW.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminViewController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminview.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'TITLE':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_VIEW.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_VIEW.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_VIEW`.TITLE, `'.TABLE_PREFIX.'ADMIN_VIEW`.CODE, `'.TABLE_PREFIX.'ADMIN_VIEW`.MODULE, `'.TABLE_PREFIX.'ADMIN_VIEW`.SCREEN, `'.TABLE_PREFIX.'ADMIN_VIEW`.CONDITION_FIELD, `'.TABLE_PREFIX.'ADMIN_VIEW`.CONDITION_VALUE, `'.TABLE_PREFIX.'ADMIN_VIEW`.TEMPLATE, `'.TABLE_PREFIX.'ADMIN_VIEW`.IS_PRINT_VIEW, `'.TABLE_PREFIX.'ADMIN_VIEW`.ID, `'.TABLE_PREFIX.'ADMIN_VIEW`.JSON, `'.TABLE_PREFIX.'ADMIN_VIEW`.UUID, `'.TABLE_PREFIX.'ADMIN_VIEW`.WFID');
    ...
    protected function initListModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_VIEW`.TITLE, `'.TABLE_PREFIX.'ADMIN_VIEW`.CODE, `'.TABLE_PREFIX.'ADMIN_VIEW`.MODULE, `'.TABLE_PREFIX.'ADMIN_VIEW`.SCREEN, `'.TABLE_PREFIX.'ADMIN_VIEW`.CONDITION_FIELD, `'.TABLE_PREFIX.'ADMIN_VIEW`.CONDITION_VALUE, `'.TABLE_PREFIX.'ADMIN_VIEW`.TEMPLATE, `'.TABLE_PREFIX.'ADMIN_VIEW`.IS_PRINT_VIEW, `'.TABLE_PREFIX.'ADMIN_VIEW`.ORDERING, `'.TABLE_PREFIX.'ADMIN_VIEW`.ID, `'.TABLE_PREFIX.'ADMIN_VIEW`.JSON, `'.TABLE_PREFIX.'ADMIN_VIEW`.UUID, `'.TABLE_PREFIX.'ADMIN_VIEW`.WFID');
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.SCREEN LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.CONDITION_FIELD LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.CONDITION_VALUE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.TEMPLATE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_VIEW.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
