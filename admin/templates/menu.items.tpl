...
<{foreach from=$menus.$depth.$parent item=menuitem}>
    <{if $menuitem->ENABLE_LEFT}>
        <li class="menu-<{$menuitem->MODULE}><{if isset($module) && $menuitem->MODULE == $module || $menuitem->ID == $parentmenu}> current<{/if}>">
            <a class="menulink menulink-<{$menuitem->MODULE}> scope-page cachable" href="<{$menuitem->LINK}>"><{if $menuitem->FONT_AWESOME_ICON}><i class="fa <{$menuitem->FONT_AWESOME_ICON}>"></i> &nbsp; <{/if}><{_t($menuitem->NAME)}></a>
