...
class _AdminSequenceController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('SEQUENCE_STEP', $columns2check) && !empty($model->SEQUENCE_STEP) && !is_numeric($model->SEQUENCE_STEP)) {
           $errors['sequence-step'] = sprintf(L_VALIDATION_NUMBER, L_SEQUENCE_STEP);
           return false;
       }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminsequence_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminsequence_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminsequence_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminsequence');

        AclController::checkPermission('adminsequence', 'list');

		ContextStack::register(APPLICATION_URL.'/adminsequence/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_SEQUENCE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminsequence_before_delete', $_model);
        }

		$model = new AdminSequenceModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminsequence', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminsequence', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminsequencelist_selection_selectall']) && $_REQUEST['adminsequencelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminsequence.list.model']) && is_object($_SESSION['adminsequence.list.model'])) {
					$model = clone $_SESSION['adminsequence.list.model'];
				} else {
		        	$model = new AdminSequenceModel();
				}
    			...
    			$selection = isset($_REQUEST['adminsequencelist_selection'])? $_REQUEST['adminsequencelist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminsequence' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminsequence', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminSequenceModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminsequence', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminsequence/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminsequence/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminsequence_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminsequence_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminsequence_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminsequence_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminsequence_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminsequence_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminsequence_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminSequenceModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminsequence_before_update', $model, $old);

        		if (!AclController::hasPermission('adminsequence', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_SEQUENCE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('adminsequence_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminsequence', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminsequence.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminsequence.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminsequence.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminsequence.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminsequence.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminsequence.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminsequence.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminsequence.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminsequence.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminsequence.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminsequence.customview'])? $_SESSION['adminsequence.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminsequence');
        ...
        $_SESSION['adminsequence.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminsequence'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminsequence', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminsequence', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminsequence', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.adminsequence.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminsequence', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminsequence', 'new');
        }
        ...
        $ids = isset($_SESSION['adminsequence.list.ids'])? $_SESSION['adminsequence.list.ids'] : array();
        ...
        PluginManager::do_action('adminsequence_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminsequence', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminsequence', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminsequence', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.adminsequence.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('MODULE', 'SEQUENCE_FORMAT', 'CURRENT_VALUE', 'SEQUENCE_STEP');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminsequence', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminsequence_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminsequence', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminsequence_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminsequence', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminsequence'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminsequence'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminSequenceController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminsequence.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'SEQUENCE_STEP__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.SEQUENCE_STEP >= '".$this->field_sanitize('SEQUENCE_STEP', $value)."'");

                        break;

                    case 'SEQUENCE_STEP__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.SEQUENCE_STEP IS NULL OR ".TABLE_PREFIX."ADMIN_SEQUENCE.SEQUENCE_STEP <= '".$this->field_sanitize('SEQUENCE_STEP', $value)."')");

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_SEQUENCE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_SEQUENCE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_SEQUENCE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_SEQUENCE.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_SEQUENCE.SEQUENCE_FORMAT LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_SEQUENCE.CURRENT_VALUE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_SEQUENCE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
