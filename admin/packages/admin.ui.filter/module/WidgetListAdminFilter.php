...
/**
 * Usage: <{module class="WidgetListAdminFilter" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminfilter.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminFilter extends AdminFilterController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminfilter.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminfilter')) {
            $vars = array_keys(get_object_vars(new AdminFilterModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminfilter', $preset, $presetvalue));
