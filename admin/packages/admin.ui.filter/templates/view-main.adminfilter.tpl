<div class="view-main">
    <{plugin key="adminfilter_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{_t('Admin filter name')}></label>
            </td>
    ...
    <{/if}>
            ...
            <{html_ref_select multilingual="0" autocomplete="1" ajax="0" method="" id="input-module" class="input-module" name="`$prefix`adminfilter_formdata_MODULE" value=$details->MODULE text=$details->reftext_MODULE datasource="ADMIN_MODULE" valuecol="MODULE" textcol="NAME" sortcol=""}>
    ...
    <script type="text/javascript">
        ...
        adminfilterview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        adminfilterview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                adminfilterview_apply_block_visibility(key, value);
            });
        }

        adminfilterview_apply_block_visibility = function (key, value) {
        ...
        adminfilterview_update_visibility_settings = function () {
            var settings = adminfilterview_get_visibility_settings();

            adminfilterview_apply_visibility_settings(settings);
        }

        $(function() {
            adminfilterview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/adminfilter/fieldSave",
//                        data: td.find(':input').serialize() + "&adminfilter_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
