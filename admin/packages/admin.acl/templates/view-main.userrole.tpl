<div class="view-main">
    <{plugin key="userrole_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_USER_ROLE_NAME"}></label>
            </td>
    ...
    <{plugin key="userrole_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        userroleview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        userroleview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                userroleview_apply_block_visibility(key, value);
            });
        }

        userroleview_apply_block_visibility = function (key, value) {
        ...
        userroleview_update_visibility_settings = function () {
            var settings = userroleview_get_visibility_settings();

            userroleview_apply_visibility_settings(settings);
        }

        $(function() {
            userroleview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/userrole/fieldSave",
//                        data: td.find(':input').serialize() + "&userrole_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
