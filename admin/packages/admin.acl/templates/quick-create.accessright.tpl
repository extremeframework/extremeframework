<div class="modal-header">
    ...
    <h3><{label key="L_QUICK_CREATE" format="ucwords"}> <{$smarty.const.L_ACCESS_RIGHT|ucwords}></h3>
</div>
...
<div id="accessright-edit" class="edit_details">
    ...
    <{if isset($messages.accessright) }>
        <ul class="message">
            <{foreach from=$messages.accessright key=field item=message}>
...
<{if !isset($excludedcolumns['ACTIONS'])}>
        ...
        <{else}>
    		<td class="form-row form-row-actions">
                <div class="form-field form-field-label">
        		    <label><{label key="L_ACTIONS"}></label>
                </div>
            </td>
