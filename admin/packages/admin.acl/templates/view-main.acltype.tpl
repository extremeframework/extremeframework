<div class="view-main">
    <{plugin key="acltype_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_ACL_TYPE_NAME"}></label>
            </td>
    ...
    <{plugin key="acltype_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        acltypeview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        acltypeview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                acltypeview_apply_block_visibility(key, value);
            });
        }

        acltypeview_apply_block_visibility = function (key, value) {
        ...
        acltypeview_update_visibility_settings = function () {
            var settings = acltypeview_get_visibility_settings();

            acltypeview_apply_visibility_settings(settings);
        }

        $(function() {
            acltypeview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/acltype/fieldSave",
//                        data: td.find(':input').serialize() + "&acltype_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
