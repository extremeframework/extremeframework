...
class _UserRoleController extends __AppController
{
    var $module = 'userrole';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('userrole_init');
    }

    private function checkConstraint($model, &$errors, $columns2check) {
        
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_USER_ROLE_NAME);
           return false;
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('userrole', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^userrole_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userrole_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userrole_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('userrole');

        AclController::checkPermission('userrole', 'list');

		ContextStack::register(APPLICATION_URL.'/userrole/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."USER_ROLE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('userrole_before_delete', $_model);
        }

		$model = new UserRoleModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('userrole', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."USER_ROLE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('userrole_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('userrole', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['userrolelist_selection_selectall']) && $_REQUEST['userrolelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['userrole.list.model']) && is_object($_SESSION['userrole.list.model'])) {
					$model = clone $_SESSION['userrole.list.model'];
				} else {
		        	$model = new UserRoleModel();
				}
    			...
    			$selection = isset($_REQUEST['userrolelist_selection'])? $_REQUEST['userrolelist_selection'] : '';
			}
        }
        ...
        if ($module != 'userrole' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('userrole', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new UserRoleModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('userrole', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/userrole/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/userrole/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userrole_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'userrole_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('userrole');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'userrole_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('userrole'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userrole_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'userrole_multiformdata_deleteditems'])? $_REQUEST[$prefix.'userrole_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('userrole_before_save', $model);

    		if ($model->UUID) {
				
				$old = new UserRoleModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('userrole_before_update', $model, $old);

        		if (!AclController::hasPermission('userrole', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."USER_ROLE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."USER_ROLE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."USER_ROLE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('userrole_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('userrole_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('userrole_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('userrole');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('userrole', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.userrole.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.userrole.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.userrole.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.userrole.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.userrole.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.userrole.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.userrole.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.userrole.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.userrole.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['userrole.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['userrole.customview'])? $_SESSION['userrole.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('userrole');
        ...
        $_SESSION['userrole.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userrole'));
		...
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userrole', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.userrole.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.userrole.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('userrole');

        $aclviewablecolumns = AclController::getAclEnabledColumns('userrole', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('userrole', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('userrole', 'new');
        }
        ...
        $ids = isset($_SESSION['userrole.list.ids'])? $_SESSION['userrole.list.ids'] : array();
        ...
        PluginManager::do_action('userrole_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userrole', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('userrole', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userrole', 'view'));
        ...
        PluginManager::do_action('userrole_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.userrole.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('userrole');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('userrole', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('userrole_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('userrole', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('userrole_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userrole', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('userrole'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('userrole'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    UserRoleController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['userrole.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."USER_ROLE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."USER_ROLE.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."USER_ROLE.$key IS NULL OR ".TABLE_PREFIX.".USER_ROLE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."USER_ROLE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."USER_ROLE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_ROLE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_ROLE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
