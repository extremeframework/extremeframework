...
/**
 * Usage: <{module class="WidgetListUserGroup" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.usergroup.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserGroup extends UserGroupController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.usergroup.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('usergroup')) {
            $vars = array_keys(get_object_vars(new UserGroupModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('usergroup', $preset, $presetvalue));
