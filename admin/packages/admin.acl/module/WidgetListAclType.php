...
/**
 * Usage: <{module class="WidgetListAclType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.acltype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAclType extends AclTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.acltype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('acltype')) {
            $vars = array_keys(get_object_vars(new AclTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('acltype', $preset, $presetvalue));
