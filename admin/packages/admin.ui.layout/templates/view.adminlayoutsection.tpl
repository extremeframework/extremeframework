    ...
    <body class="module module-<{$module}>" id="<{$module}>">
            ...
            <{$tmp1 = _t('View')}>
        <{$tmp2 = strtolower(_t('Admin Layout Section'))}>
        <{$title = "`$tmp1` `$tmp2`"}>
    ...
    <h1 class="heading">
        ...
        <span style="margin-left:10px; font-size:12px; font-weight: normal" class="hidden-print">
            <{if $previd}>
                <a style="text-decoration: none" class="button-view-prev scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/view/<{$previd}>"><{_t('Prev')}></a>&nbsp;
            <{/if}>
            <{if $nextid}>
                <a style="text-decoration: none" class="button-view-next scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/view/<{$nextid}>"><{_t('Next')}></a>
            <{/if}>
            ...
            <!-- Live search -->
            <span id="adminlayoutsection-live-search" class="live-search view-live-search"><input type="text" onfocus="if (this.value == '<{_t('Quick search', true)}>...') {this.value = '';}" onblur="if (this.value == '')  {this.value = '<{_t('Quick search', true)}>...';}" value="<{_t('Quick search', true)}>..." autocomplete="off"></span>
            <script type="text/javascript">
                $(document).ready(function(){
                	$('#adminlayoutsection-live-search input').liveSearch({url: '<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/liveSearch?term='});

            	    $('#jquery-live-search').hide();
            	    $('#adminlayoutsectionview .button-edit').focus();
                });
            </script>

            <{if isset($smarty.session.acl.adminlayoutsection.new) }>
                &nbsp;&nbsp;<a style="text-decoration: none" class="button-view-new scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/new"><i class="fa fa-plus"></i></a>
            <{/if}>
        </span>
    </h1>
...
<div class="view-control-bar hidden-print">
    ...
    <div class="view-buttons">
    	<{if $details->UUID}>
    		    			<{if isset($smarty.session.acl.adminlayoutsection.edit) && WorkflowHelper::isEditable($details->WFID)}>
        		    <div class="button-general">
        		        <a class="button-edit scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/edit/<{$details->UUID}><{if isset($preset)}>/preset/<{$preset}><{/if}><{if isset($presetvalue)}>/presetvalue/<{$presetvalue}><{/if}>" title="<{_t('Edit', true)}>"><i class="fa fa-pencil"></i></a>
        		    </div>
    		    <{/if}>
    	    ...
    	    <{/foreach}>
    	            ...
    	            <{if $admin_view_actions || $admin_view_options || isset($smarty.session.acl.adminlayoutsection.customfields)}>
        	    <div class="button-general dropdown">
        	        ...
        	        <ul class="dropdown-menu pull-right">
        	            ...
        	            <{foreach from=$admin_view_options item=item}>
                            <li><a <{if $item->IS_PRINT_VIEW}>target="_blank"<{/if}> href="<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/view/<{$details->UUID}>/<{$item->CODE}>"><{$item->TITLE}></a></li>
                        <{/foreach}>
                        <{if isset($smarty.session.acl.adminlayoutsection.customfields)}>
                            <li><a class="scope-main" href="<{$smarty.const.APPLICATION_URL}>/customfield/manage/adminlayoutsection"><{_t('Manage custom fields')}></a></li>
                        <{/if}>
                    </ul>
        	    </div>
            <{/if}>
            <div class="button-general">
                <a class="button-close scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/adminlayoutsection/close/"><span class="button-face"><{_t('Close')}></span></a>
            </div>
...
<div id="adminlayoutsectionview" class="adminlayoutsection-view view_details mainview">
...
<{if Framework::hasModule('AdminComment')}>
    <!-- Comments start -->
    <div class="adminlayoutsection-comments" data-type="ajax" data-source="<{$smarty.const.APPLICATION_URL}>/admincomment/ajaxGetComments/adminlayoutsection/<{$details->UUID}>">
    <!-- Comments end -->
<{/if}>
...
<script type="text/javascript">
    $(document).ready(function(){
    	var origin = $('#adminlayoutsectionview .form-field-value .column-name');
    	var mirror = $('#adminlayoutsectionview .mirror-name');
...
<script type="text/javascript">
    $(function() {
    	bind_hotkey('#adminlayoutsectionview', 'f2', '.button-edit');
    	bind_hotkey('#adminlayoutsectionview', 'esc', '.button-close');
    	bind_hotkey('#adminlayoutsectionview', 'left', '.button-view-prev');
    	bind_hotkey('#adminlayoutsectionview', 'right', '.button-view-next');
    });
    ...
    $(function() {
        if (document.activeElement == document.body) {
    	    $('#adminlayoutsectionview :focusable:first').focus();
        }

    	$('#adminlayoutsectionview').click(function(){
    	    var focused = $('#adminlayoutsectionview :focus');
