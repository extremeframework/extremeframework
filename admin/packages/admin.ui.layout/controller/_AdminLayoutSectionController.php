...
class _AdminLayoutSectionController extends __AppController
{
    var $module = 'adminlayoutsection';
    var $type = 'controller';

    public function __construct() {
        parent::__construct();

        PluginManager::do_action('adminlayoutsection_init');
    }

    private function checkConstraint($model, &$errors, $columns2check) {
        
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_ADMIN_LAYOUT_SECTION_NAME);
           return false;
       }
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('adminlayoutsection', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminlayoutsection_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminlayoutsection_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminlayoutsection_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminlayoutsection');

        AclController::checkPermission('adminlayoutsection', 'list');

		ContextStack::register(APPLICATION_URL.'/adminlayoutsection/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_LAYOUT_SECTION.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
		...
		$model = new AdminLayoutSectionModel();
		if (!empty($term)) {
            $model->whereAdd("REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR SHOW_TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%'");
		}
		$model->find();
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminlayoutsection');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new AdminLayoutSectionModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminlayoutsection_before_delete', $_model);
        }

		$model = new AdminLayoutSectionModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminlayoutsection', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminlayoutsection', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminlayoutsectionlist_selection_selectall']) && $_REQUEST['adminlayoutsectionlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminlayoutsection.list.model']) && is_object($_SESSION['adminlayoutsection.list.model'])) {
					$model = clone $_SESSION['adminlayoutsection.list.model'];
				} else {
		        	$model = new AdminLayoutSectionModel();
				}
    			...
    			$selection = isset($_REQUEST['adminlayoutsectionlist_selection'])? $_REQUEST['adminlayoutsectionlist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminlayoutsection' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminlayoutsection', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminLayoutSectionModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminlayoutsection', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminlayoutsection/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminlayoutsection/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminlayoutsection_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminlayoutsection_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminlayoutsection');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'NAME', 'MODULE', 'SHOW_TITLE', 'ORDERING');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminlayoutsection_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'MODULE', 'SHOW_TITLE', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminlayoutsection'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminlayoutsection_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminlayoutsection_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminlayoutsection_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminlayoutsection_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminLayoutSectionModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminlayoutsection_before_update', $model, $old);

        		if (!AclController::hasPermission('adminlayoutsection', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_LAYOUT_SECTION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('adminlayoutsection_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminlayoutsection', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminlayoutsection.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminlayoutsection.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminlayoutsection.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminlayoutsection.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminlayoutsection.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminlayoutsection.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminlayoutsection.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminlayoutsection.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminlayoutsection.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('NAME', 'MODULE', 'SHOW_TITLE', 'ORDERING');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminlayoutsection.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminlayoutsection.customview'])? $_SESSION['adminlayoutsection.customview'] : '';
    ...
    private function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminlayoutsection', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminlayoutsection', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('adminlayoutsection');
        ...
        $_SESSION['adminlayoutsection.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminlayoutsection'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminlayoutsection', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminlayoutsection', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminlayoutsection', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminlayoutsection.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.adminlayoutsection.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminlayoutsection');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminlayoutsection', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminlayoutsection', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminlayoutsection', 'new');
        }
        ...
        $ids = isset($_SESSION['adminlayoutsection.list.ids'])? $_SESSION['adminlayoutsection.list.ids'] : array();
        ...
        PluginManager::do_action('adminlayoutsection_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminlayoutsection', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminlayoutsection', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminlayoutsection', 'view'));
        ...
        PluginManager::do_action('adminlayoutsection_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.adminlayoutsection.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminlayoutsection');
        ...
        $roweditablecolumns = array('NAME', 'MODULE', 'SHOW_TITLE', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminlayoutsection', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminlayoutsection_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminlayoutsection', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminlayoutsection_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminlayoutsection', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminlayoutsection'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminlayoutsection'));

	    $this->display($smarty, $templatecode);
	}

    private function getLayoutColumns() {
        return array('NAME', 'MODULE', 'SHOW_TITLE');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminLayoutSectionController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminlayoutsection.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_LAYOUT_SECTION.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_LAYOUT_SECTION.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_LAYOUT_SECTION.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_LAYOUT_SECTION.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_LAYOUT_SECTION.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_LAYOUT_SECTION.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
