...
class AdminLabelApi extends AdminLabelController
{
    var $module = 'adminlabel';
    var $type = 'api';

	function searchAction() {
	    AclApi::checkPermission('adminlabel', 'list');
		...
		$model->selectAdd();
		$model->selectAdd('ID, LABEL');

		if (!empty($keyword)) {
			$model->whereAdd("LABEL LIKE '%$keyword%'");
		}
    ...
    function saveAction() {
        AclApi::checkPermission('adminlabel', 'edit');
    ...
    function deleteAction() {
        AclApi::checkPermission('adminlabel', 'delete');
    ...
    function viewAction() {
	    AclApi::checkPermission('adminlabel', 'view');
        ...
        if ($model->fetch()) {
            $this->json_response(array('success' => true, 'data' => $this->model_sanitize($model), 'title' => $model->LABEL));
        } else {
