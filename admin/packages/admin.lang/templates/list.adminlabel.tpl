...
<{assign var='title' value=$smarty.const.L_ADMIN_LABEL}>
...
<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="adminlabelquicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/adminlabel/search" method="post" enctype="multipart/form-data">
            <input type="text" name="adminlabel_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{$smarty.const.L_SEARCH}>" />
...
<{if isset($title) }>
    <h1 class="heading">
        ...
        <span style="margin-left:10px; font-size:12px; font-weight: normal" class="hidden-print">
            <a style="text-decoration: none" class="button-view-refresh scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminlabel"><i class="fa fa-refresh"></i></a>
        </span>
    </h1>
<{/if}>
...
<form name="adminlabellist" id="adminlabellistform" class="form-list scope-list" action="<{$smarty.const.APPLICATION_URL}>/adminlabel/" method="post">
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminlabel/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{$smarty.const.L_NEW}> <{$smarty.const.L_ADMIN_LABEL|strtolower}>"/><{label key="L_NEW"}> <{$smarty.const.L_ADMIN_LABEL|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="adminlabel_delete()"><span class="button-face"><{$smarty.const.L_DELETE}></span></a>
                        </div>
                                            <{/if}>
            	        ...
            	        <ul class="dropdown-menu pull-right">
                            <{foreach from=$admin_list_actions item=item}>
                                <{if empty($item->ACL) || isset($smarty.session.acl.adminlabel[$item->ACL])}>
                                    <li><a <{if $item->IS_OPEN_IN_NEW_WINDOW}>target="_blank"<{/if}> <{if $item->LINK}>href="<{framework_report_link format=$item->LINK}>"<{/if}> <{if $item->ACTION}>onclick="adminlabel_<{$item->ACTION}>(); return false;"<{/if}>><span class="button-face"><{label text=$item->TITLE}></span></a></li>
                                <{/if}>
                            <{/foreach}>
                        </ul>
            	    </div>
...
<!-- Relations -->
    <{php}>
    	$template->assign('copyguidelines',  sprintf(L_GUIDELINES_COPY_RELS, strtolower(L_ADMIN_LABEL)));
    	$template->assign('approveguidelines', sprintf(L_GUIDELINES_APPROVE_RELS, strtolower(L_ADMIN_LABEL)));
    	$template->assign('deleteguidelines', sprintf(L_GUIDELINES_DELETE_RELS, strtolower(L_ADMIN_LABEL), strtolower(L_ADMIN_LABEL)));
    <{/php}>

    <div id="adminlabelcopyrelations" style="display:none" title="<{$smarty.const.L_COPY}> <{$smarty.const.L_ADMIN_LABEL|strtolower}>">
        <p><{$copyguidelines}></p>
        <ul>
    ...
    <div id="adminlabelapproverelations" style="display:none" title="<{$smarty.const.L_APPROVE}> <{$smarty.const.L_ADMIN_LABEL|strtolower}>">
    ...
    <div id="adminlabeldeleterelations" style="display:none" title="<{$smarty.const.L_DELETE}> <{$smarty.const.L_ADMIN_LABEL|strtolower}>">
...
function adminlabel_search() {
	$('#adminlabellistform').attr('action', '<{$smarty.const.APPLICATION_URL}>/adminlabel/search/');
...
function adminlabel_delete() {
	var dialog = $( "#adminlabeldeleterelations" ).dialog({
		resizable: true,
		width: 500,
		modal: false,
		buttons: {
	...
	dialog.parent().appendTo($('#adminlabellistform'));
}

function adminlabel_copy() {
	var dialog = $( "#adminlabelcopyrelations" ).dialog({
		resizable: true,
		width: 500,
		modal: false,
		buttons: {
...
function adminlabel_approve() {
	var dialog = $( "#adminlabelapproverelations" ).dialog({
		resizable: true,
		width: 500,
		modal: false,
		buttons: {
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="adminlabel-list-count list-count"><{$total}></span> <{label key="L_RECORDS"}>
	</div>
	<div style="clear:both"></div>
<{/if}>
