...
/**
 * Usage: <{module class="WidgetListAdminLanguage" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminlanguage.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminLanguage extends AdminLanguageController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminlanguage.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminlanguage')) {
            $vars = array_keys(get_object_vars(new AdminLanguageModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminlanguage', $preset, $presetvalue));
