...
/**
 * Usage: <{module class="WidgetListAdminLabel" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminlabel.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminLabel extends AdminLabelController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminlabel.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('adminlabel')) {
            $vars = array_keys(get_object_vars(new AdminLabelModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminlabel', $preset, $presetvalue));
