...
/**
 * Usage: <{module class="WidgetListAdminLanguageItem" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminlanguageitem.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminLanguageItem extends AdminLanguageItemController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminlanguageitem.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminlanguageitem')) {
            $vars = array_keys(get_object_vars(new AdminLanguageItemModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminlanguageitem', $preset, $presetvalue));
