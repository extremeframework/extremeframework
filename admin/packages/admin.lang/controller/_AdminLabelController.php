...
class _AdminLabelController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('LABEL', $columns2check)) {
           ...
           $_model->find();
           if ($_model->N) {
               $errors['label'] = sprintf(L_VALIDATION_ALREADY_EXISTS, '{'.L_LABEL.'}');
               return false;
           }
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('adminlabel', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminlabel_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminlabel_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminlabel_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminlabel');

        AclController::checkPermission('adminlabel', 'list');

		ContextStack::register(APPLICATION_URL.'/adminlabel/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_LABEL.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
        ...
        $columns2return = array('ID' => 'id', 'LABEL' => 'name');
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$rows = array();
     	while ($model->fetch()) {
     		$rows[] = array('id' => $model->UUID, 'eid' => $model->ID, 'title' => $model->LABEL);
		}

		$smarty = Framework::getSmarty();

		$smarty->assign('rows', $rows);
		$smarty->assign('module', 'adminlabel');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminlabel_before_delete', $_model);
        }

		$model = new AdminLabelModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminlabel', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_LABEL.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminlabel', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminlabellist_selection_selectall']) && $_REQUEST['adminlabellist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminlabel.list.model']) && is_object($_SESSION['adminlabel.list.model'])) {
					$model = clone $_SESSION['adminlabel.list.model'];
				} else {
		        	$model = new AdminLabelModel();
				}
    			...
    			$selection = isset($_REQUEST['adminlabellist_selection'])? $_REQUEST['adminlabellist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminlabel' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminlabel', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminLabelModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminlabel', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminlabel/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminlabel/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminlabel_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminlabel_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminlabel');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminlabel_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminlabel_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminlabel_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminlabel_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminlabel_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminLabelModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminlabel_before_update', $model, $old);

        		if (!AclController::hasPermission('adminlabel', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_LABEL.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_LABEL.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_LABEL.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('adminlabel_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminlabel', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminlabel.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminlabel.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminlabel.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminlabel.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminlabel.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminlabel.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminlabel.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminlabel.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminlabel.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminlabel.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminlabel.customview'])? $_SESSION['adminlabel.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminlabel');
        ...
        $_SESSION['adminlabel.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminlabel'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminlabel', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminlabel', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminlabel', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.adminlabel.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminlabel', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminlabel', 'new');
        }
        ...
        $ids = isset($_SESSION['adminlabel.list.ids'])? $_SESSION['adminlabel.list.ids'] : array();
        ...
        PluginManager::do_action('adminlabel_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminlabel', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminlabel', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminlabel', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.adminlabel.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('LABEL');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminlabel', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminlabel_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminlabel', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminlabel_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminlabel', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminlabel'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminlabel'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminLabelController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminlabel.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_LABEL.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_LABEL.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_LABEL.LABEL LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_LABEL.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
