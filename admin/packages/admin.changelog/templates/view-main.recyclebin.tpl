<div class="view-main">
    <{plugin key="recyclebin_view_top" args=$details}>
            ...
            <td class="form-field form-field-value column-item" colspan="3">
            	<div class="attribute-line column-item type-text">
            		<div class="attribute-value">
            		                		        <{if (isset($acleditablecolumns['ITEM']) && $acleditablecolumns['ITEM']) || (isset($acleditablecolumns['*']) && (!isset($acleditablecolumns['ITEM']) || $acleditablecolumns['ITEM'])) }>
    ...
    <{/if}>
            ...
            <{html_ref_select multilingual="0" autocomplete="1" ajax="0" method="" id="input-module" class="input-module" name="`$prefix`recyclebin_formdata_MODULE" value=$details->MODULE text=$details->reftext_MODULE datasource="ADMIN_MODULE" datasourcename="AdminModule" valuecol="MODULE" textcol="NAME" sortcol="NAME"}>
    ...
    <{/if}>
            ...
            <{html_ref_select multilingual="0" autocomplete="1" ajax="0" method="" id="input-id-user" class="input-id-user" name="`$prefix`recyclebin_formdata_ID_USER" value=$details->ID_USER text=$details->reftext_ID_USER datasource="USER" datasourcename="User" valuecol="ID" textcol="FIRST_NAME+LAST_NAME" sortcol="FIRST_NAME"}>
...
<{/if}>            </tbody>
    ...
    <{plugin key="recyclebin_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        recyclebinview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        recyclebinview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                recyclebinview_apply_block_visibility(key, value);
            });
        }

        recyclebinview_apply_block_visibility = function (key, value) {
        ...
        recyclebinview_update_visibility_settings = function () {
            var settings = recyclebinview_get_visibility_settings();

            recyclebinview_apply_visibility_settings(settings);
        }

        $(function() {
            recyclebinview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/recyclebin/fieldSave",
//                        data: td.find(':input').serialize() + "&recyclebin_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
