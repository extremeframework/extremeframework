...
class _ChangeLogController extends __AppController
{
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('MODULE', $columns2check) && trim($model->MODULE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('changelog', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^changelog_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^changelog_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^changelog_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('changelog');

        AclController::checkPermission('changelog', 'list');

		ContextStack::register(APPLICATION_URL.'/changelog/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."CHANGE_LOG.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
        ...
        $columns2return = array('ID' => 'id', 'ACTION' => 'name');
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$model->orderBy('ACTION');
		$model->limit(0, 10);
		$model->find();

		$rows = array();
     	while ($model->fetch()) {
     		$rows[] = array('id' => $model->UUID, 'eid' => $model->ID, 'title' => $model->ACTION);
		}

		$smarty = Framework::getSmarty(__FILE__);

		$smarty->assign('rows', $rows);
		$smarty->assign('module', 'changelog');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('changelog_before_delete', $_model);
        }

		$model = new ChangeLogModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('changelog', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('changelog', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['changeloglist_selection_selectall']) && $_REQUEST['changeloglist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['changelog.list.model']) && is_object($_SESSION['changelog.list.model'])) {
					$model = clone $_SESSION['changelog.list.model'];
				} else {
		        	$model = new ChangeLogModel();
				}
    			...
    			$selection = isset($_REQUEST['changeloglist_selection'])? $_REQUEST['changeloglist_selection'] : '';
			}
        }
        ...
        if ($module != 'changelog' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('changelog', 'new');

		ContextStack::register(APPLICATION_URL.'/changelog/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('changelog', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new ChangeLogModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('changelog', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/changelog/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
        ...
        AclController::checkPermission('changelog', 'edit');
		...
		ContextStack::register(APPLICATION_URL.'/changelog/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'changelog_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'changelog_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('changelog');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'changelog_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'changelog_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'changelog_multiformdata_deleteditems'])? $_REQUEST[$prefix.'changelog_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('changelog', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('changelog_before_save', $model);

    		if ($model->UUID) {
				
				$old = new ChangeLogModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('changelog_before_update', $model, $old);

        		if (!AclController::hasPermission('changelog', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."CHANGE_LOG.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('changelog_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('changelog', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.changelog.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.changelog.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.changelog.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.changelog.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.changelog.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.changelog.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.changelog.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.changelog.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.changelog.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.changelog.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.changelog.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.changelog.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('ACTION', 'ITEM', 'MODULE', 'DATE_TIME', 'ID_USER');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['changelog.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['changelog.customview'])? $_SESSION['changelog.customview'] : '';
    ...
    protected function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('changelog');
        ...
        $_SESSION['changelog.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('changelog'));
		...
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('changelog', 'view'));
    ...
    protected function _view($id, $details = null, $templatecode = 'view.changelog.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('changelog', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('changelog', 'new');
        }
        ...
        $ids = isset($_SESSION['changelog.list.ids'])? $_SESSION['changelog.list.ids'] : array();
        ...
        PluginManager::do_action('changelog_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('changelog', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('changelog', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('changelog', 'view'));
    ...
    protected function _edit($id, $details = null, $templatecode = 'edit.changelog.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('ACTION', 'ITEM', 'MODULE', 'DATE_TIME', 'ID_USER');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('changelog', 'editpeer');
                }
            } else {
                ...
                PluginManager::do_action('changelog_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('changelog', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('changelog_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('changelog', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('changelog'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('changelog'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('changelog', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.UDID = 0 OR ".TABLE_PREFIX."CHANGE_LOG.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."CHANGE_LOG.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    ChangeLogController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['changelog.list.model'] = clone $model;
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'DATE_TIME__FROM':
                        $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.DATE_TIME >= '".$this->field_sanitize('DATE_TIME', $value)."'");

                        break;

                    case 'DATE_TIME__TO':
                        $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.DATE_TIME IS NULL OR ".TABLE_PREFIX."CHANGE_LOG.DATE_TIME <= '".$this->field_sanitize('DATE_TIME', $value)."')");

                        break;

                    case 'ID_USER':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.ID_USER IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.ID_USER = '$value'");
                        }

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.$key IS NULL OR ".TABLE_PREFIX.".CHANGE_LOG.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."CHANGE_LOG.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."CHANGE_LOG.ACTION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."CHANGE_LOG.ITEM LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."CHANGE_LOG.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."CHANGE_LOG.DETAILS LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."CHANGE_LOG.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
