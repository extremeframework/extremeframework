...
class _ChangeLogController extends __AppController
{
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('changelog', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.UDID = 0 OR ".TABLE_PREFIX."CHANGE_LOG.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."CHANGE_LOG.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
