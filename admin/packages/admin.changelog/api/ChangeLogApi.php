...
class ChangeLogApi extends ChangeLogController
{
    var $module = 'changelog';
    var $type = 'api';

	function searchAction() {
	    AclApi::checkPermission('changelog', 'list');
		...
		$model->selectAdd();
		$model->selectAdd('ID, ACTION');

		if (!empty($keyword)) {
			$model->whereAdd("ACTION LIKE '%$keyword%'");
		}
    ...
    function saveAction() {
        AclApi::checkPermission('changelog', 'edit');
    ...
    function deleteAction() {
        AclApi::checkPermission('changelog', 'delete');
    ...
    function viewAction() {
	    AclApi::checkPermission('changelog', 'view');
        ...
        if ($model->fetch()) {
            $this->json_response(array('success' => true, 'data' => $this->model_sanitize($model), 'title' => $model->ACTION));
        } else {
