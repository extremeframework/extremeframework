...
class UserApi extends UserController
{
	...
	function searchAction() {
		...
		$model->selectAdd();
		$model->selectAdd('ID, FIRST_NAME');

		if (!empty($keyword)) {
			$model->whereAdd("FIRST_NAME LIKE '%$keyword%'");
		}
    ...
    function viewAction() {
        ...
        if ($model->fetch()) {
            $this->json_response(array('success' => true, 'data' => $this->model_sanitize($model), 'title' => $model->FIRST_NAME));
        } else {
