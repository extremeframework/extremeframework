...
class UserLogApi extends UserLogController
{
    var $module = 'userlog';
    var $type = 'api';

	function searchAction() {
	    AclApi::checkPermission('userlog', 'list');
		...
		$model->selectAdd();
		$model->selectAdd('ID, DATE_TIME');

		if (!empty($keyword)) {
			$model->whereAdd("DATE_TIME LIKE '%$keyword%'");
		}
    ...
    function saveAction() {
        AclApi::checkPermission('userlog', 'edit');
    ...
    function deleteAction() {
        AclApi::checkPermission('userlog', 'delete');
    ...
    function viewAction() {
	    AclApi::checkPermission('userlog', 'view');
        ...
        if ($model->fetch()) {
            $this->json_response(array('success' => true, 'data' => $this->model_sanitize($model), 'title' => $model->DATE_TIME));
        } else {
