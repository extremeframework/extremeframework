...
/**
 * Usage: <{module class="WidgetListUser" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.user.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUser extends UserController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.user.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('user')) {
            $vars = array_keys(get_object_vars(new UserModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('user', $preset, $presetvalue));
