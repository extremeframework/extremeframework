...
/**
 * Usage: <{module class="WidgetListUserLog" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.userlog.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserLog extends UserLogController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.userlog.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('userlog')) {
            $vars = array_keys(get_object_vars(new UserLogModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userlog', $preset, $presetvalue));
