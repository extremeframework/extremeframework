...
class _PermissionSetItemController extends __AppController
{
    var $module = 'permissionsetitem';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('permissionsetitem_init');
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^permissionsetitem_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^permissionsetitem_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^permissionsetitem_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('permissionsetitem');

        AclController::checkPermission('permissionsetitem', 'list');

		ContextStack::register(APPLICATION_URL.'/permissionsetitem/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."PERMISSION_SET_ITEM.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'permissionsetitem');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('permissionsetitem_before_delete', $_model);
        }

		$model = new PermissionSetItemModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('permissionsetitem', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('permissionsetitem_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('permissionsetitem', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['permissionsetitemlist_selection_selectall']) && $_REQUEST['permissionsetitemlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['permissionsetitem.list.model']) && is_object($_SESSION['permissionsetitem.list.model'])) {
					$model = clone $_SESSION['permissionsetitem.list.model'];
				} else {
		        	$model = new PermissionSetItemModel();
				}
    			...
    			$selection = isset($_REQUEST['permissionsetitemlist_selection'])? $_REQUEST['permissionsetitemlist_selection'] : '';
			}
        }
        ...
        if ($module != 'permissionsetitem' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('permissionsetitem', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PermissionSetItemModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('permissionsetitem', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/permissionsetitem/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/permissionsetitem/edit/'.$id);
		$this->_edit($id);
	}

    protected function onSaveSuccess($model) {
        PermissionSetController::sync_JSON_PERMISSION_SET_ITEMS($model->ID_PERMISSION_SET);
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'permissionsetitem_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'permissionsetitem_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('permissionsetitem');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'ID_PERMISSION_SET', 'MODULE', 'ACTIONS');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'permissionsetitem_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'ID_PERMISSION_SET', 'MODULE', 'ACTIONS');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('permissionsetitem'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'permissionsetitem_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'permissionsetitem_multiformdata_deleteditems'])? $_REQUEST[$prefix.'permissionsetitem_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('permissionsetitem_before_save', $model);

    		if ($model->UUID) {
				
				$old = new PermissionSetItemModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('permissionsetitem_before_update', $model, $old);

        		if (!AclController::hasPermission('permissionsetitem', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."PERMISSION_SET_ITEM.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('permissionsetitem_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('permissionsetitem_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('permissionsetitem_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('permissionsetitem');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('permissionsetitem', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.permissionsetitem.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('permissionsetitem', 'import');

        if (isset($_FILES['permissionsetitem_importfile'])) {
        	if (!$_FILES['permissionsetitem_importfile']['error']) {
	            $filepath = $_FILES['permissionsetitem_importfile']['tmp_name'];
        		$original = $_FILES['permissionsetitem_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/permissionsetitem/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('ID_PERMISSION_SET','MODULE','ACTIONS');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['permissionsetitem.list.model']) && is_object($_SESSION['permissionsetitem.list.model'])) {
			$model = clone $_SESSION['permissionsetitem.list.model'];
		} else {
        	$model = new PermissionSetItemModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'permissionsetitem_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/permissionsetitem_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/permissionsetitem_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('ID_PERMISSION_SET','MODULE','ACTIONS');
        $filename = 'permissionsetitem_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PermissionSetItem');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.permissionsetitem.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.permissionsetitem.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.permissionsetitem.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.permissionsetitem.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.permissionsetitem.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.permissionsetitem.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.permissionsetitem.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.permissionsetitem.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('ID_PERMISSION_SET', 'MODULE', 'ACTIONS');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['permissionsetitem.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['permissionsetitem.customview'])? $_SESSION['permissionsetitem.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('permissionsetitem', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('permissionsetitem', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('permissionsetitem');
        ...
        $_SESSION['permissionsetitem.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('permissionsetitem'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('permissionsetitem', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('permissionsetitem', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('permissionsetitem', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.permissionsetitem.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.permissionsetitem.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('permissionsetitem');

        $aclviewablecolumns = AclController::getAclEnabledColumns('permissionsetitem', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('permissionsetitem', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('permissionsetitem', 'new');
        }
        ...
        $ids = isset($_SESSION['permissionsetitem.list.ids'])? $_SESSION['permissionsetitem.list.ids'] : array();
        ...
        PluginManager::do_action('permissionsetitem_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('permissionsetitem', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('permissionsetitem', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('permissionsetitem', 'view'));
        ...
        PluginManager::do_action('permissionsetitem_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.permissionsetitem.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('permissionsetitem');
        ...
        $roweditablecolumns = array('ID_PERMISSION_SET', 'MODULE', 'ACTIONS');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('permissionsetitem', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('permissionsetitem', $model->UUID, $model);
            }
        ...
        if ($details->UUID) {
            if (isset($details->ID_PERMISSION_SET) && $details->ID_PERMISSION_SET) {
                $acleditablecolumns['ID_PERMISSION_SET'] = false;

                $roweditablecolumns = array_diff($roweditablecolumns, array('ID_PERMISSION_SET'));
            }
        }
        ...
        PluginManager::do_action('permissionsetitem_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('permissionsetitem', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('permissionsetitem'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('permissionsetitem'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    PermissionSetItemController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['permissionsetitem.list.model'] = clone $model;
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_PERMISSION_SET':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.ID_PERMISSION_SET IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.ID_PERMISSION_SET = '$value'");
                        }

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.$key IS NULL OR ".TABLE_PREFIX.".PERMISSION_SET_ITEM.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."PERMISSION_SET_ITEM.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.ID_PERMISSION_SET, `'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.MODULE, `'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.ACTIONS, `'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.ID, `'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.JSON, `'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.UUID, `'.TABLE_PREFIX.'PERMISSION_SET_ITEM`.WFID');
    ...
    protected function getParentWiseAclCheckString() {
        // Parent ids
        $pc = new PermissionSetController();
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."PERMISSION_SET_ITEM.ID_PERMISSION_SET IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."PERMISSION_SET_ITEM.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PERMISSION_SET_ITEM.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PERMISSION_SET_ITEM.ACTIONS LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PERMISSION_SET_ITEM.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.permissionsetitem.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('ID_PERMISSION_SET','MODULE','ACTIONS');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('permissionsetitem_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('permissionsetitem_imported', $model);
		}
