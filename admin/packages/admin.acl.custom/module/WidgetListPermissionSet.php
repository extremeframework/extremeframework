...
/**
 * Usage: <{module class="WidgetListPermissionSet" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.permissionset.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPermissionSet extends PermissionSetController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.permissionset.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('permissionset')) {
            $vars = array_keys(get_object_vars(new PermissionSetModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('permissionset', $preset, $presetvalue));
