...
/**
 * Usage: <{module class="WidgetListCustomAccessRight" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.customaccessright.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListCustomAccessRight extends CustomAccessRightController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.customaccessright.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('customaccessright')) {
            $vars = array_keys(get_object_vars(new CustomAccessRightModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('customaccessright', $preset, $presetvalue));
