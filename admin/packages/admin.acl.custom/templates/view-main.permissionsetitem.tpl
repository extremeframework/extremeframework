<div class="view-main">
    <{plugin key="permissionsetitem_view_top" args=$details}>
    ...
    <{/if}>
            ...
            <{html_ref_select multilingual="0" autocomplete="1" ajax="0" method="" id="input-module" class="input-module" name="`$prefix`permissionsetitem_formdata_MODULE" value=$details->MODULE text=$details->reftext_MODULE datasource="ADMIN_MODULE" datasourcename="AdminModule" valuecol="MODULE" textcol="NAME" sortcol="NAME"}>
...
<{/if}>            </tbody>
    ...
    <{plugin key="permissionsetitem_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        permissionsetitemview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        permissionsetitemview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                permissionsetitemview_apply_block_visibility(key, value);
            });
        }

        permissionsetitemview_apply_block_visibility = function (key, value) {
        ...
        permissionsetitemview_update_visibility_settings = function () {
            var settings = permissionsetitemview_get_visibility_settings();

            permissionsetitemview_apply_visibility_settings(settings);
        }

        $(function() {
            permissionsetitemview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/permissionsetitem/fieldSave",
//                        data: td.find(':input').serialize() + "&permissionsetitem_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
