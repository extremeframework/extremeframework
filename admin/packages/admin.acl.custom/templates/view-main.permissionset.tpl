<div class="view-main">
    <{plugin key="permissionset_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{_t('Permission set name')}></label>
            </td>
    ...
    <{plugin key="permissionset_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        permissionsetview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        permissionsetview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                permissionsetview_apply_block_visibility(key, value);
            });
        }

        permissionsetview_apply_block_visibility = function (key, value) {
        ...
        permissionsetview_update_visibility_settings = function () {
            var settings = permissionsetview_get_visibility_settings();

            permissionsetview_apply_visibility_settings(settings);
        }

        $(function() {
            permissionsetview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/permissionset/fieldSave",
//                        data: td.find(':input').serialize() + "&permissionset_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
