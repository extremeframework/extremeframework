...
/**
 * Usage: <{module class="WidgetListDashboardItemType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.dashboarditemtype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListDashboardItemType extends DashboardItemTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.dashboarditemtype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('dashboarditemtype')) {
            $vars = array_keys(get_object_vars(new DashboardItemTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('dashboarditemtype', $preset, $presetvalue));
