...
/**
 * Usage: <{module class="WidgetListDashboardItem" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.dashboarditem.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListDashboardItem extends DashboardItemController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.dashboarditem.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('dashboarditem')) {
            $vars = array_keys(get_object_vars(new DashboardItemModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('dashboarditem', $preset, $presetvalue));
