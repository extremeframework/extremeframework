...
class _DashboardController extends __AppController
{
    ...
    public function __construct() {
        ...
        PluginManager::do_action('dashboard_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('TITLE', $columns2check) && trim($model->TITLE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('dashboard', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^dashboard_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^dashboard_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^dashboard_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('dashboard');

        AclController::checkPermission('dashboard', 'list');

		ContextStack::register(APPLICATION_URL.'/dashboard/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."DASHBOARD.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
		...
		if (!empty($term)) {
            $model->whereAdd("REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR SUBTITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%'");
		}
		$model->find();
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('dashboard_before_delete', $_model);
        }

		$model = new DashboardModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('dashboard', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."DASHBOARD.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('dashboard', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['dashboardlist_selection_selectall']) && $_REQUEST['dashboardlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['dashboard.list.model']) && is_object($_SESSION['dashboard.list.model'])) {
					$model = clone $_SESSION['dashboard.list.model'];
				} else {
		        	$model = new DashboardModel();
				}
    			...
    			$selection = isset($_REQUEST['dashboardlist_selection'])? $_REQUEST['dashboardlist_selection'] : '';
			}
        }
        ...
        if ($module != 'dashboard' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('dashboard', 'new');

		ContextStack::register(APPLICATION_URL.'/dashboard/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('dashboard', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new DashboardModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('dashboard', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/dashboard/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
        ...
        AclController::checkPermission('dashboard', 'edit');
		...
		ContextStack::register(APPLICATION_URL.'/dashboard/edit/'.$id);
		$this->_edit($id);
	}
    ...
    static function sync_JSON_DASHBOARD_ITEMS($id) {
        $model = new DashboardItemModel();
        $model->ID_DASHBOARD = $id;
        ...
        DashboardController::update_json($id, 'DASHBOARD_ITEM', $items);
    }
    ...
    protected function getTempCreateId() {
        ...
        if (!isset($_SESSION['dashboard.tmpid'])) {
            $create = true;
        } else {
            $model = new DashboardModel();

            $model->UUID = $_SESSION['dashboard.tmpid'];
        ...
        if ($create) {
            ...
            $_SESSION['dashboard.tmpid'] = $model->UUID;
        }

        return $_SESSION['dashboard.tmpid'];
    }

    protected function clearTempCreateId() {
        if (isset($_SESSION['dashboard.tmpid'])) {
            unset($_SESSION['dashboard.tmpid']);
        }
    }

    protected function clearTempCreateItem() {
        if (isset($_SESSION['dashboard.tmpid'])) {
            self::deleteItem($_SESSION['dashboard.tmpid']);
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'dashboard_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'dashboard_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('dashboard');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'NAME', 'TITLE', 'SUBTITLE', 'ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'dashboard_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'TITLE', 'SUBTITLE', 'ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('dashboard'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'dashboard_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'dashboard_multiformdata_deleteditems'])? $_REQUEST[$prefix.'dashboard_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('dashboard', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('dashboard_before_save', $model);

    		if ($model->UUID) {
				
				$old = new DashboardModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('dashboard_before_update', $model, $old);

        		if (!AclController::hasPermission('dashboard', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."DASHBOARD.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."DASHBOARD.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."DASHBOARD.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('dashboard_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('dashboard', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.dashboard.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
		    ...
		    ContextStack::replace(APPLICATION_URL.'/dashboard/view/'.$model->UUID);
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.dashboard.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.dashboard.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.dashboard.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.dashboard.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.dashboard.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.dashboard.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.dashboard.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.dashboard.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.dashboard.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.dashboard.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.dashboard.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('NAME', 'ID_ADMIN_PACKAGE');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['dashboard.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['dashboard.customview'])? $_SESSION['dashboard.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('dashboard', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('dashboard', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('dashboard');
        ...
        $_SESSION['dashboard.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('dashboard'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('dashboard', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('dashboard', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('dashboard', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.dashboard.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.dashboard.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('dashboard');

        $aclviewablecolumns = AclController::getAclEnabledColumns('dashboard', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('dashboard', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('dashboard', 'new');
        }
        ...
        $ids = isset($_SESSION['dashboard.list.ids'])? $_SESSION['dashboard.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('dashboard', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('dashboard', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('dashboard', 'view'));
        ...
        PluginManager::do_action('dashboard_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.dashboard.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('dashboard');
        ...
        $roweditablecolumns = array('NAME', 'TITLE', 'SUBTITLE', 'ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('dashboard', 'editpeer');
                }
            } else {
                ...
                PluginManager::do_action('dashboard_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('dashboard', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('dashboard', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('dashboard'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('dashboard'));

	    $this->display($smarty, $templatecode);
	}

    protected function getLayoutColumns() {
        return array('NAME', 'TITLE', 'SUBTITLE', 'ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('dashboard', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."DASHBOARD.UDID = 0 OR ".TABLE_PREFIX."DASHBOARD.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."DASHBOARD.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    DashboardController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['dashboard.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."DASHBOARD.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'TITLE':
                        $model->whereAdd(TABLE_PREFIX."DASHBOARD.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'ID_ADMIN_PACKAGE':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."DASHBOARD.ID_ADMIN_PACKAGE IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."DASHBOARD.ID_ADMIN_PACKAGE = '$value'");
                        }
                    ...
                    case 'ID_ADMIN_MENU':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."DASHBOARD.ID_ADMIN_MENU IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."DASHBOARD.ID_ADMIN_MENU = '$value'");
                        }
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."DASHBOARD.$key IS NULL OR ".TABLE_PREFIX.".DASHBOARD.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."DASHBOARD.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'DASHBOARD`.NAME, `'.TABLE_PREFIX.'DASHBOARD`.TITLE, `'.TABLE_PREFIX.'DASHBOARD`.SUBTITLE, `'.TABLE_PREFIX.'DASHBOARD`.ID_ADMIN_PACKAGE, `'.TABLE_PREFIX.'DASHBOARD`.ID_ADMIN_MENU, `'.TABLE_PREFIX.'DASHBOARD`.ID, `'.TABLE_PREFIX.'DASHBOARD`.JSON, `'.TABLE_PREFIX.'DASHBOARD`.UUID, `'.TABLE_PREFIX.'DASHBOARD`.WFID');
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."DASHBOARD.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD.SUBTITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
