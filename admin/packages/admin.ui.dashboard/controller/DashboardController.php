...
{
    function showDashoard($id_dashboard) {
        ...
        while ($model->fetch()) {
            if ($model->ID_DASHBOARD_ITEM_TYPE == 'block') {
                $blockitems[] = clone $model;
            } else {
                $linkitems[] = clone $model;
            }
        }
        ...
        $smarty->assign('blockitems', $blockitems);
        $smarty->assign('linkitems', $linkitems);

		$this->display($smarty, 'dashboard.tpl');
    }
