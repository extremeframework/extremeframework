...
class _DashboardItemTypeController extends __AppController
{
    var $module = 'dashboarditemtype';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('dashboarditemtype_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('CODE', $columns2check)) {
           $_model = new DashboardItemTypeModel();
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('dashboarditemtype', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^dashboarditemtype_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^dashboarditemtype_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^dashboarditemtype_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('dashboarditemtype');

        ContextStack::check2redirect('dashboarditemtype');

        AclController::checkPermission('dashboarditemtype', 'list');

		ContextStack::register(APPLICATION_URL.'/dashboarditemtype/list/');
    ...
    public function updateOrderAction() {
        ...
        AclController::checkPermission('dashboarditemtype', 'edit');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."DASHBOARD_ITEM_TYPE.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'dashboarditemtype');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('dashboarditemtype_before_delete', $_model);
        }

		$model = new DashboardItemTypeModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('dashboarditemtype', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('dashboarditemtype', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('dashboarditemtype_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('dashboarditemtype', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['dashboarditemtypelist_selection_selectall']) && $_REQUEST['dashboarditemtypelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['dashboarditemtype.list.model']) && is_object($_SESSION['dashboarditemtype.list.model'])) {
					$model = clone $_SESSION['dashboarditemtype.list.model'];
				} else {
		        	$model = new DashboardItemTypeModel();
				}
    			...
    			$selection = isset($_REQUEST['dashboarditemtypelist_selection'])? $_REQUEST['dashboarditemtypelist_selection'] : '';
			}
        }
        ...
        if ($module != 'dashboarditemtype' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('dashboarditemtype', 'new');

		ContextStack::register(APPLICATION_URL.'/dashboarditemtype/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('dashboarditemtype', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new DashboardItemTypeModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('dashboarditemtype', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/dashboarditemtype/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/dashboarditemtype/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'dashboarditemtype_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'dashboarditemtype_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('dashboarditemtype');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'dashboarditemtype_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'CODE', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('dashboarditemtype'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'dashboarditemtype_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'dashboarditemtype_multiformdata_deleteditems'])? $_REQUEST[$prefix.'dashboarditemtype_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('dashboarditemtype', $model);
            
            
            $this->onBeforeSave($model);
            PluginManager::do_action('dashboarditemtype_before_save', $model);

    		if ($model->UUID) {
				
				$old = new DashboardItemTypeModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('dashboarditemtype_before_update', $model, $old);

        		if (!AclController::hasPermission('dashboarditemtype', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."DASHBOARD_ITEM_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('dashboarditemtype_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('dashboarditemtype_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('dashboarditemtype_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('dashboarditemtype');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('dashboarditemtype', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.dashboarditemtype.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('dashboarditemtype', 'import');

        if (isset($_FILES['dashboarditemtype_importfile'])) {
        	if (!$_FILES['dashboarditemtype_importfile']['error']) {
	            $filepath = $_FILES['dashboarditemtype_importfile']['tmp_name'];
        		$original = $_FILES['dashboarditemtype_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/dashboarditemtype/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['dashboarditemtype.list.model']) && is_object($_SESSION['dashboarditemtype.list.model'])) {
			$model = clone $_SESSION['dashboarditemtype.list.model'];
		} else {
        	$model = new DashboardItemTypeModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'dashboarditemtype_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/dashboarditemtype_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/dashboarditemtype_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','ORDERING');
        $filename = 'dashboarditemtype_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('DashboardItemType');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.dashboarditemtype.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.dashboarditemtype.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.dashboarditemtype.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.dashboarditemtype.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.dashboarditemtype.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.dashboarditemtype.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.dashboarditemtype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.dashboarditemtype.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.dashboarditemtype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.dashboarditemtype.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.dashboarditemtype.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['dashboarditemtype.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['dashboarditemtype.customview'])? $_SESSION['dashboarditemtype.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('dashboarditemtype', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('dashboarditemtype', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('dashboarditemtype');
        ...
        $_SESSION['dashboarditemtype.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('dashboarditemtype'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('dashboarditemtype', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('dashboarditemtype', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('dashboarditemtype', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.dashboarditemtype.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.dashboarditemtype.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('dashboarditemtype');

        $aclviewablecolumns = AclController::getAclEnabledColumns('dashboarditemtype', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('dashboarditemtype', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('dashboarditemtype', 'new');
        }
        ...
        $ids = isset($_SESSION['dashboarditemtype.list.ids'])? $_SESSION['dashboarditemtype.list.ids'] : array();
        ...
        PluginManager::do_action('dashboarditemtype_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('dashboarditemtype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('dashboarditemtype', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('dashboarditemtype', 'view'));
        ...
        PluginManager::do_action('dashboarditemtype_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.dashboarditemtype.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('dashboarditemtype');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('dashboarditemtype', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('dashboarditemtype_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('dashboarditemtype', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('dashboarditemtype_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('dashboarditemtype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('dashboarditemtype'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('dashboarditemtype'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('dashboarditemtype', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.UDID = 0 OR ".TABLE_PREFIX."DASHBOARD_ITEM_TYPE.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."DASHBOARD_ITEM_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    DashboardItemTypeController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['dashboarditemtype.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.$key IS NULL OR ".TABLE_PREFIX.".DASHBOARD_ITEM_TYPE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."DASHBOARD_ITEM_TYPE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.NAME, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.CODE, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.ID, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.JSON, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.UUID, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.WFID');
    }

    protected function initListModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.NAME, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.CODE, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.ORDERING, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.ID, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.JSON, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.UUID, `'.TABLE_PREFIX.'DASHBOARD_ITEM_TYPE`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."DASHBOARD_ITEM_TYPE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD_ITEM_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD_ITEM_TYPE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."DASHBOARD_ITEM_TYPE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.dashboarditemtype.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('dashboarditemtype_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('dashboarditemtype_imported', $model);
		}
