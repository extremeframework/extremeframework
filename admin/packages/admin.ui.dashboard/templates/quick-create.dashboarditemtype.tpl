<div class="modal-header">
    ...
    <h3><{_t('Quick create')|ucwords}> <{_t('Dashboard Item Type')|ucwords}></h3>
</div>
...
<tr class="form-row form-row-name form-row-mandatory">
        ...
        <{if $preset == 'NAME'}>
            <input type="hidden" class="input-name" name="dashboarditemtype_formdata_NAME" value="<{$presetvalue}>" />
        <{elseif isset($acleditablecolumns['NAME']) && !$acleditablecolumns['NAME'] || !isset($acleditablecolumns['*']) && !isset($acleditablecolumns['NAME'])}>
            <input type="hidden" class="input-name" name="dashboarditemtype_formdata_NAME" value="<{$details->NAME}>" />
        <{else}>
    		<td class="form-field form-field-label">
    		    <label><{_t('Dashboard item type name')}><span class="mandatory">*</span></label>
            </td>
            <td class="form-field form-field-value column-name" colspan="3">
                                    

    <input class="input-name input-type-text" type="text" name="<{$prefix}>dashboarditemtype_formdata_NAME" value="<{$details->NAME|escape}>"  />
...
<tr class="form-row form-row-code form-row-mandatory">
        ...
        <{if $preset == 'CODE'}>
            <input type="hidden" class="input-code" name="dashboarditemtype_formdata_CODE" value="<{$presetvalue}>" />
        <{elseif isset($acleditablecolumns['CODE']) && !$acleditablecolumns['CODE'] || !isset($acleditablecolumns['*']) && !isset($acleditablecolumns['CODE'])}>
            <input type="hidden" class="input-code" name="dashboarditemtype_formdata_CODE" value="<{$details->CODE}>" />
        <{else}>
    		<td class="form-field form-field-label">
    ...
    <input class="input-code input-type-text" type="text" name="<{$prefix}>dashboarditemtype_formdata_CODE" value="<{$details->CODE|escape}>"  />
...
<tr class="form-row form-row-ordering">
        ...
        <{if $preset == 'ORDERING'}>
            <input type="hidden" class="input-ordering" name="dashboarditemtype_formdata_ORDERING" value="<{$presetvalue}>" />
        <{elseif isset($acleditablecolumns['ORDERING']) && !$acleditablecolumns['ORDERING'] || !isset($acleditablecolumns['*']) && !isset($acleditablecolumns['ORDERING'])}>
            <input type="hidden" class="input-ordering" name="dashboarditemtype_formdata_ORDERING" value="<{$details->ORDERING}>" />
        <{else}>
    		<td class="form-field form-field-label">
        ...
        <input class="input-ordering input-type-number number-format" type="text" name="<{$prefix}>dashboarditemtype_formdata_ORDERING" value="<{if $details->ORDERING != 0}><{$details->ORDERING}><{/if}>"  />
