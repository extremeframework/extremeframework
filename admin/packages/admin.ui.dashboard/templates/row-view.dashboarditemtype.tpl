<tr id="item_<{$row->UUID}>" class="row-view <{if $counter%2 }>row1<{else}>row0<{/if}> -<{$row->reftext_|ascode}>" data-id="<{$row->UUID}>">
	...
	<td class="checkbox">
		<input type="checkbox" id="chk-dashboarditemtype-<{$row->UUID}>" class="dashboarditemtypelistchk" name="dashboarditemtypelist_selection[]" value="<{$row->UUID}>" />
	...
	<{/foreach}>
	<{plugin key="dashboarditemtype_list_columns_data" args=$row}>
	<td class="actions">
	    <{if isset($smarty.session.acl.dashboarditemtype.edit) && WorkflowHelper::isEditable($row->WFID) && !$readonly}>
            <a class="edit scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/dashboarditemtype/edit/<{$row->UUID}>"><i class="fa fa-pencil-square-o" title="<{_t('Edit', true)}>"></i></a>
        <{/if}>
    	    ...
    	    <span class="dropdown">
    	        ...
    	        <ul class="dropdown-menu pull-right">
    	            <{foreach from=$admin_view_options item=view}>
                        <li><a <{if $view->IS_PRINT_VIEW}>target="_blank"<{/if}> href="<{$smarty.const.APPLICATION_URL}>/dashboarditemtype/view/<{$row->UUID}>/<{$view->CODE}>"><{$view->TITLE}></a></li>
                    <{/foreach}>
                </ul>
    	    </span>
        <{/if}>
