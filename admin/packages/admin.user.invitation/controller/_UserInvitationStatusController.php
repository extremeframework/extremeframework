...
class _UserInvitationStatusController extends __AppController
{
    var $module = 'userinvitationstatus';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('userinvitationstatus_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('userinvitationstatus', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^userinvitationstatus_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userinvitationstatus_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userinvitationstatus_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('userinvitationstatus');

        ContextStack::check2redirect('userinvitationstatus');

        AclController::checkPermission('userinvitationstatus', 'list');

		ContextStack::register(APPLICATION_URL.'/userinvitationstatus/list/');
    ...
    public function updateOrderAction() {
        ...
        AclController::checkPermission('userinvitationstatus', 'edit');
    ...
    public function customFilterAction() {
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            $afm->UUID = $id;
            $afm->MODULE = 'userinvitationstatus';
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."USER_INVITATION_STATUS.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'userinvitationstatus');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new UserInvitationStatusModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('userinvitationstatus_before_delete', $_model);
        }

		$model = new UserInvitationStatusModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('userinvitationstatus', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('userinvitationstatus', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('userinvitationstatus_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('userinvitationstatus', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['userinvitationstatuslist_selection_selectall']) && $_REQUEST['userinvitationstatuslist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['userinvitationstatus.list.model']) && is_object($_SESSION['userinvitationstatus.list.model'])) {
					$model = clone $_SESSION['userinvitationstatus.list.model'];
				} else {
		        	$model = new UserInvitationStatusModel();
				}
    			...
    			$selection = isset($_REQUEST['userinvitationstatuslist_selection'])? $_REQUEST['userinvitationstatuslist_selection'] : '';
			}
        }
        ...
        if ($module != 'userinvitationstatus' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('userinvitationstatus', 'new');

		ContextStack::register(APPLICATION_URL.'/userinvitationstatus/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('userinvitationstatus', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new UserInvitationStatusModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('userinvitationstatus', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/userinvitationstatus/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/userinvitationstatus/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userinvitationstatus_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'userinvitationstatus_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('userinvitationstatus');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'userinvitationstatus_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'CODE', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('userinvitationstatus'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userinvitationstatus_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'userinvitationstatus_multiformdata_deleteditems'])? $_REQUEST[$prefix.'userinvitationstatus_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('userinvitationstatus', $model);
            
            
            $this->onBeforeSave($model);
            PluginManager::do_action('userinvitationstatus_before_save', $model);

    		if ($model->UUID) {
				
				$old = new UserInvitationStatusModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('userinvitationstatus_before_update', $model, $old);

        		if (!AclController::hasPermission('userinvitationstatus', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."USER_INVITATION_STATUS.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('userinvitationstatus_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('userinvitationstatus_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('userinvitationstatus_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('userinvitationstatus');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('userinvitationstatus', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.userinvitationstatus.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('userinvitationstatus', 'import');

        if (isset($_FILES['userinvitationstatus_importfile'])) {
        	if (!$_FILES['userinvitationstatus_importfile']['error']) {
	            $filepath = $_FILES['userinvitationstatus_importfile']['tmp_name'];
        		$original = $_FILES['userinvitationstatus_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/userinvitationstatus/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['userinvitationstatus.list.model']) && is_object($_SESSION['userinvitationstatus.list.model'])) {
			$model = clone $_SESSION['userinvitationstatus.list.model'];
		} else {
        	$model = new UserInvitationStatusModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'userinvitationstatus_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/userinvitationstatus_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/userinvitationstatus_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','ORDERING');
        $filename = 'userinvitationstatus_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('UserInvitationStatus');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.userinvitationstatus.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.userinvitationstatus.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.userinvitationstatus.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.userinvitationstatus.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.userinvitationstatus.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.userinvitationstatus.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.userinvitationstatus.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.userinvitationstatus.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.userinvitationstatus.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.userinvitationstatus.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.userinvitationstatus.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['userinvitationstatus.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['userinvitationstatus.customview'])? $_SESSION['userinvitationstatus.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('userinvitationstatus', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('userinvitationstatus', 'view');
        ...
        $_SESSION['userinvitationstatus.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userinvitationstatus'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('userinvitationstatus', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('userinvitationstatus', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userinvitationstatus', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.userinvitationstatus.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.userinvitationstatus.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('userinvitationstatus');

        $aclviewablecolumns = AclController::getAclEnabledColumns('userinvitationstatus', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('userinvitationstatus', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('userinvitationstatus', 'new');
        }
        ...
        PluginManager::do_action('userinvitationstatus_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userinvitationstatus', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('userinvitationstatus', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userinvitationstatus', 'view'));
        ...
        PluginManager::do_action('userinvitationstatus_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.userinvitationstatus.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('userinvitationstatus');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('userinvitationstatus', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('userinvitationstatus_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('userinvitationstatus', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('userinvitationstatus_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userinvitationstatus', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('userinvitationstatus'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('userinvitationstatus'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('userinvitationstatus', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.UDID = 0 OR ".TABLE_PREFIX."USER_INVITATION_STATUS.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."USER_INVITATION_STATUS.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    UserInvitationStatusController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['userinvitationstatus.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.$key IS NULL OR ".TABLE_PREFIX.".USER_INVITATION_STATUS.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."USER_INVITATION_STATUS.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'USER_INVITATION_STATUS`.NAME, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.CODE, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.ID, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.JSON, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.UUID, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.WFID');
    }

    protected function initListModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'USER_INVITATION_STATUS`.NAME, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.CODE, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.ORDERING, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.ID, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.JSON, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.UUID, `'.TABLE_PREFIX.'USER_INVITATION_STATUS`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."USER_INVITATION_STATUS.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_INVITATION_STATUS.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_INVITATION_STATUS.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_INVITATION_STATUS.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.userinvitationstatus.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('userinvitationstatus_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('userinvitationstatus_imported', $model);
		}
