...
class _UserInvitationController extends __AppController
{
    ...
    public function __construct() {
        ...
        PluginManager::do_action('userinvitation_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('INVITEE_EMAIL', $columns2check) && !empty($model->INVITEE_EMAIL) && !preg_match('/^([.0-9a-z_-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,4})$/i', $model->INVITEE_EMAIL)) {
           $errors['invitee-email'] = sprintf(_t('L_VALIDATION_EMAIL'), _t('Invitee email'));
           return false;
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('userinvitation', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^userinvitation_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userinvitation_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userinvitation_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('userinvitation');

        AclController::checkPermission('userinvitation', 'list');

		ContextStack::register(APPLICATION_URL.'/userinvitation/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."USER_INVITATION.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
        ...
        $columns2return = array('ID' => 'id', 'INVITEE_NAME' => 'name');

        $term = isset($_REQUEST['term'])? $_REQUEST['term'] : '';

		$model = new UserInvitationModel();
		if (!empty($term)) {
            $model->whereAdd("REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR INVITEE_NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%'");
		}
		$model->find();
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$rows = array();
     	while ($model->fetch()) {
     		$rows[] = array('id' => $model->UUID, 'eid' => $model->ID, 'title' => $model->INVITEE_NAME);
		}

		$smarty = Framework::getSmarty(__FILE__);

		$smarty->assign('rows', $rows);
		$smarty->assign('module', 'userinvitation');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new UserInvitationModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('userinvitation_before_delete', $_model);
        }

		$model = new UserInvitationModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('userinvitation', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('userinvitation', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('userinvitation_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('userinvitation', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['userinvitationlist_selection_selectall']) && $_REQUEST['userinvitationlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['userinvitation.list.model']) && is_object($_SESSION['userinvitation.list.model'])) {
					$model = clone $_SESSION['userinvitation.list.model'];
				} else {
		        	$model = new UserInvitationModel();
				}
    			...
    			$selection = isset($_REQUEST['userinvitationlist_selection'])? $_REQUEST['userinvitationlist_selection'] : '';
			}
        }
        ...
        if ($module != 'userinvitation' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
		...
		ContextStack::register(APPLICATION_URL.'/userinvitation/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('userinvitation', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new UserInvitationModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('userinvitation', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/userinvitation/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
        ...
        AclController::checkPermission('userinvitation', 'edit');
		...
		ContextStack::register(APPLICATION_URL.'/userinvitation/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userinvitation_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'userinvitation_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('userinvitation');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'INVITEE_NAME', 'INVITEE_EMAIL', 'ID_USER_GROUP', 'ID_USER_ROLE', 'VALID_FROM', 'VALID_UNTIL');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'userinvitation_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'INVITEE_NAME', 'INVITEE_EMAIL', 'ID_USER_GROUP', 'ID_USER_ROLE', 'VALID_FROM', 'VALID_UNTIL');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('userinvitation'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userinvitation_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'userinvitation_multiformdata_deleteditems'])? $_REQUEST[$prefix.'userinvitation_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('userinvitation', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('userinvitation_before_save', $model);

    		if ($model->UUID) {
				
				$old = new UserInvitationModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('userinvitation_before_update', $model, $old);

        		if (!AclController::hasPermission('userinvitation', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."USER_INVITATION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('userinvitation_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('userinvitation_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('userinvitation_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('userinvitation');
    ...
    protected function bind2refobject(&$model, $refobject = null) {
        if ($refobject != null) {
            ...
            if ($refclass == 'UserInvitationStatusModel' && empty($model->ID_USER_INVITATION_STATUS)) {
                $model->ID_USER_INVITATION_STATUS = $refobject->CODE;
            }
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('userinvitation', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.userinvitation.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('userinvitation', 'import');

        if (isset($_FILES['userinvitation_importfile'])) {
        	if (!$_FILES['userinvitation_importfile']['error']) {
	            $filepath = $_FILES['userinvitation_importfile']['tmp_name'];
        		$original = $_FILES['userinvitation_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/userinvitation/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('INVITEE_NAME','INVITEE_EMAIL','ID_USER_GROUP','ID_USER_ROLE','VALID_FROM','VALID_UNTIL','ID_USER_INVITATION_STATUS','DATE_SENT','DATE_ACCEPTED');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['userinvitation.list.model']) && is_object($_SESSION['userinvitation.list.model'])) {
			$model = clone $_SESSION['userinvitation.list.model'];
		} else {
        	$model = new UserInvitationModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'userinvitation_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/userinvitation_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/userinvitation_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('INVITEE_NAME','INVITEE_EMAIL','ID_USER_GROUP','ID_USER_ROLE','VALID_FROM','VALID_UNTIL','ID_USER_INVITATION_STATUS','DATE_SENT','DATE_ACCEPTED');
        $filename = 'userinvitation_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('UserInvitation');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.userinvitation.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.userinvitation.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.userinvitation.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.userinvitation.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.userinvitation.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.userinvitation.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.userinvitation.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.userinvitation.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.userinvitation.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.userinvitation.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.userinvitation.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('INVITEE_NAME', 'INVITEE_EMAIL', 'ID_USER_GROUP', 'ID_USER_ROLE', 'ID_USER_INVITATION_STATUS');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['userinvitation.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['userinvitation.customview'])? $_SESSION['userinvitation.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('userinvitation', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('userinvitation', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('userinvitation');
        ...
        $_SESSION['userinvitation.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userinvitation'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('userinvitation', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('userinvitation', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userinvitation', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.userinvitation.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.userinvitation.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('userinvitation');

        $aclviewablecolumns = AclController::getAclEnabledColumns('userinvitation', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('userinvitation', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('userinvitation', 'new');
        }
        ...
        $ids = isset($_SESSION['userinvitation.list.ids'])? $_SESSION['userinvitation.list.ids'] : array();
        ...
        PluginManager::do_action('userinvitation_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userinvitation', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('userinvitation', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userinvitation', 'view'));
        ...
        PluginManager::do_action('userinvitation_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.userinvitation.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('userinvitation');
        ...
        $roweditablecolumns = array('INVITEE_NAME', 'INVITEE_EMAIL', 'ID_USER_GROUP', 'ID_USER_ROLE', 'VALID_FROM', 'VALID_UNTIL', 'ID_USER_INVITATION_STATUS', 'DATE_SENT', 'DATE_ACCEPTED');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('userinvitation', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('userinvitation', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('userinvitation_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userinvitation', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('userinvitation'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('userinvitation'));

	    $this->display($smarty, $templatecode);
	}

    protected function getLayoutColumns() {
        return array('INVITEE_NAME', 'INVITEE_EMAIL', 'ID_USER_GROUP', 'ID_USER_ROLE', 'VALID_FROM', 'VALID_UNTIL', 'ID_USER_INVITATION_STATUS', 'DATE_SENT', 'DATE_ACCEPTED');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('userinvitation', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."USER_INVITATION.UDID = 0 OR ".TABLE_PREFIX."USER_INVITATION.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."USER_INVITATION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    UserInvitationController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['userinvitation.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_USER_GROUP':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.ID_USER_GROUP IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.ID_USER_GROUP = '$value'");
                        }
                    ...
                    case 'ID_USER_ROLE':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.ID_USER_ROLE IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.ID_USER_ROLE = '$value'");
                        }

                        break;

                    case 'VALID_FROM__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.VALID_FROM >= '".$this->field_sanitize('VALID_FROM', $value)."'");

                        break;

                    case 'VALID_FROM__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.VALID_FROM IS NULL OR ".TABLE_PREFIX."USER_INVITATION.VALID_FROM <= '".$this->field_sanitize('VALID_FROM', $value)."')");

                        break;

                    case 'VALID_UNTIL__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.VALID_UNTIL >= '".$this->field_sanitize('VALID_UNTIL', $value)."'");

                        break;

                    case 'VALID_UNTIL__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.VALID_UNTIL IS NULL OR ".TABLE_PREFIX."USER_INVITATION.VALID_UNTIL <= '".$this->field_sanitize('VALID_UNTIL', $value)."')");

                        break;

                    case 'ID_USER_INVITATION_STATUS':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.ID_USER_INVITATION_STATUS IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."USER_INVITATION.ID_USER_INVITATION_STATUS = '$value'");
                        }

                        break;

                    case 'DATE_SENT__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.DATE_SENT >= '".$this->field_sanitize('DATE_SENT', $value)."'");

                        break;

                    case 'DATE_SENT__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.DATE_SENT IS NULL OR ".TABLE_PREFIX."USER_INVITATION.DATE_SENT <= '".$this->field_sanitize('DATE_SENT', $value)."')");

                        break;

                    case 'DATE_ACCEPTED__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.DATE_ACCEPTED >= '".$this->field_sanitize('DATE_ACCEPTED', $value)."'");

                        break;

                    case 'DATE_ACCEPTED__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_INVITATION.DATE_ACCEPTED IS NULL OR ".TABLE_PREFIX."USER_INVITATION.DATE_ACCEPTED <= '".$this->field_sanitize('DATE_ACCEPTED', $value)."')");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."USER_INVITATION.$key IS NULL OR ".TABLE_PREFIX.".USER_INVITATION.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."USER_INVITATION.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'USER_INVITATION`.INVITEE_NAME, `'.TABLE_PREFIX.'USER_INVITATION`.INVITEE_EMAIL, `'.TABLE_PREFIX.'USER_INVITATION`.ID_USER_GROUP, `'.TABLE_PREFIX.'USER_INVITATION`.ID_USER_ROLE, `'.TABLE_PREFIX.'USER_INVITATION`.VALID_FROM, `'.TABLE_PREFIX.'USER_INVITATION`.VALID_UNTIL, `'.TABLE_PREFIX.'USER_INVITATION`.ID_USER_INVITATION_STATUS, `'.TABLE_PREFIX.'USER_INVITATION`.DATE_SENT, `'.TABLE_PREFIX.'USER_INVITATION`.DATE_ACCEPTED, `'.TABLE_PREFIX.'USER_INVITATION`.ID, `'.TABLE_PREFIX.'USER_INVITATION`.JSON, `'.TABLE_PREFIX.'USER_INVITATION`.UUID, `'.TABLE_PREFIX.'USER_INVITATION`.WFID');
        ...
        if ($join) {
            ...
            if (Framework::hasModule('UserInvitationStatus')) {
                $model->selectAdd('reftable_ID_USER_INVITATION_STATUS.NAME as reftext_ID_USER_INVITATION_STATUS');
                $model->selectAdd('reftable_ID_USER_INVITATION_STATUS.UUID as refuuid_ID_USER_INVITATION_STATUS');
            }
        }
        ...
        if ($join) {
            ...
            if (Framework::hasModule('UserInvitationStatus')) {
                $model->joinAdd(array('ID_USER_INVITATION_STATUS',TABLE_PREFIX.'USER_INVITATION_STATUS:CODE'), 'LEFT', 'reftable_ID_USER_INVITATION_STATUS');
            }
        }
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."USER_INVITATION.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_INVITATION.INVITEE_NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_INVITATION.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.userinvitation.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('INVITEE_NAME','INVITEE_EMAIL','ID_USER_GROUP','ID_USER_ROLE','VALID_FROM','VALID_UNTIL','ID_USER_INVITATION_STATUS','DATE_SENT','DATE_ACCEPTED');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('userinvitation_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('userinvitation_imported', $model);
		}
    ...
    protected function _label2refval($refcolumn, $reflabel) {
        ...
        if (isset($valuecache[$refcolumn][$reflabel])) {
            ...
            switch ($refcolumn) {
                ...
                case 'ID_USER_INVITATION_STATUS':
                    $model = new UserInvitationStatusModel();
                    $model->NAME = $reflabel;
                    if ($model->find(1)) {
                        $value = $model->CODE;
                    } else {
