...
class _UserExtraDomainController extends __AppController
{
    ...
    public function __construct() {
        ...
        PluginManager::do_action('userextradomain_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('EXTRA_UDID', $columns2check) && !empty($model->EXTRA_UDID) && !is_numeric($model->EXTRA_UDID)) {
           $errors['extra-udid'] = sprintf(_t('L_VALIDATION_NUMBER'), _t('Extra udid'));
           return false;
       }
       if (in_array('ID_USER', $columns2check) || in_array('EXTRA_UDID', $columns2check)) {
           $_model = new UserExtraDomainModel();
           $_model->ID_USER = $model->ID_USER;
           $_model->EXTRA_UDID = $model->EXTRA_UDID;
           ...
           $_model->find();
           if ($_model->N) {
               $errors['id-user+extra-udid'] = sprintf(L_VALIDATION_ALREADY_EXISTS, '{'.L_USER.', '.L_EXTRA_UDID.'}');
               return false;
           }
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('userextradomain', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^userextradomain_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userextradomain_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^userextradomain_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('userextradomain');

        ContextStack::check2redirect('userextradomain');

        AclController::checkPermission('userextradomain', 'list');

		ContextStack::register(APPLICATION_URL.'/userextradomain/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."USER_EXTRA_DOMAIN.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'userextradomain');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('userextradomain_before_delete', $_model);
        }

		$model = new UserExtraDomainModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('userextradomain', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('userextradomain', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('userextradomain_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('userextradomain', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['userextradomainlist_selection_selectall']) && $_REQUEST['userextradomainlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['userextradomain.list.model']) && is_object($_SESSION['userextradomain.list.model'])) {
					$model = clone $_SESSION['userextradomain.list.model'];
				} else {
		        	$model = new UserExtraDomainModel();
				}
    			...
    			$selection = isset($_REQUEST['userextradomainlist_selection'])? $_REQUEST['userextradomainlist_selection'] : '';
			}
        }
        ...
        if ($module != 'userextradomain' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('userextradomain', 'new');

		ContextStack::register(APPLICATION_URL.'/userextradomain/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('userextradomain', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new UserExtraDomainModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('userextradomain', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/userextradomain/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
        ...
        AclController::checkPermission('userextradomain', 'edit');
		...
		ContextStack::register(APPLICATION_URL.'/userextradomain/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userextradomain_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'userextradomain_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function field_sanitize($column, $value) {
		if ($column == 'EXTRA_UDID') {
            $value = preg_replace('/[^0-9\.]/is', '', $value);
        } elseif ($column == 'VALID_FROM' && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/', $value)) {
            $format = DATE_FORMAT;
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('userextradomain');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'ID_USER', 'EXTRA_UDID', 'VALID_FROM', 'VALID_UNTIL');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'userextradomain_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'ID_USER', 'EXTRA_UDID', 'VALID_FROM', 'VALID_UNTIL');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('userextradomain'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'userextradomain_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'userextradomain_multiformdata_deleteditems'])? $_REQUEST[$prefix.'userextradomain_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('userextradomain', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('userextradomain_before_save', $model);

    		if ($model->UUID) {
				
				$old = new UserExtraDomainModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('userextradomain_before_update', $model, $old);

        		if (!AclController::hasPermission('userextradomain', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."USER_EXTRA_DOMAIN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('userextradomain_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('userextradomain_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('userextradomain_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('userextradomain');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('userextradomain', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.userextradomain.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('userextradomain', 'import');

        if (isset($_FILES['userextradomain_importfile'])) {
        	if (!$_FILES['userextradomain_importfile']['error']) {
	            $filepath = $_FILES['userextradomain_importfile']['tmp_name'];
        		$original = $_FILES['userextradomain_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/userextradomain/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('ID_USER','EXTRA_UDID','VALID_FROM','VALID_UNTIL');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['userextradomain.list.model']) && is_object($_SESSION['userextradomain.list.model'])) {
			$model = clone $_SESSION['userextradomain.list.model'];
		} else {
        	$model = new UserExtraDomainModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'userextradomain_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/userextradomain_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/userextradomain_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('ID_USER','EXTRA_UDID','VALID_FROM','VALID_UNTIL');
        $filename = 'userextradomain_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('UserExtraDomain');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.userextradomain.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.userextradomain.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.userextradomain.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.userextradomain.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.userextradomain.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.userextradomain.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.userextradomain.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.userextradomain.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.userextradomain.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.userextradomain.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.userextradomain.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('ID_USER', 'EXTRA_UDID', 'VALID_FROM', 'VALID_UNTIL');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['userextradomain.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['userextradomain.customview'])? $_SESSION['userextradomain.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('userextradomain', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('userextradomain', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('userextradomain');
        ...
        $_SESSION['userextradomain.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userextradomain'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('userextradomain', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('userextradomain', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userextradomain', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.userextradomain.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.userextradomain.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('userextradomain');

        $aclviewablecolumns = AclController::getAclEnabledColumns('userextradomain', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('userextradomain', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('userextradomain', 'new');
        }
        ...
        $ids = isset($_SESSION['userextradomain.list.ids'])? $_SESSION['userextradomain.list.ids'] : array();
        ...
        PluginManager::do_action('userextradomain_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userextradomain', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('userextradomain', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('userextradomain', 'view'));
        ...
        PluginManager::do_action('userextradomain_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.userextradomain.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('userextradomain');
        ...
        $roweditablecolumns = array('ID_USER', 'EXTRA_UDID', 'VALID_FROM', 'VALID_UNTIL');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('userextradomain', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('userextradomain', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('userextradomain_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('userextradomain', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('userextradomain'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('userextradomain'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('userextradomain', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.UDID = 0 OR ".TABLE_PREFIX."USER_EXTRA_DOMAIN.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."USER_EXTRA_DOMAIN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    UserExtraDomainController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['userextradomain.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_USER':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.ID_USER IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.ID_USER = '$value'");
                        }

                        break;

                    case 'EXTRA_UDID__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.EXTRA_UDID >= '".$this->field_sanitize('EXTRA_UDID', $value)."'");

                        break;

                    case 'EXTRA_UDID__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.EXTRA_UDID IS NULL OR ".TABLE_PREFIX."USER_EXTRA_DOMAIN.EXTRA_UDID <= '".$this->field_sanitize('EXTRA_UDID', $value)."')");

                        break;

                    case 'VALID_FROM__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.VALID_FROM >= '".$this->field_sanitize('VALID_FROM', $value)."'");

                        break;

                    case 'VALID_FROM__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.VALID_FROM IS NULL OR ".TABLE_PREFIX."USER_EXTRA_DOMAIN.VALID_FROM <= '".$this->field_sanitize('VALID_FROM', $value)."')");

                        break;

                    case 'VALID_UNTIL__FROM':
                        $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.VALID_UNTIL >= '".$this->field_sanitize('VALID_UNTIL', $value)."'");

                        break;

                    case 'VALID_UNTIL__TO':
                        $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.VALID_UNTIL IS NULL OR ".TABLE_PREFIX."USER_EXTRA_DOMAIN.VALID_UNTIL <= '".$this->field_sanitize('VALID_UNTIL', $value)."')");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.$key IS NULL OR ".TABLE_PREFIX.".USER_EXTRA_DOMAIN.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."USER_EXTRA_DOMAIN.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.ID_USER, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.EXTRA_UDID, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.VALID_FROM, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.VALID_UNTIL, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.ID, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.JSON, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.UUID, `'.TABLE_PREFIX.'USER_EXTRA_DOMAIN`.WFID');
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."USER_EXTRA_DOMAIN.ID_USER IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."USER_EXTRA_DOMAIN.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."USER_EXTRA_DOMAIN.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.userextradomain.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('ID_USER','EXTRA_UDID','VALID_FROM','VALID_UNTIL');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('userextradomain_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('userextradomain_imported', $model);
		}
