<div class="view-main">
    <{plugin key="userinvitationstatus_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{_t('User invitation status name')}></label>
            </td>
    ...
    <{plugin key="userinvitationstatus_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        userinvitationstatusview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        userinvitationstatusview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                userinvitationstatusview_apply_block_visibility(key, value);
            });
        }

        userinvitationstatusview_apply_block_visibility = function (key, value) {
        ...
        userinvitationstatusview_update_visibility_settings = function () {
            var settings = userinvitationstatusview_get_visibility_settings();

            userinvitationstatusview_apply_visibility_settings(settings);
        }

        $(function() {
            userinvitationstatusview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/userinvitationstatus/fieldSave",
//                        data: td.find(':input').serialize() + "&userinvitationstatus_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
