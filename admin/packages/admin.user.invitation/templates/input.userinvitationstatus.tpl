...
<body class="module module-<{$module}> additional-userinvitationstatus">
    ...
    <div class="edit-buttons edit-buttons-top hidden-print">
        <{foreach from=$formactions key=actiontitle item=actionurl}>
            <div class="button-general button-save btn btn-success">
                <a onclick="$('#userinvitationstatusform').attr('action', '<{$actionurl}>');$('#userinvitationstatusform').submit();return false;"><span class="button-face"><{$actiontitle}></span></a>
            </div>
        <{/foreach}>
        <a class="button-cancel scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/userinvitationstatus/cancel/?back=0"><span class="button-face"><{_t('Cancel')}></span></a>
    ...
    <div class="section" style="padding: 10px 0;">
        <div>
            <div class="edit-main edit_details">
                ...
                <form name="userinvitationstatusform" id="userinvitationstatusform" class="form-edit scope-main" action="" method="post" enctype="multipart/form-data">
                    <table class="table table-bordered table-custom-layout equal-split">
                        <tbody>
                            <{foreach from=$columns item=column }>
                                ...
                                <{elseif $column == 'NAME' }>
                                    <tr class="form-row form-row-name <{if in_array($column, $mandatories) }>form-row-mandatory<{/if}>">
                                		<td>
                                		    <div class="form-field form-field-label">
                                		        <label><{_t('User invitation status name')}><{if in_array('NAME', $mandatories) }><span class="mandatory">*</span><{/if}></label>
                                		    </div>
                                        </td>
                                        <td colspan="3">
                            ...
                            <input class="input-name" type="text" name="<{$prefix}>userinvitationstatus_formdata_NAME" value="<{$tmp_value|escape}>" <{if !$row_edit}>size="40"<{/if}> />
                                                                    </div>
                                        </td>
                                    </tr>
                            ...
                            <input class="input-code" type="text" name="<{$prefix}>userinvitationstatus_formdata_CODE" value="<{$tmp_value|escape}>" <{if !$row_edit}>size="40"<{/if}> />
                                                                    </div>
                                        </td>
                                    </tr>
...
<script type="text/javascript">
    $(function() {
    	bind_hotkey('#userinvitationstatusform', 'f2', '.button-save');
    	bind_hotkey('#userinvitationstatusform', 'esc', '.button-cancel');
    });
