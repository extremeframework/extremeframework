...
/**
 * Usage: <{module class="WidgetListUserExtraDomain" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.userextradomain.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserExtraDomain extends UserExtraDomainController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.userextradomain.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('userextradomain')) {
            $vars = array_keys(get_object_vars(new UserExtraDomainModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userextradomain', $preset, $presetvalue));
