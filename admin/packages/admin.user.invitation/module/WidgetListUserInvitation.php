...
/**
 * Usage: <{module class="WidgetListUserInvitation" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.userinvitation.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserInvitation extends UserInvitationController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.userinvitation.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('userinvitation')) {
            $vars = array_keys(get_object_vars(new UserInvitationModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userinvitation', $preset, $presetvalue));
