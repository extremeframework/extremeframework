...
/**
 * Usage: <{module class="WidgetListUserInvitationStatus" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.userinvitationstatus.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserInvitationStatus extends UserInvitationStatusController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.userinvitationstatus.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('userinvitationstatus')) {
            $vars = array_keys(get_object_vars(new UserInvitationStatusModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userinvitationstatus', $preset, $presetvalue));
