...
class UserPackageApi extends UserPackageController
{
    var $module = 'userpackage';
    var $type = 'api';

	function searchAction() {
	    AclApi::checkPermission('userpackage', 'list');
		...
		$model->selectAdd();
		$model->selectAdd('ID, INSTALLATION_DATE');

		if (!empty($keyword)) {
			$model->whereAdd("INSTALLATION_DATE LIKE '%$keyword%'");
		}
    ...
    function saveAction() {
        AclApi::checkPermission('userpackage', 'edit');
    ...
    function deleteAction() {
        AclApi::checkPermission('userpackage', 'delete');
    ...
    function viewAction() {
	    AclApi::checkPermission('userpackage', 'view');
        ...
        if ($model->fetch()) {
            $this->json_response(array('success' => true, 'data' => $this->model_sanitize($model), 'title' => $model->INSTALLATION_DATE));
        } else {
