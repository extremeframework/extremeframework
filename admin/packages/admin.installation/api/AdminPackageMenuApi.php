...
class AdminPackageMenuApi extends AdminPackageMenuController
{
    var $module = 'adminpackagemenu';
    var $type = 'api';

	function searchAction() {
	    AclApi::checkPermission('adminpackagemenu', 'list');
		...
		$model->selectAdd();
		$model->selectAdd('ID, JSON');

		if (!empty($keyword)) {
			$model->whereAdd("JSON LIKE '%$keyword%'");
		}
    ...
    function saveAction() {
        AclApi::checkPermission('adminpackagemenu', 'edit');
    ...
    function deleteAction() {
        AclApi::checkPermission('adminpackagemenu', 'delete');
    ...
    function viewAction() {
	    AclApi::checkPermission('adminpackagemenu', 'view');
        ...
        if ($model->fetch()) {
            $this->json_response(array('success' => true, 'data' => $this->model_sanitize($model), 'title' => $model->JSON));
        } else {
