...
/**
 * Usage: <{module class="WidgetListAdminPackageIndustry" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpackageindustry.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPackageIndustry extends AdminPackageIndustryController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpackageindustry.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpackageindustry')) {
            $vars = array_keys(get_object_vars(new AdminPackageIndustryModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackageindustry', $preset, $presetvalue));
