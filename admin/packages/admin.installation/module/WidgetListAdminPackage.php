...
/**
 * Usage: <{module class="WidgetListAdminPackage" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpackage.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPackage extends AdminPackageController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpackage.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpackage')) {
            $vars = array_keys(get_object_vars(new AdminPackageModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackage', $preset, $presetvalue));
