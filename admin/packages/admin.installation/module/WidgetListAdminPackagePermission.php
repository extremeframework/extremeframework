...
/**
 * Usage: <{module class="WidgetListAdminPackagePermission" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpackagepermission.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPackagePermission extends AdminPackagePermissionController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpackagepermission.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpackagepermission')) {
            $vars = array_keys(get_object_vars(new AdminPackagePermissionModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackagepermission', $preset, $presetvalue));
