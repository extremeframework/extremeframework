...
/**
 * Usage: <{module class="WidgetListAdminPackageShortcut" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpackageshortcut.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPackageShortcut extends AdminPackageShortcutController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpackageshortcut.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpackageshortcut')) {
            $vars = array_keys(get_object_vars(new AdminPackageShortcutModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackageshortcut', $preset, $presetvalue));
