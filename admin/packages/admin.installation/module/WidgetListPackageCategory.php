...
/**
 * Usage: <{module class="WidgetListPackageCategory" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.packagecategory.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPackageCategory extends PackageCategoryController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.packagecategory.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('packagecategory')) {
            $vars = array_keys(get_object_vars(new PackageCategoryModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('packagecategory', $preset, $presetvalue));
