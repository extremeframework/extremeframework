...
/**
 * Usage: <{module class="WidgetListAdminPackageMenu" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpackagemenu.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPackageMenu extends AdminPackageMenuController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpackagemenu.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpackagemenu')) {
            $vars = array_keys(get_object_vars(new AdminPackageMenuModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackagemenu', $preset, $presetvalue));
