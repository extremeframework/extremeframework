...
/**
 * Usage: <{module class="WidgetListUserPackage" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.userpackage.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserPackage extends UserPackageController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.userpackage.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('userpackage')) {
            $vars = array_keys(get_object_vars(new UserPackageModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userpackage', $preset, $presetvalue));
