...
/**
 * Usage: <{module class="WidgetListAdminPackageCategory" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpackagecategory.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPackageCategory extends AdminPackageCategoryController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpackagecategory.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpackagecategory')) {
            $vars = array_keys(get_object_vars(new AdminPackageCategoryModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackagecategory', $preset, $presetvalue));
