...
class _AdminPackageIndustryController extends __AppController
{
    var $module = 'adminpackageindustry';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('adminpackageindustry_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('CODE', $columns2check)) {
           $_model = new AdminPackageIndustryModel();
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('adminpackageindustry', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminpackageindustry_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackageindustry_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackageindustry_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('adminpackageindustry');

        ContextStack::check2redirect('adminpackageindustry');

        AclController::checkPermission('adminpackageindustry', 'list');

		ContextStack::register(APPLICATION_URL.'/adminpackageindustry/list/');
    ...
    public function updateOrderAction() {
        ...
        AclController::checkPermission('adminpackageindustry', 'edit');

	    $items = isset($_REQUEST['item'])? $_REQUEST['item'] : array();

        if (is_array($items) && !empty($items)) {
    		$model = new AdminPackageIndustryModel();
    ...
    public function customFilterAction() {
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            $afm->UUID = $id;
            $afm->MODULE = 'adminpackageindustry';
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminpackageindustry');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackageindustry_before_delete', $_model);
        }

		$model = new AdminPackageIndustryModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminpackageindustry', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('adminpackageindustry', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('adminpackageindustry_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminpackageindustry', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminpackageindustrylist_selection_selectall']) && $_REQUEST['adminpackageindustrylist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminpackageindustry.list.model']) && is_object($_SESSION['adminpackageindustry.list.model'])) {
					$model = clone $_SESSION['adminpackageindustry.list.model'];
				} else {
		        	$model = new AdminPackageIndustryModel();
				}
    			...
    			$selection = isset($_REQUEST['adminpackageindustrylist_selection'])? $_REQUEST['adminpackageindustrylist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'adminpackage': 
                            (new AdminPackageController())->delete('ID_ADMIN_PACKAGE_INDUSTRY', $_ids);
                            break;
        ...
        if ($module != 'adminpackageindustry' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('adminpackageindustry', 'new');

		ContextStack::register(APPLICATION_URL.'/adminpackageindustry/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminpackageindustry', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminPackageIndustryModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminpackageindustry', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminpackageindustry/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminpackageindustry/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackageindustry_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminpackageindustry_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminpackageindustry');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackageindustry_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'CODE', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminpackageindustry'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackageindustry_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminpackageindustry_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminpackageindustry_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('adminpackageindustry', $model);
            
            
            $this->onBeforeSave($model);
            PluginManager::do_action('adminpackageindustry_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminPackageIndustryModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminpackageindustry_before_update', $model, $old);

        		if (!AclController::hasPermission('adminpackageindustry', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminpackageindustry_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminpackageindustry_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminpackageindustry_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminpackageindustry');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminpackageindustry', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminpackageindustry.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminpackageindustry', 'import');

        if (isset($_FILES['adminpackageindustry_importfile'])) {
        	if (!$_FILES['adminpackageindustry_importfile']['error']) {
	            $filepath = $_FILES['adminpackageindustry_importfile']['tmp_name'];
        		$original = $_FILES['adminpackageindustry_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminpackageindustry/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('NAME','CODE','ORDERING');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminpackageindustry.list.model']) && is_object($_SESSION['adminpackageindustry.list.model'])) {
			$model = clone $_SESSION['adminpackageindustry.list.model'];
		} else {
        	$model = new AdminPackageIndustryModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminpackageindustry_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminpackageindustry_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminpackageindustry_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','ORDERING');
        $filename = 'adminpackageindustry_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminPackageIndustry');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.adminpackageindustry.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.adminpackageindustry.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.adminpackageindustry.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminpackageindustry.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminpackageindustry.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminpackageindustry.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminpackageindustry.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminpackageindustry.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminpackageindustry.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminpackageindustry.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminpackageindustry.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('NAME', 'CODE', 'ORDERING');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminpackageindustry.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminpackageindustry.customview'])? $_SESSION['adminpackageindustry.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminpackageindustry', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackageindustry', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminpackageindustry');
        ...
        $_SESSION['adminpackageindustry.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackageindustry'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminpackageindustry', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminpackageindustry', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackageindustry', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminpackageindustry.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.adminpackageindustry.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminpackageindustry');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackageindustry', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackageindustry', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackageindustry', 'new');
        }
        ...
        $ids = isset($_SESSION['adminpackageindustry.list.ids'])? $_SESSION['adminpackageindustry.list.ids'] : array();
        ...
        PluginManager::do_action('adminpackageindustry_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackageindustry', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminpackageindustry', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackageindustry', 'view'));
        ...
        PluginManager::do_action('adminpackageindustry_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminpackageindustry.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminpackageindustry');
        ...
        $roweditablecolumns = array('NAME', 'CODE', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminpackageindustry', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminpackageindustry_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminpackageindustry', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminpackageindustry_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackageindustry', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminpackageindustry'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminpackageindustry'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('adminpackageindustry', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.UDID = 0 OR ".TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminPackageIndustryController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminpackageindustry.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_PACKAGE_INDUSTRY.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.NAME, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.CODE, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.ID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.JSON, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.UUID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.WFID');
    }

    protected function initListModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.NAME, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.CODE, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.ORDERING, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.ID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.JSON, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.UUID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_INDUSTRY`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_INDUSTRY.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.adminpackageindustry.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('NAME','CODE','ORDERING');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminpackageindustry_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('adminpackageindustry_imported', $model);
		}
