...
class _AdminPackageMenuController extends __AppController
{
    var $module = 'adminpackagemenu';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('adminpackagemenu_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ID_ADMIN_PACKAGE', $columns2check) || in_array('ID_ADMIN_MENU', $columns2check)) {
           $_model = new AdminPackageMenuModel();
           $_model->ID_ADMIN_PACKAGE = $model->ID_ADMIN_PACKAGE;
           $_model->ID_ADMIN_MENU = $model->ID_ADMIN_MENU;
           ...
           $_model->find();
           if ($_model->N) {
               $errors['id-admin-package+id-admin-menu'] = sprintf(L_VALIDATION_ALREADY_EXISTS, '{'.L_ADMIN_PACKAGE.', '.L_ADMIN_MENU.'}');
               return false;
           }
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('adminpackagemenu', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminpackagemenu_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackagemenu_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackagemenu_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminpackagemenu');

        AclController::checkPermission('adminpackagemenu', 'list');

		ContextStack::register(APPLICATION_URL.'/adminpackagemenu/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_PACKAGE_MENU.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminpackagemenu');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackagemenu_before_delete', $_model);
        }

		$model = new AdminPackageMenuModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminpackagemenu', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackagemenu_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminpackagemenu', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminpackagemenulist_selection_selectall']) && $_REQUEST['adminpackagemenulist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminpackagemenu.list.model']) && is_object($_SESSION['adminpackagemenu.list.model'])) {
					$model = clone $_SESSION['adminpackagemenu.list.model'];
				} else {
		        	$model = new AdminPackageMenuModel();
				}
    			...
    			$selection = isset($_REQUEST['adminpackagemenulist_selection'])? $_REQUEST['adminpackagemenulist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminpackagemenu' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminpackagemenu', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminPackageMenuModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminpackagemenu', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminpackagemenu/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminpackagemenu/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackagemenu_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminpackagemenu_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminpackagemenu');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackagemenu_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminpackagemenu'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackagemenu_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminpackagemenu_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminpackagemenu_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminpackagemenu_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminPackageMenuModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminpackagemenu_before_update', $model, $old);

        		if (!AclController::hasPermission('adminpackagemenu', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_PACKAGE_MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminpackagemenu_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminpackagemenu_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminpackagemenu_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminpackagemenu');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminpackagemenu', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminpackagemenu.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminpackagemenu', 'import');

        if (isset($_FILES['adminpackagemenu_importfile'])) {
        	if (!$_FILES['adminpackagemenu_importfile']['error']) {
	            $filepath = $_FILES['adminpackagemenu_importfile']['tmp_name'];
        		$original = $_FILES['adminpackagemenu_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminpackagemenu/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('ID_ADMIN_PACKAGE','ID_ADMIN_MENU');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminpackagemenu.list.model']) && is_object($_SESSION['adminpackagemenu.list.model'])) {
			$model = clone $_SESSION['adminpackagemenu.list.model'];
		} else {
        	$model = new AdminPackageMenuModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminpackagemenu_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminpackagemenu_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminpackagemenu_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('ID_ADMIN_PACKAGE','ID_ADMIN_MENU');
        $filename = 'adminpackagemenu_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminPackageMenu');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminpackagemenu.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminpackagemenu.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminpackagemenu.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminpackagemenu.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminpackagemenu.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminpackagemenu.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminpackagemenu.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminpackagemenu.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminpackagemenu.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminpackagemenu.customview'])? $_SESSION['adminpackagemenu.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminpackagemenu', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackagemenu', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminpackagemenu');
        ...
        $_SESSION['adminpackagemenu.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackagemenu'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminpackagemenu', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminpackagemenu', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackagemenu', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminpackagemenu.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.adminpackagemenu.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminpackagemenu');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackagemenu', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackagemenu', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackagemenu', 'new');
        }
        ...
        $ids = isset($_SESSION['adminpackagemenu.list.ids'])? $_SESSION['adminpackagemenu.list.ids'] : array();
        ...
        PluginManager::do_action('adminpackagemenu_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackagemenu', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminpackagemenu', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackagemenu', 'view'));
        ...
        PluginManager::do_action('adminpackagemenu_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminpackagemenu.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminpackagemenu');
        ...
        $roweditablecolumns = array('ID_ADMIN_PACKAGE', 'ID_ADMIN_MENU');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminpackagemenu', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminpackagemenu', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminpackagemenu_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackagemenu', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminpackagemenu'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminpackagemenu'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminPackageMenuController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminpackagemenu.list.model'] = clone $model;
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_ADMIN_PACKAGE':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.ID_ADMIN_PACKAGE IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.ID_ADMIN_PACKAGE = '$value'");
                        }
                    ...
                    case 'ID_ADMIN_MENU':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.ID_ADMIN_MENU IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.ID_ADMIN_MENU = '$value'");
                        }
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_PACKAGE_MENU.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_MENU.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PACKAGE_MENU`.ID_ADMIN_PACKAGE, `'.TABLE_PREFIX.'ADMIN_PACKAGE_MENU`.ID_ADMIN_MENU, `'.TABLE_PREFIX.'ADMIN_PACKAGE_MENU`.ID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_MENU`.JSON, `'.TABLE_PREFIX.'ADMIN_PACKAGE_MENU`.UUID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_MENU`.WFID');
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."ADMIN_PACKAGE_MENU.ID_ADMIN_PACKAGE IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_MENU.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_MENU.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.adminpackagemenu.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('ID_ADMIN_PACKAGE','ID_ADMIN_MENU');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminpackagemenu_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('adminpackagemenu_imported', $model);
		}
