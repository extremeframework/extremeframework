...
class _AdminPackageCategoryController extends __AppController
{
    var $module = 'adminpackagecategory';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('adminpackagecategory_init');
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminpackagecategory_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackagecategory_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackagecategory_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminpackagecategory');

        AclController::checkPermission('adminpackagecategory', 'list');

		ContextStack::register(APPLICATION_URL.'/adminpackagecategory/list/');
    ...
    public function customFilterAction() {
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            $afm->UUID = $id;
            $afm->MODULE = 'adminpackagecategory';
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminpackagecategory');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new AdminPackageCategoryModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackagecategory_before_delete', $_model);
        }

		$model = new AdminPackageCategoryModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminpackagecategory', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackagecategory_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminpackagecategory', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminpackagecategorylist_selection_selectall']) && $_REQUEST['adminpackagecategorylist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminpackagecategory.list.model']) && is_object($_SESSION['adminpackagecategory.list.model'])) {
					$model = clone $_SESSION['adminpackagecategory.list.model'];
				} else {
		        	$model = new AdminPackageCategoryModel();
				}
    			...
    			$selection = isset($_REQUEST['adminpackagecategorylist_selection'])? $_REQUEST['adminpackagecategorylist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminpackagecategory' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminpackagecategory', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminPackageCategoryModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminpackagecategory', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminpackagecategory/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminpackagecategory/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackagecategory_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminpackagecategory_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackagecategory_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackagecategory_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminpackagecategory_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminpackagecategory_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminpackagecategory_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminPackageCategoryModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminpackagecategory_before_update', $model, $old);

        		if (!AclController::hasPermission('adminpackagecategory', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminpackagecategory_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminpackagecategory_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminpackagecategory_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminpackagecategory');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminpackagecategory', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminpackagecategory.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminpackagecategory', 'import');

        if (isset($_FILES['adminpackagecategory_importfile'])) {
        	if (!$_FILES['adminpackagecategory_importfile']['error']) {
	            $filepath = $_FILES['adminpackagecategory_importfile']['tmp_name'];
        		$original = $_FILES['adminpackagecategory_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminpackagecategory/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminpackagecategory.list.model']) && is_object($_SESSION['adminpackagecategory.list.model'])) {
			$model = clone $_SESSION['adminpackagecategory.list.model'];
		} else {
        	$model = new AdminPackageCategoryModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminpackagecategory_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminpackagecategory_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminpackagecategory_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
		...
		$worksheet =& $workbook->addWorksheet('AdminPackageCategory');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminpackagecategory.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminpackagecategory.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminpackagecategory.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminpackagecategory.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminpackagecategory.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminpackagecategory.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminpackagecategory.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminpackagecategory.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminpackagecategory.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminpackagecategory.customview'])? $_SESSION['adminpackagecategory.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminpackagecategory', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackagecategory', 'view');
        ...
        $_SESSION['adminpackagecategory.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackagecategory'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminpackagecategory', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminpackagecategory', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackagecategory', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminpackagecategory.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.adminpackagecategory.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminpackagecategory');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackagecategory', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackagecategory', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackagecategory', 'new');
        }
        ...
        $ids = isset($_SESSION['adminpackagecategory.list.ids'])? $_SESSION['adminpackagecategory.list.ids'] : array();
        ...
        PluginManager::do_action('adminpackagecategory_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackagecategory', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminpackagecategory', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackagecategory', 'view'));
        ...
        PluginManager::do_action('adminpackagecategory_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminpackagecategory.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminpackagecategory');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminpackagecategory', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminpackagecategory_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminpackagecategory', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminpackagecategory_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackagecategory', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminpackagecategory'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminpackagecategory'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminPackageCategoryController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminpackagecategory.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_PACKAGE_CATEGORY.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_CATEGORY.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function _import($templatecode = 'import.adminpackagecategory.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminpackagecategory_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('adminpackagecategory_imported', $model);
		}
