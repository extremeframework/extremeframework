...
class _PackageCategoryController extends __AppController
{
    var $module = 'packagecategory';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('packagecategory_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('packagecategory', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^packagecategory_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^packagecategory_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^packagecategory_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('packagecategory');

        ContextStack::check2redirect('packagecategory');

        AclController::checkPermission('packagecategory', 'list');

		ContextStack::register(APPLICATION_URL.'/packagecategory/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."PACKAGE_CATEGORY.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
		...
		$model = new PackageCategoryModel();
		if (!empty($term)) {
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'packagecategory');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new PackageCategoryModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('packagecategory_before_delete', $_model);
        }

		$model = new PackageCategoryModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('packagecategory', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}

        $this->enforceObjectAclCheck('packagecategory', $model);
		
		$model->delete();

        foreach ($_models as $_model) {
            $this->onDeleteSuccess($_model);
            PluginManager::do_action('packagecategory_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('packagecategory', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['packagecategorylist_selection_selectall']) && $_REQUEST['packagecategorylist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['packagecategory.list.model']) && is_object($_SESSION['packagecategory.list.model'])) {
					$model = clone $_SESSION['packagecategory.list.model'];
				} else {
		        	$model = new PackageCategoryModel();
				}
    			...
    			$selection = isset($_REQUEST['packagecategorylist_selection'])? $_REQUEST['packagecategorylist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'adminpackage': 
                            (new AdminPackageController())->delete('ID_PACKAGE_CATEGORY', $_ids);
                            break;
        ...
        if ($module != 'packagecategory' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('packagecategory', 'edit');

		ContextStack::register(APPLICATION_URL.'/packagecategory/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('packagecategory', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PackageCategoryModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('packagecategory', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/packagecategory/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/packagecategory/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'packagecategory_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'packagecategory_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('packagecategory');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'packagecategory_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('packagecategory'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'packagecategory_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'packagecategory_multiformdata_deleteditems'])? $_REQUEST[$prefix.'packagecategory_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            CustomFieldHelper::updateCustomFieldValues('packagecategory', $model);
            
            
            $this->onBeforeSave($model);
            PluginManager::do_action('packagecategory_before_save', $model);

    		if ($model->UUID) {
				
				$old = new PackageCategoryModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('packagecategory_before_update', $model, $old);

        		if (!AclController::hasPermission('packagecategory', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."PACKAGE_CATEGORY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('packagecategory_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('packagecategory_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('packagecategory_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('packagecategory');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('packagecategory', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.packagecategory.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('packagecategory', 'import');

        if (isset($_FILES['packagecategory_importfile'])) {
        	if (!$_FILES['packagecategory_importfile']['error']) {
	            $filepath = $_FILES['packagecategory_importfile']['tmp_name'];
        		$original = $_FILES['packagecategory_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/packagecategory/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['packagecategory.list.model']) && is_object($_SESSION['packagecategory.list.model'])) {
			$model = clone $_SESSION['packagecategory.list.model'];
		} else {
        	$model = new PackageCategoryModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'packagecategory_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/packagecategory_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/packagecategory_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME');
        $filename = 'packagecategory_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PackageCategory');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.packagecategory.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.packagecategory.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.packagecategory.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.packagecategory.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.packagecategory.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.packagecategory.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.packagecategory.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.packagecategory.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.packagecategory.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.packagecategory.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.packagecategory.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['packagecategory.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['packagecategory.customview'])? $_SESSION['packagecategory.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('packagecategory', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('packagecategory', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('packagecategory');
        ...
        $_SESSION['packagecategory.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('packagecategory'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('packagecategory', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('packagecategory', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('packagecategory', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.packagecategory.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.packagecategory.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('packagecategory');

        $aclviewablecolumns = AclController::getAclEnabledColumns('packagecategory', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('packagecategory', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('packagecategory', 'new');
        }
        ...
        $ids = isset($_SESSION['packagecategory.list.ids'])? $_SESSION['packagecategory.list.ids'] : array();
        ...
        PluginManager::do_action('packagecategory_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('packagecategory', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('packagecategory', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('packagecategory', 'view'));
        ...
        PluginManager::do_action('packagecategory_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.packagecategory.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('packagecategory');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('packagecategory', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('packagecategory_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('packagecategory', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('packagecategory_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('packagecategory', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('packagecategory'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('packagecategory'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('packagecategory', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.UDID = 0 OR ".TABLE_PREFIX."PACKAGE_CATEGORY.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    PackageCategoryController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['packagecategory.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.$key IS NULL OR ".TABLE_PREFIX.".PACKAGE_CATEGORY.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."PACKAGE_CATEGORY.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'PACKAGE_CATEGORY`.NAME, `'.TABLE_PREFIX.'PACKAGE_CATEGORY`.ID, `'.TABLE_PREFIX.'PACKAGE_CATEGORY`.JSON, `'.TABLE_PREFIX.'PACKAGE_CATEGORY`.UUID, `'.TABLE_PREFIX.'PACKAGE_CATEGORY`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."PACKAGE_CATEGORY.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PACKAGE_CATEGORY.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PACKAGE_CATEGORY.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.packagecategory.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('packagecategory_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('packagecategory_imported', $model);
		}
