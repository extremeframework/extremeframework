...
class _AdminPackageShortcutController extends __AppController
{
    var $module = 'adminpackageshortcut';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('adminpackageshortcut_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('PATH', $columns2check) && trim($model->PATH) == '') {
    ...
    protected function handleFileUploads(&$model) {
	    if (isset($_FILES) && !empty($_FILES)) {
            foreach ($_FILES as $column => $file) {
                if (preg_match('/^adminpackageshortcut_formdata_(.*)/', $column, $matches)) {
                    $column = $matches[1];
                }
                ...
                if (!empty($file['name'])) {
                    ...
                    $rel = 'adminpackageshortcut/'.strtolower(str_replace('_', '-', $column));
                    ...
                    if (in_array(strtolower($extension), array('.gif', '.jpg', '.jpeg', '.png'))) {
                        ...
                        if (!($width > 0 || $height > 0) && AdminImageConfigHelper::hasImageConfig('adminpackageshortcut', $column)) {
                            $config = AdminImageConfigHelper::getImageConfig('adminpackageshortcut', $column);
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminpackageshortcut_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackageshortcut_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackageshortcut_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminpackageshortcut');

        AclController::checkPermission('adminpackageshortcut', 'list');

		ContextStack::register(APPLICATION_URL.'/adminpackageshortcut/list/');
    ...
    public function customFilterAction() {
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            $afm->UUID = $id;
            $afm->MODULE = 'adminpackageshortcut';
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminpackageshortcut');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackageshortcut_before_delete', $_model);
        }

		$model = new AdminPackageShortcutModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminpackageshortcut', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackageshortcut_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminpackageshortcut', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminpackageshortcutlist_selection_selectall']) && $_REQUEST['adminpackageshortcutlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminpackageshortcut.list.model']) && is_object($_SESSION['adminpackageshortcut.list.model'])) {
					$model = clone $_SESSION['adminpackageshortcut.list.model'];
				} else {
		        	$model = new AdminPackageShortcutModel();
				}
    			...
    			$selection = isset($_REQUEST['adminpackageshortcutlist_selection'])? $_REQUEST['adminpackageshortcutlist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminpackageshortcut' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminpackageshortcut', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminPackageShortcutModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminpackageshortcut', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminpackageshortcut/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminpackageshortcut/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackageshortcut_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminpackageshortcut_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminpackageshortcut');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'NAME', 'ID_ADMIN_PACKAGE', 'IMAGE', 'PATH');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackageshortcut_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'ID_ADMIN_PACKAGE', 'IMAGE', 'PATH');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminpackageshortcut'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackageshortcut_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminpackageshortcut_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminpackageshortcut_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminpackageshortcut_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminPackageShortcutModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminpackageshortcut_before_update', $model, $old);

        		if (!AclController::hasPermission('adminpackageshortcut', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminpackageshortcut_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminpackageshortcut_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminpackageshortcut_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminpackageshortcut');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminpackageshortcut', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminpackageshortcut.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminpackageshortcut', 'import');

        if (isset($_FILES['adminpackageshortcut_importfile'])) {
        	if (!$_FILES['adminpackageshortcut_importfile']['error']) {
	            $filepath = $_FILES['adminpackageshortcut_importfile']['tmp_name'];
        		$original = $_FILES['adminpackageshortcut_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminpackageshortcut/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('NAME','ID_ADMIN_PACKAGE','IMAGE','PATH');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminpackageshortcut.list.model']) && is_object($_SESSION['adminpackageshortcut.list.model'])) {
			$model = clone $_SESSION['adminpackageshortcut.list.model'];
		} else {
        	$model = new AdminPackageShortcutModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminpackageshortcut_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminpackageshortcut_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminpackageshortcut_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','ID_ADMIN_PACKAGE','IMAGE','PATH');
        $filename = 'adminpackageshortcut_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminPackageShortcut');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminpackageshortcut.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminpackageshortcut.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminpackageshortcut.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminpackageshortcut.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminpackageshortcut.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminpackageshortcut.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminpackageshortcut.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminpackageshortcut.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('NAME', 'ID_ADMIN_PACKAGE', 'IMAGE', 'PATH');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminpackageshortcut.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminpackageshortcut.customview'])? $_SESSION['adminpackageshortcut.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminpackageshortcut', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackageshortcut', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('adminpackageshortcut');
        ...
        $_SESSION['adminpackageshortcut.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackageshortcut'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminpackageshortcut', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminpackageshortcut', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackageshortcut', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminpackageshortcut.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.adminpackageshortcut.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminpackageshortcut');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackageshortcut', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackageshortcut', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackageshortcut', 'new');
        }
        ...
        $ids = isset($_SESSION['adminpackageshortcut.list.ids'])? $_SESSION['adminpackageshortcut.list.ids'] : array();
        ...
        PluginManager::do_action('adminpackageshortcut_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackageshortcut', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminpackageshortcut', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackageshortcut', 'view'));
        ...
        PluginManager::do_action('adminpackageshortcut_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminpackageshortcut.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminpackageshortcut');
        ...
        $roweditablecolumns = array('NAME', 'ID_ADMIN_PACKAGE', 'PATH');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminpackageshortcut', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminpackageshortcut', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminpackageshortcut_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackageshortcut', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminpackageshortcut'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminpackageshortcut'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminPackageShortcutController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminpackageshortcut.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'ID_ADMIN_PACKAGE':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.ID_ADMIN_PACKAGE IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.ID_ADMIN_PACKAGE = '$value'");
                        }
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_PACKAGE_SHORTCUT.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.NAME, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.ID_ADMIN_PACKAGE, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.IMAGE, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.PATH, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.ID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.JSON, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.UUID, `'.TABLE_PREFIX.'ADMIN_PACKAGE_SHORTCUT`.WFID');
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.ID_ADMIN_PACKAGE IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.PATH LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE_SHORTCUT.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.adminpackageshortcut.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('NAME','ID_ADMIN_PACKAGE','IMAGE','PATH');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminpackageshortcut_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('adminpackageshortcut_imported', $model);
		}
