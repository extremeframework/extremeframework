...
class _AdminPackageController extends __AppController
{
    var $module = 'adminpackage';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('adminpackage_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('AUTHOR_URL', $columns2check) && !empty($model->AUTHOR_URL) && !preg_match('/^(http|https|ftp):\/\/(([A-Z0-9][A-Z0-9_-]*)(\.?[A-Z0-9][A-Z0-9_-]*)+)(:(\d+))?/i', $model->AUTHOR_URL)) {
           $errors['author-url'] = sprintf(_t('L_VALIDATION_URL'), _t('Author url'));
           return false;
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('adminpackage', $model, $errors)) {
            return false;
        }
    ...
    protected function handleFileUploads(&$model) {
	    if (isset($_FILES) && !empty($_FILES)) {
            foreach ($_FILES as $column => $file) {
                if (preg_match('/^adminpackage_formdata_(.*)/', $column, $matches)) {
                    $column = $matches[1];
                }
                ...
                if (!empty($file['name'])) {
                    ...
                    $rel = 'adminpackage/'.strtolower(str_replace('_', '-', $column));
                    ...
                    if (in_array(strtolower($extension), array('.gif', '.jpg', '.jpeg', '.png'))) {
                        ...
                        if (!($width > 0 || $height > 0) && AdminImageConfigHelper::hasImageConfig('adminpackage', $column)) {
                            $config = AdminImageConfigHelper::getImageConfig('adminpackage', $column);
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminpackage_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackage_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpackage_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminpackage');

        AclController::checkPermission('adminpackage', 'list');

		ContextStack::register(APPLICATION_URL.'/adminpackage/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_PACKAGE.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminpackage');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackage_before_delete', $_model);
        }

		$model = new AdminPackageModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminpackage', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpackage_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminpackage', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminpackagelist_selection_selectall']) && $_REQUEST['adminpackagelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminpackage.list.model']) && is_object($_SESSION['adminpackage.list.model'])) {
					$model = clone $_SESSION['adminpackage.list.model'];
				} else {
		        	$model = new AdminPackageModel();
				}
    			...
    			$selection = isset($_REQUEST['adminpackagelist_selection'])? $_REQUEST['adminpackagelist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminpackage' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminpackage', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminPackageModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminpackage', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminpackage/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminpackage/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackage_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminpackage_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function field_sanitize($column, $value) {
		if ($column == 'INSTALLATION_DATE' && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $value)) {
            $format = DATE_FORMAT;
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        ...
        $columns2edit = array('UUID', 'NAME', 'CODE', 'ID_ADMIN_PACKAGE_TYPE', 'ID_ADMIN_PACKAGE_CATEGORY', 'ID_ADMIN_PACKAGE_INDUSTRY', 'IMAGE', 'DESCRIPTION', 'ENTRY_PATH', 'AUTHOR', 'VERSION', 'PACKAGE_URL', 'AUTHOR_URL', 'PACKAGE_PATH', 'INSTALLATION_DATE', 'LATEST_UPDATE');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackage_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpackage_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminpackage_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminpackage_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminpackage_before_save', $model);

    		if ($model->UUID) {
				$model->LATEST_UPDATE = date('Y-m-d H:i:s');
				$old = new AdminPackageModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminpackage_before_update', $model, $old);

        		if (!AclController::hasPermission('adminpackage', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_PACKAGE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminpackage_updated', $model, $old);
            } else {
                $model->ID = null;
                $model->LATEST_UPDATE = date('Y-m-d H:i:s');
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminpackage_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminpackage_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminpackage');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminpackage', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminpackage.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminpackage.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminpackage.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminpackage.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminpackage.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminpackage.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminpackage.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminpackage.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminpackage.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminpackage.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminpackage.customview'])? $_SESSION['adminpackage.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminpackage', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackage', 'view');
        ...
        $_SESSION['adminpackage.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpackage'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminpackage', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminpackage', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackage', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminpackage.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.adminpackage.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminpackage');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpackage', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackage', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpackage', 'new');
        }
        ...
        PluginManager::do_action('adminpackage_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackage', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminpackage', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpackage', 'view'));
        ...
        PluginManager::do_action('adminpackage_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminpackage.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminpackage');
        ...
        $roweditablecolumns = array('NAME', 'CODE', 'ID_ADMIN_PACKAGE_TYPE', 'ID_ADMIN_PACKAGE_CATEGORY', 'ID_ADMIN_PACKAGE_INDUSTRY', 'ENTRY_PATH', 'AUTHOR', 'VERSION', 'PACKAGE_PATH', 'INSTALLATION_DATE', 'LATEST_UPDATE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminpackage', 'editpeer');
                }
            } else {
                // Set default values here
                if ($recent = $this->getRecentModel()) {
                    $model->ID_ADMIN_PACKAGE_TYPE = $recent->ID_ADMIN_PACKAGE_TYPE;
                    $model->ID_ADMIN_PACKAGE_CATEGORY = $recent->ID_ADMIN_PACKAGE_CATEGORY;
                    $model->ID_ADMIN_PACKAGE_INDUSTRY = $recent->ID_ADMIN_PACKAGE_INDUSTRY;
                }

                $this->onInitialization($model);
                PluginManager::do_action('adminpackage_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminpackage', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminpackage_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpackage', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminpackage'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminpackage'));

	    $this->display($smarty, $templatecode);
	}

    protected function getLayoutColumns() {
        return array('NAME', 'CODE', 'ID_ADMIN_PACKAGE_TYPE', 'ID_ADMIN_PACKAGE_CATEGORY', 'ID_ADMIN_PACKAGE_INDUSTRY', 'IMAGE', 'DESCRIPTION', 'ENTRY_PATH', 'AUTHOR', 'VERSION', 'PACKAGE_URL', 'AUTHOR_URL', 'PACKAGE_PATH', 'INSTALLATION_DATE', 'LATEST_UPDATE');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminPackageController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminpackage.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    case 'ID_ADMIN_PACKAGE_CATEGORY':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.ID_ADMIN_PACKAGE_CATEGORY IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.ID_ADMIN_PACKAGE_CATEGORY = '$value'");
                        }
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_PACKAGE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PACKAGE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PACKAGE`.NAME, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.CODE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID_ADMIN_PACKAGE_TYPE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID_ADMIN_PACKAGE_CATEGORY, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID_ADMIN_PACKAGE_INDUSTRY, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.IMAGE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.DESCRIPTION, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ENTRY_PATH, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.AUTHOR, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.VERSION, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.PACKAGE_URL, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.AUTHOR_URL, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.PACKAGE_PATH, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.INSTALLATION_DATE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.LATEST_UPDATE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.JSON, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.UUID, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.WFID');
    ...
    protected function initListModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PACKAGE`.NAME, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.CODE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID_ADMIN_PACKAGE_TYPE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID_ADMIN_PACKAGE_CATEGORY, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID_ADMIN_PACKAGE_INDUSTRY, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.IMAGE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ENTRY_PATH, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.AUTHOR, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.VERSION, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.PACKAGE_PATH, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.INSTALLATION_DATE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.LATEST_UPDATE, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.ID, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.JSON, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.UUID, `'.TABLE_PREFIX.'ADMIN_PACKAGE`.WFID');
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.DESCRIPTION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.ENTRY_PATH LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.AUTHOR LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.VERSION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.PACKAGE_PATH LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PACKAGE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
