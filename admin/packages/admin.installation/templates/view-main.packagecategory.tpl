<div class="view-main">
    <{plugin key="packagecategory_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{_t('Package category name')}></label>
            </td>
    ...
    <input class="input-name input-type-text" type="text" name="<{$prefix}>packagecategory_formdata_NAME" value="<{$details->NAME|escape}>"  />
                                </span>
                                -->
                            <{else}>
    ...
    <{plugin key="packagecategory_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        packagecategoryview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        packagecategoryview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                packagecategoryview_apply_block_visibility(key, value);
            });
        }

        packagecategoryview_apply_block_visibility = function (key, value) {
        ...
        packagecategoryview_update_visibility_settings = function () {
            var settings = packagecategoryview_get_visibility_settings();

            packagecategoryview_apply_visibility_settings(settings);
        }

        $(function() {
            packagecategoryview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/packagecategory/fieldSave",
//                        data: td.find(':input').serialize() + "&packagecategory_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
