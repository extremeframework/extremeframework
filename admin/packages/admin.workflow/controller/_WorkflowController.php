...
class _WorkflowController extends __AppController
{
    var $module = 'workflow';
    var $type = 'controller';
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('CODE', $columns2check)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('workflow', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^workflow_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflow_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflow_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('workflow');

        AclController::checkPermission('workflow', 'list');

		ContextStack::register(APPLICATION_URL.'/workflow/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."WORKFLOW.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('workflow_before_delete', $_model);
        }

		$model = new WorkflowModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('workflow', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."WORKFLOW.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('workflow', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['workflowlist_selection_selectall']) && $_REQUEST['workflowlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['workflow.list.model']) && is_object($_SESSION['workflow.list.model'])) {
					$model = clone $_SESSION['workflow.list.model'];
				} else {
		        	$model = new WorkflowModel();
				}
    			...
    			$selection = isset($_REQUEST['workflowlist_selection'])? $_REQUEST['workflowlist_selection'] : '';
			}
        }
        ...
        if ($module != 'workflow' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('workflow', 'new');

		ContextStack::register(APPLICATION_URL.'/workflow/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('workflow', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new WorkflowModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('workflow', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/workflow/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
        ...
        AclController::checkPermission('workflow', 'edit');
		...
		ContextStack::register(APPLICATION_URL.'/workflow/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflow_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'workflow_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('workflow');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'workflow_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflow_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
				...
				$old = new WorkflowModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('workflow_before_update', $model, $old);

        		if (!AclController::hasPermission('workflow', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."WORKFLOW.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('workflow_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('workflow', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.workflow.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.workflow.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.workflow.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.workflow.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.workflow.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.workflow.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.workflow.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.workflow.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.workflow.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.workflow.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.workflow.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.workflow.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['workflow.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['workflow.customview'])? $_SESSION['workflow.customview'] : '';
    ...
    protected function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('workflow');
        ...
        $_SESSION['workflow.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('workflow'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('workflow', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('workflow', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflow', 'view'));
    ...
    protected function _view($id, $details = null, $templatecode = 'view.workflow.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflow', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflow', 'new');
        }
        ...
        $ids = isset($_SESSION['workflow.list.ids'])? $_SESSION['workflow.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflow', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('workflow', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflow', 'view'));
    ...
    protected function _edit($id, $details = null, $templatecode = 'edit.workflow.tpl', $restoredraft = true) {
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('workflow', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('workflow_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('workflow', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflow', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('workflow'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('workflow'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('workflow', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."WORKFLOW.UDID = 0 OR ".TABLE_PREFIX."WORKFLOW.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."WORKFLOW.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    WorkflowController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['workflow.list.model'] = clone $model;
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW.$key IS NULL OR ".TABLE_PREFIX.".WORKFLOW.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'WORKFLOW`.NAME, `'.TABLE_PREFIX.'WORKFLOW`.CODE, `'.TABLE_PREFIX.'WORKFLOW`.DESCRIPTION, `'.TABLE_PREFIX.'WORKFLOW`.ID, `'.TABLE_PREFIX.'WORKFLOW`.JSON, `'.TABLE_PREFIX.'WORKFLOW`.UUID, `'.TABLE_PREFIX.'WORKFLOW`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."WORKFLOW.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW.DESCRIPTION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
