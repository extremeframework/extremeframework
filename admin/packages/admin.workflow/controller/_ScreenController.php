...
class _ScreenController extends __AppController
{
    var $module = 'screen';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('screen_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('CODE', $columns2check)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('screen', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^screen_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^screen_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^screen_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('screen');

        AclController::checkPermission('screen', 'list');

		ContextStack::register(APPLICATION_URL.'/screen/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."SCREEN.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('screen_before_delete', $_model);
        }

		$model = new ScreenModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('screen', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."SCREEN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('screen', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['screenlist_selection_selectall']) && $_REQUEST['screenlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['screen.list.model']) && is_object($_SESSION['screen.list.model'])) {
					$model = clone $_SESSION['screen.list.model'];
				} else {
		        	$model = new ScreenModel();
				}
    			...
    			$selection = isset($_REQUEST['screenlist_selection'])? $_REQUEST['screenlist_selection'] : '';
			}
        }
        ...
        if ($module != 'screen' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('screen', 'new');

		ContextStack::register(APPLICATION_URL.'/screen/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('screen', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new ScreenModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('screen', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/screen/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
        ...
        AclController::checkPermission('screen', 'edit');
		...
		ContextStack::register(APPLICATION_URL.'/screen/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'screen_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'screen_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('screen');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'TITLE', 'ID_WORKFLOW', 'CODE');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'screen_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'TITLE', 'ID_WORKFLOW', 'CODE');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('screen'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'screen_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'screen_multiformdata_deleteditems'])? $_REQUEST[$prefix.'screen_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            CustomFieldHelper::updateCustomFieldValues('screen', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('screen_before_save', $model);
    		...
    		if ($model->UUID) {
                ...
                PluginManager::do_action('screen_before_update', $model, $old);

        		if (!AclController::hasPermission('screen', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."SCREEN.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."SCREEN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."SCREEN.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('screen_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('screen', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.screen.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.screen.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.screen.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.screen.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.screen.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.screen.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.screen.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.screen.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.screen.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.screen.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.screen.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.screen.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('TITLE', 'ID_WORKFLOW', 'CODE');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['screen.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['screen.customview'])? $_SESSION['screen.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('screen', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('screen', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('screen');
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('screen'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('screen', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('screen', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('screen', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.screen.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.screen.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('screen');

        $aclviewablecolumns = AclController::getAclEnabledColumns('screen', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('screen', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('screen', 'new');
        }
        ...
        $ids = isset($_SESSION['screen.list.ids'])? $_SESSION['screen.list.ids'] : array();
        ...
        PluginManager::do_action('screen_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('screen', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('screen', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('screen', 'view'));
        ...
        PluginManager::do_action('screen_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.screen.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('screen');
        ...
        $roweditablecolumns = array('TITLE', 'ID_WORKFLOW', 'CODE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('screen', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('screen', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('screen_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('screen', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('screen'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('screen'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('screen', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."SCREEN.UDID = 0 OR ".TABLE_PREFIX."SCREEN.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."SCREEN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    ScreenController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['screen.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'TITLE':
                        $model->whereAdd(TABLE_PREFIX."SCREEN.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'ID_WORKFLOW':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."SCREEN.ID_WORKFLOW IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."SCREEN.ID_WORKFLOW = '$value'");
                        }
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."SCREEN.$key IS NULL OR ".TABLE_PREFIX.".SCREEN.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."SCREEN.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."SCREEN.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
