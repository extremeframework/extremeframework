    ...
    <body class="module module-<{$module}>" id="<{$module}>">
            ...
            <{$tmp1 = _t('View')}>
        <{$tmp2 = strtolower(_t('Workflow Transition'))}>
        <{$title = "`$tmp1` `$tmp2`"}>
    ...
    <h1 class="heading">
        ...
        <span style="margin-left:10px; font-size:12px; font-weight: normal" class="hidden-print">
            <{if $previd}>
                <a style="text-decoration: none" class="button-view-prev scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/workflowtransition/view/<{$previd}>"><{_t('Prev')}></a>&nbsp;
            <{/if}>
            <{if $nextid}>
                <a style="text-decoration: none" class="button-view-next scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/workflowtransition/view/<{$nextid}>"><{_t('Next')}></a>
            <{/if}>
    ...
    <div class="view-buttons">
    	<{if $details->UUID}>
        		    ...
        		    <div class="button-general">
        		        <a class="button-edit scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/workflowtransition/edit/<{$details->UUID}><{if isset($preset)}>/preset/<{$preset}><{/if}><{if isset($presetvalue)}>/presetvalue/<{$presetvalue}><{/if}>" title="<{_t('Edit', true)}>"><i class="fa fa-pencil"></i></a>
        		    </div>
    		    <{/if}>
    	    ...
    	    <{/foreach}>
    	            ...
    	            <{if $admin_view_actions || $admin_view_options || isset($smarty.session.acl.workflowtransition.customfields)}>
        	    <div class="button-general dropdown">
        	        ...
        	        <ul class="dropdown-menu pull-right">
                        ...
                        <{/foreach}>
                        <{if isset($smarty.session.acl.workflowtransition.customfields)}>
                            <li><a class="scope-main" href="<{$smarty.const.APPLICATION_URL}>/customfield/manage/workflowtransition"><{_t('Manage custom fields')}></a></li>
                        <{/if}>
                    </ul>
        	    </div>
            <{/if}>
            <div class="button-general">
                <a class="button-close scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/workflowtransition/close/"><span class="button-face"><{_t('Close')}></span></a>
            </div>
