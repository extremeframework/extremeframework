...
/**
 * Usage: <{module class="WidgetListWorkflowTransition" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.workflowtransition.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListWorkflowTransition extends WorkflowTransitionController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.workflowtransition.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('workflowtransition')) {
            $vars = array_keys(get_object_vars(new WorkflowTransitionModel()));
