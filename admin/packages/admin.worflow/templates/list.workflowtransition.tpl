...
<{assign var='title' value=$smarty.const.L_WORKFLOW_TRANSITION}>
...
<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="workflowtransitionquicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/workflowtransition/search" method="post" enctype="multipart/form-data">
            <input type="text" name="workflowtransition_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{$smarty.const.L_SEARCH}>" />
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/workflowtransition/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{$smarty.const.L_NEW}> <{$smarty.const.L_WORKFLOW_TRANSITION|strtolower}>"/><{label key="L_NEW"}> <{$smarty.const.L_WORKFLOW_TRANSITION|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="workflowtransition_delete()"><span class="button-face"><{$smarty.const.L_DELETE}></span></a>
                        </div>
                                            <{/if}>
            	        ...
            	        <ul class="dropdown-menu pull-right">
                            <{foreach from=$admin_list_actions item=item}>
                                <{if empty($item->ACL) || isset($smarty.session.acl.workflowtransition[$item->ACL])}>
                                    <li><a <{if $item->IS_OPEN_IN_NEW_WINDOW}>target="_blank"<{/if}> <{if $item->LINK}>href="<{framework_report_link format=$item->LINK}>"<{/if}> <{if $item->ACTION}>onclick="workflowtransition_<{$item->ACTION}>(); return false;"<{/if}>><span class="button-face"><{label text=$item->TITLE}></span></a></li>
                                <{/if}>
                            <{/foreach}>
                        </ul>
            	    </div>
...
<!-- Relations -->
    ...
    <div id="workflowtransitioncopyrelations" style="display:none" title="<{$smarty.const.L_COPY}> <{$smarty.const.L_WORKFLOW_TRANSITION|strtolower}>">
        <p><{$copyguidelines}></p>
        <ul>
    ...
    <div id="workflowtransitionapproverelations" style="display:none" title="<{$smarty.const.L_APPROVE}> <{$smarty.const.L_WORKFLOW_TRANSITION|strtolower}>">
    ...
    <div id="workflowtransitiondeleterelations" style="display:none" title="<{$smarty.const.L_DELETE}> <{$smarty.const.L_WORKFLOW_TRANSITION|strtolower}>">
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="workflowtransition-list-count list-count"><{$total}></span> <{label key="L_RECORDS"}>
	</div>
	<div style="clear:both"></div>
<{/if}>
