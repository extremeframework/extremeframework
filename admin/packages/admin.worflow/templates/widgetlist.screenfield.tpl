...
<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="screenfieldquicksearch" class="form-quicksearch scope-main" action="<{$smarty.const.APPLICATION_URL}>/screenfield/search" method="post" enctype="multipart/form-data">
            <input type="text" name="screenfield_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{$smarty.const.L_SEARCH}>" />
...
<form name="screenfieldlist" id="screenfieldlistform" class="form-list scope-main" action="<{$smarty.const.APPLICATION_URL}>/screenfield/" method="post">
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/screenfield/new/<{if isset($preset) && isset($presetvalue)}>preset/<{$preset}>/presetvalue/<{$presetvalue}><{/if}><{if isset($presetstring)}>?<{$presetstring}><{/if}>"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{$smarty.const.L_NEW}> <{$smarty.const.L_SCREEN_FIELD|strtolower}>"/><{label key="L_NEW"}> <{$smarty.const.L_SCREEN_FIELD|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="screenfield_delete()"><span class="button-face"><{$smarty.const.L_DELETE}></span></a>
                        </div>
                                            <{/if}>
            	        ...
            	        <ul class="dropdown-menu pull-right">
                            <{foreach from=$admin_list_actions item=item}>
                                <{if empty($item->ACL) || isset($smarty.session.acl.screenfield[$item->ACL])}>
                                    <li><a <{if $item->IS_OPEN_IN_NEW_WINDOW}>target="_blank"<{/if}> <{if $item->LINK}>href="<{framework_report_link format=$item->LINK}>"<{/if}> <{if $item->ACTION}>onclick="screenfield_<{$item->ACTION}>(); return false;"<{/if}>><span class="button-face"><{label text=$item->TITLE}></span></a></li>
                                <{/if}>
                            <{/foreach}>
                        </ul>
            	    </div>
...
<{if $pagination || $total > 10}>
	...
	<div style="clear:both"></div>
            ...
            <script type="text/javascript">
            $(function() {
            	bind_hotkey('#screenfieldlistform', 'left', '.pagination-prev');
            	bind_hotkey('#screenfieldlistform', 'right', '.pagination-next');
            });
        </script>
    <{else}>
	<br>
	<div style="float:left">
		<span class="screenfield-list-count list-count"><{$total}></span> <{label key="L_RECORDS"}>
	</div>
	<div style="clear:both"></div>
<{/if}>
