<div class="view-main">
    <{plugin key="valuetype_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_VALUE_TYPE_NAME"}></label>
            </td>
    ...
    <{plugin key="valuetype_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        valuetypeview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        valuetypeview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                valuetypeview_apply_block_visibility(key, value);
            });
        }

        valuetypeview_apply_block_visibility = function (key, value) {
        ...
        valuetypeview_update_visibility_settings = function () {
            var settings = valuetypeview_get_visibility_settings();

            valuetypeview_apply_visibility_settings(settings);
        }

        $(function() {
            valuetypeview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/valuetype/fieldSave",
//                        data: td.find(':input').serialize() + "&valuetype_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
