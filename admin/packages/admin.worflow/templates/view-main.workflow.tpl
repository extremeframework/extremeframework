<div class="view-main">
    <{plugin key="workflow_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_WORKFLOW_NAME"}></label>
            </td>
	...
	<{/if}>
        ...
        <{if !isset($columns2hide) || !in_array('DESCRIPTION', $columns2hide) }>
    		<td class="form-field form-field-label column-description">
	            		        <label class="attribute-name"><{label key="L_DESCRIPTION"}></label>
            </td>
        ...
        <{$details->DESCRIPTION|plugin:"workflow_view_field_DESCRIPTION":$details}>
                                    </span>
                                <!--
                    		    <span class="edit-mode" style="display:none">
    ...
    <script type="text/javascript">
        ...
        workflowview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        workflowview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                workflowview_apply_block_visibility(key, value);
            });
        }

        workflowview_apply_block_visibility = function (key, value) {
        ...
        workflowview_update_visibility_settings = function () {
            var settings = workflowview_get_visibility_settings();

            workflowview_apply_visibility_settings(settings);
        }

        $(function() {
            workflowview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/workflow/fieldSave",
//                        data: td.find(':input').serialize() + "&workflow_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
