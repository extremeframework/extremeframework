...
/**
 * Usage: <{module class="WidgetListWorkflowStage" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.workflowstage.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListWorkflowStage extends WorkflowStageController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.workflowstage.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('workflowstage')) {
            $vars = array_keys(get_object_vars(new WorkflowStageModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('workflowstage', $preset, $presetvalue));
