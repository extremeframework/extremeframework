...
/**
 * Usage: <{module class="WidgetListWorkflowLog" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.workflowlog.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListWorkflowLog extends WorkflowLogController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.workflowlog.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('workflowlog')) {
            $vars = array_keys(get_object_vars(new WorkflowLogModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('workflowlog', $preset, $presetvalue));
