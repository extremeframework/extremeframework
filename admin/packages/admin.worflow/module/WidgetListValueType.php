...
/**
 * Usage: <{module class="WidgetListValueType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.valuetype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListValueType extends ValueTypeController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.valuetype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('valuetype')) {
            $vars = array_keys(get_object_vars(new ValueTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('valuetype', $preset, $presetvalue));
