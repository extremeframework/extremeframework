...
class _WorkflowApplicationController extends __AppController
{
    var $module = 'workflowapplication';
    var $type = 'controller';
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^workflowapplication_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflowapplication_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflowapplication_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('workflowapplication');

        AclController::checkPermission('workflowapplication', 'list');

		ContextStack::register(APPLICATION_URL.'/workflowapplication/list/');
    ...
    public function customFilterAction() {
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            $afm->UUID = $id;
            $afm->MODULE = 'workflowapplication';
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."WORKFLOW_APPLICATION.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'workflowapplication');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('workflowapplication_before_delete', $_model);
        }

		$model = new WorkflowApplicationModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('workflowapplication', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('workflowapplication', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['workflowapplicationlist_selection_selectall']) && $_REQUEST['workflowapplicationlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['workflowapplication.list.model']) && is_object($_SESSION['workflowapplication.list.model'])) {
					$model = clone $_SESSION['workflowapplication.list.model'];
				} else {
		        	$model = new WorkflowApplicationModel();
				}
    			...
    			$selection = isset($_REQUEST['workflowapplicationlist_selection'])? $_REQUEST['workflowapplicationlist_selection'] : '';
			}
        }
        ...
        if ($module != 'workflowapplication' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('workflowapplication', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new WorkflowApplicationModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('workflowapplication', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/workflowapplication/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/workflowapplication/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowapplication_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'workflowapplication_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('workflowapplication');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'MODULE', 'ID_WORKFLOW');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowapplication_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'MODULE', 'ID_WORKFLOW');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('workflowapplication'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowapplication_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'workflowapplication_multiformdata_deleteditems'])? $_REQUEST[$prefix.'workflowapplication_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('workflowapplication_before_save', $model);

    		if ($model->UUID) {
				
				$old = new WorkflowApplicationModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('workflowapplication_before_update', $model, $old);

        		if (!AclController::hasPermission('workflowapplication', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."WORKFLOW_APPLICATION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('workflowapplication_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('workflowapplication', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.workflowapplication.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.workflowapplication.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.workflowapplication.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.workflowapplication.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.workflowapplication.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.workflowapplication.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.workflowapplication.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.workflowapplication.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.workflowapplication.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('MODULE', 'ID_WORKFLOW');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['workflowapplication.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['workflowapplication.customview'])? $_SESSION['workflowapplication.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('workflowapplication');
        ...
        $_SESSION['workflowapplication.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('workflowapplication'));
		...
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflowapplication', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.workflowapplication.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.workflowapplication.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('workflowapplication');

        $aclviewablecolumns = AclController::getAclEnabledColumns('workflowapplication', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflowapplication', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflowapplication', 'new');
        }
        ...
        $ids = isset($_SESSION['workflowapplication.list.ids'])? $_SESSION['workflowapplication.list.ids'] : array();
        ...
        PluginManager::do_action('workflowapplication_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflowapplication', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('workflowapplication', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflowapplication', 'view'));
        ...
        PluginManager::do_action('workflowapplication_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.workflowapplication.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('workflowapplication');
        ...
        $roweditablecolumns = array('MODULE', 'ID_WORKFLOW');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('workflowapplication', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('workflowapplication', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('workflowapplication_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflowapplication', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('workflowapplication'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('workflowapplication'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    WorkflowApplicationController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['workflowapplication.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_WORKFLOW':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.ID_WORKFLOW IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.ID_WORKFLOW = '$value'");
                        }

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.$key IS NULL OR ".TABLE_PREFIX.".WORKFLOW_APPLICATION.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW_APPLICATION.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."WORKFLOW_APPLICATION.ID_WORKFLOW IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."WORKFLOW_APPLICATION.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_APPLICATION.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_APPLICATION.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
