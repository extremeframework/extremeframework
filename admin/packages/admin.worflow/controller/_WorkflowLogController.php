...
class _WorkflowLogController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('MODULE', $columns2check) && trim($model->MODULE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('workflowlog', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^workflowlog_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflowlog_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflowlog_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('workflowlog');

        AclController::checkPermission('workflowlog', 'list');

		ContextStack::register(APPLICATION_URL.'/workflowlog/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."WORKFLOW_LOG.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('workflowlog_before_delete', $_model);
        }

		$model = new WorkflowLogModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('workflowlog', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('workflowlog', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['workflowloglist_selection_selectall']) && $_REQUEST['workflowloglist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['workflowlog.list.model']) && is_object($_SESSION['workflowlog.list.model'])) {
					$model = clone $_SESSION['workflowlog.list.model'];
				} else {
		        	$model = new WorkflowLogModel();
				}
    			...
    			$selection = isset($_REQUEST['workflowloglist_selection'])? $_REQUEST['workflowloglist_selection'] : '';
			}
        }
        ...
        if ($module != 'workflowlog' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('workflowlog', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new WorkflowLogModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('workflowlog', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/workflowlog/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/workflowlog/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowlog_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'workflowlog_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('workflowlog');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowlog_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowlog_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'workflowlog_multiformdata_deleteditems'])? $_REQUEST[$prefix.'workflowlog_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('workflowlog_before_save', $model);

    		if ($model->UUID) {
				
				$old = new WorkflowLogModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('workflowlog_before_update', $model, $old);

        		if (!AclController::hasPermission('workflowlog', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."WORKFLOW_LOG.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('workflowlog_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('workflowlog', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.workflowlog.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.workflowlog.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.workflowlog.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.workflowlog.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.workflowlog.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.workflowlog.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.workflowlog.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.workflowlog.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.workflowlog.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('ID_WORKFLOW', 'ID_WORKFLOW_TRANSITION', 'MODULE', 'OBJECT_ID', 'DATE', 'ID_USER');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['workflowlog.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['workflowlog.customview'])? $_SESSION['workflowlog.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('workflowlog');
        ...
        $_SESSION['workflowlog.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('workflowlog'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('workflowlog', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('workflowlog', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflowlog', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.workflowlog.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflowlog', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflowlog', 'new');
        }
        ...
        $ids = isset($_SESSION['workflowlog.list.ids'])? $_SESSION['workflowlog.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflowlog', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('workflowlog', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflowlog', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.workflowlog.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('ID_WORKFLOW', 'ID_WORKFLOW_TRANSITION', 'MODULE', 'OBJECT_ID', 'DATE', 'ID_USER');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('workflowlog', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('workflowlog', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflowlog', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('workflowlog'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('workflowlog'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    WorkflowLogController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['workflowlog.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_WORKFLOW':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.ID_WORKFLOW IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.ID_WORKFLOW = '$value'");
                        }

                        break;

                    case 'ID_WORKFLOW_TRANSITION':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.ID_WORKFLOW_TRANSITION IS NULL");
                        } else {
                    ...
                    case 'ID_USER':
                        if ($value == '__NULL__') {
                            ...
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.ID_USER = '$value'");
                        }

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.$key IS NULL OR ".TABLE_PREFIX.".WORKFLOW_LOG.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW_LOG.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."WORKFLOW_LOG.ID_WORKFLOW IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."WORKFLOW_LOG.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_LOG.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_LOG.DETAILS LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_LOG.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
