...
class _ScreenFieldController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('screenfield', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^screenfield_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^screenfield_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^screenfield_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('screenfield');

        AclController::checkPermission('screenfield', 'list');

		ContextStack::register(APPLICATION_URL.'/screenfield/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."SCREEN_FIELD.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('screenfield_before_delete', $_model);
        }

		$model = new ScreenFieldModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('screenfield', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('screenfield', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['screenfieldlist_selection_selectall']) && $_REQUEST['screenfieldlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['screenfield.list.model']) && is_object($_SESSION['screenfield.list.model'])) {
					$model = clone $_SESSION['screenfield.list.model'];
				} else {
		        	$model = new ScreenFieldModel();
				}
    			...
    			$selection = isset($_REQUEST['screenfieldlist_selection'])? $_REQUEST['screenfieldlist_selection'] : '';
			}
        }
        ...
        if ($module != 'screenfield' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('screenfield', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new ScreenFieldModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('screenfield', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/screenfield/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/screenfield/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'screenfield_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'screenfield_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('screenfield');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'screenfield_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'screenfield_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'screenfield_multiformdata_deleteditems'])? $_REQUEST[$prefix.'screenfield_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('screenfield_before_save', $model);

    		if ($model->UUID) {
				
				$old = new ScreenFieldModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('screenfield_before_update', $model, $old);

        		if (!AclController::hasPermission('screenfield', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."SCREEN_FIELD.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('screenfield_before_create', $model);
    ...
    private function bind2refobject(&$model, $refobject = null) {
        if ($refobject != null) {
            ...
            if ($refclass == 'ScreenModel' && empty($model->ID_SCREEN)) {
                $model->ID_SCREEN = $refobject->CODE;
            }
            if ($refclass == 'ValueTypeModel' && empty($model->ID_VALUE_TYPE)) {
                $model->ID_VALUE_TYPE = $refobject->CODE;
            }
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('screenfield', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.screenfield.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.screenfield.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.screenfield.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.screenfield.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.screenfield.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.screenfield.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.screenfield.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.screenfield.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.screenfield.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('ID_SCREEN', 'TITLE', 'CODE', 'ID_VALUE_TYPE', 'DATASOURCE', 'VALUECOL', 'ORDERING');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['screenfield.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['screenfield.customview'])? $_SESSION['screenfield.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('screenfield');
        ...
        $_SESSION['screenfield.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('screenfield'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('screenfield', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('screenfield', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('screenfield', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.screenfield.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('screenfield', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('screenfield', 'new');
        }
        ...
        $ids = isset($_SESSION['screenfield.list.ids'])? $_SESSION['screenfield.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('screenfield', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('screenfield', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('screenfield', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.screenfield.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('ID_SCREEN', 'TITLE', 'CODE', 'ID_VALUE_TYPE', 'DATASOURCE', 'CONDITION', 'VALUECOL', 'TEXTCOL', 'IS_MANDATORY', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('screenfield', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('screenfield', $model->UUID, $model);
            }
        ...
        if ($details->UUID) {
            if (isset($details->ID_SCREEN) && $details->ID_SCREEN) {
                $acleditablecolumns['ID_SCREEN'] = false;

                $roweditablecolumns = array_diff($roweditablecolumns, array('ID_SCREEN'));
            }
        }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('screenfield', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('screenfield'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('screenfield'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    ScreenFieldController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['screenfield.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_SCREEN':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.ID_SCREEN IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.ID_SCREEN = '$value'");
                        }

                        break;

                    case 'TITLE':
                        $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'ID_VALUE_TYPE':
                        $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.ID_VALUE_TYPE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.$key IS NULL OR ".TABLE_PREFIX.".SCREEN_FIELD.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."SCREEN_FIELD.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."SCREEN_FIELD.ID_SCREEN IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.ID_VALUE_TYPE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.DATASOURCE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.CONDITION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.VALUECOL LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.TEXTCOL LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."SCREEN_FIELD.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
