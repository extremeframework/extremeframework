...
class _ScreenController extends __AppController
{
    ...
    private function _view($id, $details = null, $templatecode = 'view.screen.tpl') {
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.screen.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('screen');
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('screen', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."SCREEN.UDID = 0 OR ".TABLE_PREFIX."SCREEN.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."SCREEN.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
