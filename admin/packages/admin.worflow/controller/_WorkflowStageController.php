...
class _WorkflowStageController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_WORKFLOW_STAGE_NAME);
           return false;
       }
       if (in_array('CODE', $columns2check) && trim($model->CODE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('workflowstage', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^workflowstage_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflowstage_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^workflowstage_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('workflowstage');

        AclController::checkPermission('workflowstage', 'list');

		ContextStack::register(APPLICATION_URL.'/workflowstage/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."WORKFLOW_STAGE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('workflowstage_before_delete', $_model);
        }

		$model = new WorkflowStageModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('workflowstage', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('workflowstage_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('workflowstage', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['workflowstagelist_selection_selectall']) && $_REQUEST['workflowstagelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['workflowstage.list.model']) && is_object($_SESSION['workflowstage.list.model'])) {
					$model = clone $_SESSION['workflowstage.list.model'];
				} else {
		        	$model = new WorkflowStageModel();
				}
    			...
    			$selection = isset($_REQUEST['workflowstagelist_selection'])? $_REQUEST['workflowstagelist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'accessright': 
                            (new AccessRightController())->delete('WFID', $_ids);
                            break;

                        case 'acltype': 
                            (new AclTypeController())->delete('WFID', $_ids);
                            break;

                        case 'adminfilter': 
                            (new AdminFilterController())->delete('WFID', $_ids);
                            break;

                        case 'adminlabel': 
                            (new AdminLabelController())->delete('WFID', $_ids);
                            break;

                        case 'adminlanguage': 
                            (new AdminLanguageController())->delete('WFID', $_ids);
                            break;

                        case 'adminlanguageitem': 
                            (new AdminLanguageItemController())->delete('WFID', $_ids);
                            break;

                        case 'adminlayoutfield': 
                            (new AdminLayoutFieldController())->delete('WFID', $_ids);
                            break;

                        case 'adminlayoutsection': 
                            (new AdminLayoutSectionController())->delete('WFID', $_ids);
                            break;

                        case 'adminmenu': 
                            (new AdminMenuController())->delete('WFID', $_ids);
                            break;

                        case 'adminmenuitem': 
                            (new AdminMenuItemController())->delete('WFID', $_ids);
                            break;
                        ...
                        case 'adminview': 
                            (new AdminViewController())->delete('WFID', $_ids);
                            break;

                        case 'changelog': 
                            (new ChangeLogController())->delete('WFID', $_ids);
                            break;

                        case 'dashboard': 
                            (new DashboardController())->delete('WFID', $_ids);
                            break;

                        case 'field': 
                            (new FieldController())->delete('WFID', $_ids);
                            break;
                        ...
                        case 'parameter': 
                            (new ParameterController())->delete('WFID', $_ids);
                            break;

                        case 'parametergroup': 
                            (new ParameterGroupController())->delete('WFID', $_ids);
                            break;
                        ...
                        case 'screen': 
                            (new ScreenController())->delete('WFID', $_ids);
                            break;
                        ...
                        case 'user': 
                            (new UserController())->delete('WFID', $_ids);
                            break;

                        case 'usergroup': 
                            (new UserGroupController())->delete('WFID', $_ids);
                            break;

                        case 'userlog': 
                            (new UserLogController())->delete('WFID', $_ids);
                            break;

                        case 'usermembership': 
                            (new UserMembershipController())->delete('WFID', $_ids);
                            break;

                        case 'userpreference': 
                            (new UserPreferenceController())->delete('WFID', $_ids);
                            break;

                        case 'userrole': 
                            (new UserRoleController())->delete('WFID', $_ids);
                            break;
                        ...
                        case 'workflow': 
                            (new WorkflowController())->delete('WFID', $_ids);
                            break;

                        case 'workflowapplication': 
                            (new WorkflowApplicationController())->delete('WFID', $_ids);
                            break;

                        case 'workflowlog': 
                            (new WorkflowLogController())->delete('WFID', $_ids);
                            break;

                        case 'workflowtransition': 
                            (new WorkflowTransitionController())->delete('START_ID_WORKFLOW_STAGE', $_ids);
                            break;
        ...
        if ($module != 'workflowstage' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('workflowstage', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new WorkflowStageModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('workflowstage', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/workflowstage/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/workflowstage/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowstage_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'workflowstage_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('workflowstage');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowstage_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'workflowstage_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'workflowstage_multiformdata_deleteditems'])? $_REQUEST[$prefix.'workflowstage_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('workflowstage_before_save', $model);

    		if ($model->UUID) {
				
				$old = new WorkflowStageModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('workflowstage_before_update', $model, $old);

        		if (!AclController::hasPermission('workflowstage', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."WORKFLOW_STAGE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('workflowstage_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('workflowstage_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('workflowstage_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('workflowstage');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('workflowstage', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.workflowstage.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.workflowstage.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.workflowstage.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.workflowstage.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.workflowstage.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.workflowstage.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.workflowstage.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.workflowstage.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.workflowstage.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['workflowstage.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['workflowstage.customview'])? $_SESSION['workflowstage.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('workflowstage');
        ...
        $_SESSION['workflowstage.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('workflowstage'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('workflowstage', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('workflowstage', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflowstage', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.workflowstage.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflowstage', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('workflowstage', 'new');
        }
        ...
        $ids = isset($_SESSION['workflowstage.list.ids'])? $_SESSION['workflowstage.list.ids'] : array();
        ...
        PluginManager::do_action('workflowstage_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflowstage', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('workflowstage', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('workflowstage', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.workflowstage.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('ID_WORKFLOW', 'NAME', 'CODE', 'IS_DEFAULT', 'IS_BINDING_OBJECT_EDITABLE', 'IS_BINDING_OBJECT_DELETABLE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('workflowstage', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('workflowstage_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('workflowstage', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('workflowstage_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('workflowstage', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('workflowstage'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('workflowstage'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    WorkflowStageController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['workflowstage.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_WORKFLOW':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.ID_WORKFLOW IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.ID_WORKFLOW = '$value'");
                        }

                        break;

                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.$key IS NULL OR ".TABLE_PREFIX.".WORKFLOW_STAGE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."WORKFLOW_STAGE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'WORKFLOW_STAGE`.ID_WORKFLOW, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.NAME, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.CODE, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.IS_DEFAULT, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.IS_BINDING_OBJECT_EDITABLE, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.IS_BINDING_OBJECT_DELETABLE, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.ID, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.JSON, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.UUID, `'.TABLE_PREFIX.'WORKFLOW_STAGE`.WFID');
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."WORKFLOW_STAGE.ID_WORKFLOW IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."WORKFLOW_STAGE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_STAGE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_STAGE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."WORKFLOW_STAGE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
