...
class _ValueTypeController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_VALUE_TYPE_NAME);
           return false;
       }
       ...
       if (in_array('CODE', $columns2check)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('valuetype', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^valuetype_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^valuetype_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^valuetype_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('valuetype');

        AclController::checkPermission('valuetype', 'list');

		ContextStack::register(APPLICATION_URL.'/valuetype/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."VALUE_TYPE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('valuetype_before_delete', $_model);
        }

		$model = new ValueTypeModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('valuetype', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('valuetype', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['valuetypelist_selection_selectall']) && $_REQUEST['valuetypelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['valuetype.list.model']) && is_object($_SESSION['valuetype.list.model'])) {
					$model = clone $_SESSION['valuetype.list.model'];
				} else {
		        	$model = new ValueTypeModel();
				}
    			...
    			$selection = isset($_REQUEST['valuetypelist_selection'])? $_REQUEST['valuetypelist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'screenfield': 
                            (new ScreenFieldController())->delete('ID_VALUE_TYPE', $_ids);
                            break;
        ...
        if ($module != 'valuetype' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('valuetype', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new ValueTypeModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('valuetype', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/valuetype/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/valuetype/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'valuetype_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'valuetype_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'valuetype_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'valuetype_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'valuetype_multiformdata_deleteditems'])? $_REQUEST[$prefix.'valuetype_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('valuetype_before_save', $model);

    		if ($model->UUID) {
				
				$old = new ValueTypeModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('valuetype_before_update', $model, $old);

        		if (!AclController::hasPermission('valuetype', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."VALUE_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('valuetype_before_create', $model);
        ...
        DraftHelper::clearAllDrafts('valuetype');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('valuetype', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.valuetype.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.valuetype.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.valuetype.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.valuetype.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.valuetype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.valuetype.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.valuetype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.valuetype.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.valuetype.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['valuetype.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['valuetype.customview'])? $_SESSION['valuetype.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('valuetype');
        ...
        $_SESSION['valuetype.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('valuetype'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('valuetype', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('valuetype', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('valuetype', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.valuetype.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('valuetype', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('valuetype', 'new');
        }
        ...
        $ids = isset($_SESSION['valuetype.list.ids'])? $_SESSION['valuetype.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('valuetype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('valuetype', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('valuetype', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.valuetype.tpl', $restoredraft = true) {
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('valuetype', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('valuetype_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('valuetype', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('valuetype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('valuetype'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('valuetype'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    ValueTypeController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['valuetype.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.$key IS NULL OR ".TABLE_PREFIX.".VALUE_TYPE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."VALUE_TYPE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."VALUE_TYPE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."VALUE_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."VALUE_TYPE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."VALUE_TYPE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
