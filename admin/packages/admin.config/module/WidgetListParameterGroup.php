...
/**
 * Usage: <{module class="WidgetListParameterGroup" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.parametergroup.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListParameterGroup extends ParameterGroupController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.parametergroup.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('parametergroup')) {
            $vars = array_keys(get_object_vars(new ParameterGroupModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('parametergroup', $preset, $presetvalue));
