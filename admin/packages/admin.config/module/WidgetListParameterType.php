...
/**
 * Usage: <{module class="WidgetListParameterType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.parametertype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListParameterType extends ParameterTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.parametertype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('parametertype')) {
            $vars = array_keys(get_object_vars(new ParameterTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('parametertype', $preset, $presetvalue));
