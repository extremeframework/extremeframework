...
<{assign var='title' value=_t('Parameter Type')}>
...
<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="parametertypequicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/parametertype/search" method="get">
            <input type="text" name="parametertype_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{_t('Search', true)}>" />
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main cachable" href="<{$smarty.const.APPLICATION_URL}>/parametertype/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{_t('New', true)}> <{_t('Parameter Type', true)|strtolower}>"/><{_t('New')}> <{_t('Parameter Type')|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="parametertype_delete()"><span class="button-face"><{_t('Delete')}></span></a>
                        </div>
                                            <{/if}>
...
<!-- Relations -->
    <{php}>
    	$template->assign('copyguidelines',  sprintf(_t('L_GUIDELINES_COPY_RELS', true), strtolower(_t('Parameter Type', true))));
    	$template->assign('approveguidelines', sprintf(_t('L_GUIDELINES_APPROVE_RELS', true), strtolower(_t('Parameter Type', true))));
    	$template->assign('deleteguidelines', sprintf(_t('L_GUIDELINES_DELETE_RELS', true), strtolower(_t('Parameter Type', true)), strtolower(_t('Parameter Type', true))));
    <{/php}>

    <div id="parametertypecopyrelations" style="display:none" title="<{_t('Copy', true)}> <{_t('Parameter Type', true)|strtolower}>">
        <p><{$copyguidelines}></p>
        <ul>
    ...
    <div id="parametertypeapproverelations" style="display:none" title="<{_t('Approve', true)}> <{_t('Parameter Type', true)|strtolower}>">
    ...
    <div id="parametertypedeleterelations" style="display:none" title="<{_t('Delete', true)}> <{_t('Parameter Type', true)|strtolower}>">
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="parametertype-list-count list-count"><{$total}></span> <{_t('Records')}>
	</div>
	<div style="clear:both"></div>
<{/if}>
