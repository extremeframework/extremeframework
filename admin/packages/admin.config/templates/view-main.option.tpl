<div class="view-main">
    <{plugin key="option_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_OPTION_NAME"}></label>
            </td>
	...
	<{/if}>
        ...
        <{if !isset($columns2hide) || !in_array('VALUE', $columns2hide) }>
    		<td class="form-field form-field-label column-value">
	            		        <label class="attribute-name"><{label key="L_VALUE"}></label>
            </td>
	...
	<{/if}>
        ...
        <{if !isset($columns2hide) || !in_array('CONTEXT', $columns2hide) }>
    		<td class="form-field form-field-label column-context">
	            		        <label class="attribute-name"><{label key="L_CONTEXT"}></label>
            </td>
            <td class="form-field form-field-value column-context" colspan="3">
            	<div class="attribute-line column-context type-text">
            		<div class="attribute-value">
            		                		        <{if (isset($acleditablecolumns['CONTEXT']) && $acleditablecolumns['CONTEXT']) || (isset($acleditablecolumns['*']) && (!isset($acleditablecolumns['CONTEXT']) || $acleditablecolumns['CONTEXT'])) }>
                    		    <span class="value-mode">
                                    
    <{$details->CONTEXT|escape}>
                                </span>
                                <!--
                    		    <span class="edit-mode" style="display:none">
    ...
    <{plugin key="option_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        optionview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        optionview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                optionview_apply_block_visibility(key, value);
            });
        }

        optionview_apply_block_visibility = function (key, value) {
        ...
        optionview_update_visibility_settings = function () {
            var settings = optionview_get_visibility_settings();

            optionview_apply_visibility_settings(settings);
        }

        $(function() {
            optionview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/option/fieldSave",
//                        data: td.find(':input').serialize() + "&option_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
