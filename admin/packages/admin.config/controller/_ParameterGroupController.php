...
class _ParameterGroupController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_PARAMETER_GROUP_NAME);
           return false;
       }
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('parametergroup', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^parametergroup_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^parametergroup_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^parametergroup_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('parametergroup');

        AclController::checkPermission('parametergroup', 'list');

		ContextStack::register(APPLICATION_URL.'/parametergroup/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."PARAMETER_GROUP.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'parametergroup');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new ParameterGroupModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('parametergroup_before_delete', $_model);
        }

		$model = new ParameterGroupModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('parametergroup', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('parametergroup', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['parametergrouplist_selection_selectall']) && $_REQUEST['parametergrouplist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['parametergroup.list.model']) && is_object($_SESSION['parametergroup.list.model'])) {
					$model = clone $_SESSION['parametergroup.list.model'];
				} else {
		        	$model = new ParameterGroupModel();
				}
    			...
    			$selection = isset($_REQUEST['parametergrouplist_selection'])? $_REQUEST['parametergrouplist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'parameter': 
                            (new ParameterController())->delete('ID_PARAMETER_GROUP', $_ids);
                            break;
        ...
        if ($module != 'parametergroup' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('parametergroup', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new ParameterGroupModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('parametergroup', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/parametergroup/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/parametergroup/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'parametergroup_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'parametergroup_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('parametergroup');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'parametergroup_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'parametergroup_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'parametergroup_multiformdata_deleteditems'])? $_REQUEST[$prefix.'parametergroup_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('parametergroup_before_save', $model);

    		if ($model->UUID) {
				
				$old = new ParameterGroupModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('parametergroup_before_update', $model, $old);

        		if (!AclController::hasPermission('parametergroup', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."PARAMETER_GROUP.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('parametergroup_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('parametergroup', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.parametergroup.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.parametergroup.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.parametergroup.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.parametergroup.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.parametergroup.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.parametergroup.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.parametergroup.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.parametergroup.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.parametergroup.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['parametergroup.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['parametergroup.customview'])? $_SESSION['parametergroup.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('parametergroup');
        ...
        $_SESSION['parametergroup.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('parametergroup'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('parametergroup', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('parametergroup', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('parametergroup', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.parametergroup.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('parametergroup', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('parametergroup', 'new');
        }
        ...
        $ids = isset($_SESSION['parametergroup.list.ids'])? $_SESSION['parametergroup.list.ids'] : array();
        ...
        PluginManager::do_action('parametergroup_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('parametergroup', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('parametergroup', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('parametergroup', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.parametergroup.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('NAME', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('parametergroup', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('parametergroup_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('parametergroup', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('parametergroup_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('parametergroup', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('parametergroup'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('parametergroup'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    ParameterGroupController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['parametergroup.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.$key IS NULL OR ".TABLE_PREFIX.".PARAMETER_GROUP.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."PARAMETER_GROUP.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'PARAMETER_GROUP`.NAME, `'.TABLE_PREFIX.'PARAMETER_GROUP`.ID, `'.TABLE_PREFIX.'PARAMETER_GROUP`.JSON, `'.TABLE_PREFIX.'PARAMETER_GROUP`.UUID, `'.TABLE_PREFIX.'PARAMETER_GROUP`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."PARAMETER_GROUP.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER_GROUP.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER_GROUP.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
