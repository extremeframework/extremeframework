...
class _ParameterController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('CODE', $columns2check)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('parameter', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^parameter_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^parameter_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^parameter_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('parameter');

        AclController::checkPermission('parameter', 'list');

		ContextStack::register(APPLICATION_URL.'/parameter/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."PARAMETER.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('parameter_before_delete', $_model);
        }

		$model = new ParameterModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('parameter', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."PARAMETER.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('parameter', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['parameterlist_selection_selectall']) && $_REQUEST['parameterlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['parameter.list.model']) && is_object($_SESSION['parameter.list.model'])) {
					$model = clone $_SESSION['parameter.list.model'];
				} else {
		        	$model = new ParameterModel();
				}
    			...
    			$selection = isset($_REQUEST['parameterlist_selection'])? $_REQUEST['parameterlist_selection'] : '';
			}
        }
        ...
        if ($module != 'parameter' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('parameter', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new ParameterModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('parameter', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/parameter/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/parameter/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'parameter_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'parameter_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('parameter');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'parameter_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'parameter_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'parameter_multiformdata_deleteditems'])? $_REQUEST[$prefix.'parameter_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('parameter_before_save', $model);

    		if ($model->UUID) {
				
				$old = new ParameterModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('parameter_before_update', $model, $old);

        		if (!AclController::hasPermission('parameter', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."PARAMETER.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."PARAMETER.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."PARAMETER.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('parameter_before_create', $model);
    ...
    private function bind2refobject(&$model, $refobject = null) {
        if ($refobject != null) {
            ...
            if ($refclass == 'ParameterGroupModel' && empty($model->ID_PARAMETER_GROUP)) {
                $model->ID_PARAMETER_GROUP = $refobject->ID;
            }
            if ($refclass == 'ParameterTypeModel' && empty($model->ID_PARAMETER_TYPE)) {
                $model->ID_PARAMETER_TYPE = $refobject->ID;
            }
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('parameter', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.parameter.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.parameter.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.parameter.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.parameter.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.parameter.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.parameter.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.parameter.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.parameter.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.parameter.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['parameter.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['parameter.customview'])? $_SESSION['parameter.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('parameter');
        ...
        $_SESSION['parameter.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('parameter'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('parameter', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('parameter', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('parameter', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.parameter.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('parameter', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('parameter', 'new');
        }
        ...
        $ids = isset($_SESSION['parameter.list.ids'])? $_SESSION['parameter.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('parameter', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('parameter', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('parameter', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.parameter.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('NAME', 'CODE', 'ID_PARAMETER_GROUP', 'ID_PARAMETER_TYPE', 'VALUE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('parameter', 'editpeer');
                }
            } else {
                // Set default values here
                if ($recent = $this->getRecentModel()) {
                    $model->ID_PARAMETER_GROUP = $recent->ID_PARAMETER_GROUP;
                    $model->ID_PARAMETER_TYPE = $recent->ID_PARAMETER_TYPE;
                }

                $this->onInitialization($model);
                PluginManager::do_action('parameter_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('parameter', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('parameter', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('parameter'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('parameter'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    ParameterController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['parameter.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."PARAMETER.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'ID_PARAMETER_GROUP':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."PARAMETER.ID_PARAMETER_GROUP IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."PARAMETER.ID_PARAMETER_GROUP = '$value'");
                        }

                        break;

                    case 'ID_PARAMETER_TYPE':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."PARAMETER.ID_PARAMETER_TYPE IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."PARAMETER.ID_PARAMETER_TYPE = '$value'");
                        }

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."PARAMETER.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."PARAMETER.$key IS NULL OR ".TABLE_PREFIX.".PARAMETER.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."PARAMETER.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."PARAMETER.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER.VALUE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER.DESCRIPTION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PARAMETER.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
