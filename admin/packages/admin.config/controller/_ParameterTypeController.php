...
class _ParameterTypeController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(_t('L_VALIDATION_NOT_EMPTY'), _t('L_PARAMETER_TYPE_NAME'));
           return false;
       }
    ...
    private function _view($id, $details = null, $templatecode = 'view.parametertype.tpl') {
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.parametertype.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('parametertype');
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('parametertype', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."PARAMETER_TYPE.UDID = 0 OR ".TABLE_PREFIX."PARAMETER_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
