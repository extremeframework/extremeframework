...
/**
 * Usage: <{module class="WidgetListAdminPage" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminpage.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminPage extends AdminPageController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminpage.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminpage')) {
            $vars = array_keys(get_object_vars(new AdminPageModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpage', $preset, $presetvalue));
