...
class _AdminPageController extends __AppController
{
    var $module = 'adminpage';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('adminpage_init');
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('SLUG', $columns2check) && trim($model->SLUG) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('adminpage', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminpage_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpage_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminpage_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminpage');

        AclController::checkPermission('adminpage', 'list');

		ContextStack::register(APPLICATION_URL.'/adminpage/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_PAGE.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'adminpage');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpage_before_delete', $_model);
        }

		$model = new AdminPageModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminpage', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminpage_deleted', $_model);
        }
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminpage', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminpagelist_selection_selectall']) && $_REQUEST['adminpagelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminpage.list.model']) && is_object($_SESSION['adminpage.list.model'])) {
					$model = clone $_SESSION['adminpage.list.model'];
				} else {
		        	$model = new AdminPageModel();
				}
    			...
    			$selection = isset($_REQUEST['adminpagelist_selection'])? $_REQUEST['adminpagelist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminpage' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminpage', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminPageModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminpage', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminpage/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminpage/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpage_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminpage_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminpage');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'TITLE', 'SLUG', 'CONTENT');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpage_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'TITLE', 'SLUG', 'CONTENT');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminpage'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminpage_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminpage_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminpage_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminpage_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminPageModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminpage_before_update', $model, $old);

        		if (!AclController::hasPermission('adminpage', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_PAGE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('adminpage_updated', $model, $old);
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('adminpage_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('adminpage_created', $model);
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('adminpage');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminpage', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminpage.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminpage', 'import');

        if (isset($_FILES['adminpage_importfile'])) {
        	if (!$_FILES['adminpage_importfile']['error']) {
	            $filepath = $_FILES['adminpage_importfile']['tmp_name'];
        		$original = $_FILES['adminpage_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminpage/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('TITLE','SLUG','CONTENT');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminpage.list.model']) && is_object($_SESSION['adminpage.list.model'])) {
			$model = clone $_SESSION['adminpage.list.model'];
		} else {
        	$model = new AdminPageModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminpage_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminpage_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminpage_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('TITLE','SLUG','CONTENT');
        $filename = 'adminpage_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminPage');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminpage.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminpage.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminpage.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminpage.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminpage.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminpage.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminpage.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminpage.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminpage.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminpage.customview'])? $_SESSION['adminpage.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminpage', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpage', 'view');
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminpage');
        ...
        $_SESSION['adminpage.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminpage'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminpage', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminpage', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpage', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.adminpage.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.adminpage.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminpage');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminpage', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpage', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminpage', 'new');
        }
        ...
        $ids = isset($_SESSION['adminpage.list.ids'])? $_SESSION['adminpage.list.ids'] : array();
        ...
        PluginManager::do_action('adminpage_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpage', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminpage', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminpage', 'view'));
        ...
        PluginManager::do_action('adminpage_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminpage.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminpage');
        ...
        $roweditablecolumns = array('TITLE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminpage', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminpage_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminpage', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminpage_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminpage', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminpage'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminpage'));

	    $this->display($smarty, $templatecode);
	}

    protected function getLayoutColumns() {
        return array('TITLE', 'SLUG', 'CONTENT');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminPageController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminpage.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'TITLE':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_PAGE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_PAGE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PAGE`.TITLE, `'.TABLE_PREFIX.'ADMIN_PAGE`.SLUG, `'.TABLE_PREFIX.'ADMIN_PAGE`.CONTENT, `'.TABLE_PREFIX.'ADMIN_PAGE`.ID, `'.TABLE_PREFIX.'ADMIN_PAGE`.JSON, `'.TABLE_PREFIX.'ADMIN_PAGE`.UUID, `'.TABLE_PREFIX.'ADMIN_PAGE`.WFID');
    }

    protected function initListModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_PAGE`.TITLE, `'.TABLE_PREFIX.'ADMIN_PAGE`.SLUG, `'.TABLE_PREFIX.'ADMIN_PAGE`.ID, `'.TABLE_PREFIX.'ADMIN_PAGE`.JSON, `'.TABLE_PREFIX.'ADMIN_PAGE`.UUID, `'.TABLE_PREFIX.'ADMIN_PAGE`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_PAGE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PAGE.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PAGE.SLUG LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PAGE.CONTENT LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_PAGE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.adminpage.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('TITLE','SLUG','CONTENT');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminpage_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('adminpage_imported', $model);
		}
