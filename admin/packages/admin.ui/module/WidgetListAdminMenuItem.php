...
/**
 * Usage: <{module class="WidgetListAdminMenuItem" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminmenuitem.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminMenuItem extends AdminMenuItemController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminmenuitem.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('adminmenuitem')) {
            $vars = array_keys(get_object_vars(new AdminMenuItemModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminmenuitem', $preset, $presetvalue));
