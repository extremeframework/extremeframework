...
/**
 * Usage: <{module class="WidgetListAdminModule" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminmodule.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminModule extends AdminModuleController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminmodule.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminmodule')) {
            $vars = array_keys(get_object_vars(new AdminModuleModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminmodule', $preset, $presetvalue));
