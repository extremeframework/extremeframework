...
/**
 * Usage: <{module class="WidgetListField" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.field.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListField extends FieldController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.field.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('field')) {
            $vars = array_keys(get_object_vars(new FieldModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('field', $preset, $presetvalue));
