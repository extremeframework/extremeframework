...
/**
 * Usage: <{module class="WidgetListUserPreference" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.userpreference.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListUserPreference extends UserPreferenceController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.userpreference.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('userpreference')) {
            $vars = array_keys(get_object_vars(new UserPreferenceModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('userpreference', $preset, $presetvalue));
