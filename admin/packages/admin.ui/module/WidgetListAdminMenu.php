...
/**
 * Usage: <{module class="WidgetListAdminMenu" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminmenu.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminMenu extends AdminMenuController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminmenu.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('adminmenu')) {
            $vars = array_keys(get_object_vars(new AdminMenuModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminmenu', $preset, $presetvalue));
