...
/**
 * Usage: <{module class="WidgetListDashboard" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.dashboard.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListDashboard extends DashboardController {
    function fetch($params, $smarty) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.dashboard.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        $smarty = clone $smarty;

        if (AclController::hasPermission('dashboard')) {
            $vars = array_keys(get_object_vars(new DashboardModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('dashboard', $preset, $presetvalue));
