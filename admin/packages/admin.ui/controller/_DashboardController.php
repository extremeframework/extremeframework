...
class _DashboardController extends __AppController
{
    ...
    private function _view($id, $details = null, $templatecode = 'view.dashboard.tpl') {
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.dashboard.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('dashboard');
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('dashboard', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."DASHBOARD.UDID = 0 OR ".TABLE_PREFIX."DASHBOARD.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."DASHBOARD.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
