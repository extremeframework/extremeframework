...
class _AdminMenuController extends __AppController
{
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(_t('L_VALIDATION_NOT_EMPTY'), _t('L_ADMIN_MENU_NAME'));
           return false;
       }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('adminmenu', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.UDID = 0 OR ".TABLE_PREFIX."ADMIN_MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
