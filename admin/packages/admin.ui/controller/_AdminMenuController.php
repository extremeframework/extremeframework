...
class _AdminMenuController extends __AppController
{
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('adminmenu', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminmenu_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminmenu_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminmenu_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminmenu');
    ...
    public function updateOrderAction() {
        ...
        AclController::checkPermission('adminmenu', 'edit');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_MENU.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminmenu_before_delete', $_model);
        }

		$model = new AdminMenuModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminmenu', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminmenu', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminmenulist_selection_selectall']) && $_REQUEST['adminmenulist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminmenu.list.model']) && is_object($_SESSION['adminmenu.list.model'])) {
					$model = clone $_SESSION['adminmenu.list.model'];
				} else {
		        	$model = new AdminMenuModel();
				}
    			...
    			$selection = isset($_REQUEST['adminmenulist_selection'])? $_REQUEST['adminmenulist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminmenu' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('adminmenu', 'new');

		ContextStack::register(APPLICATION_URL.'/adminmenu/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminmenu', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminMenuModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminmenu', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminmenu/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminmenu/edit/'.$id);
		$this->_edit($id);
	}
    ...
    static function sync_JSON_ADMIN_MENU_ITEMS($id) {
        $model = new AdminMenuItemModel();
        $model->ID_ADMIN_MENU = $id;
        ...
        AdminMenuController::update_json($id, 'ADMIN_MENU_ITEM', $items);
    }
    ...
    protected function getTempCreateId() {
        ...
        if (!isset($_SESSION['adminmenu.tmpid'])) {
            $create = true;
        } else {
            $model = new AdminMenuModel();

            $model->UUID = $_SESSION['adminmenu.tmpid'];
        ...
        if ($create) {
            ...
            $_SESSION['adminmenu.tmpid'] = $model->UUID;
        }

        return $_SESSION['adminmenu.tmpid'];
    }

    protected function clearTempCreateId() {
        if (isset($_SESSION['adminmenu.tmpid'])) {
            unset($_SESSION['adminmenu.tmpid']);
        }
    }

    protected function clearTempCreateItem() {
        if (isset($_SESSION['adminmenu.tmpid'])) {
            self::deleteItem($_SESSION['adminmenu.tmpid']);
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminmenu_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminmenu_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminmenu');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'NAME', 'ORDERING', 'IS_ENABLED');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminmenu_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'ORDERING', 'IS_ENABLED');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('adminmenu'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminmenu_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
                ...
                PluginManager::do_action('adminmenu_before_update', $model, $old);

        		if (!AclController::hasPermission('adminmenu', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminmenu', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminmenu.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.adminmenu.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.adminmenu.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.adminmenu.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminmenu.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminmenu.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminmenu.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminmenu.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminmenu.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminmenu.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminmenu.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminmenu.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('NAME', 'ORDERING', 'IS_ENABLED');
        }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminmenu.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminmenu.customview'])? $_SESSION['adminmenu.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('adminmenu', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminmenu', 'view');
        ...
        $_SESSION['adminmenu.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminmenu'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminmenu', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminmenu', 'listitem'));
    ...
    protected function _view($id, $details = null, $templatecode = 'view.adminmenu.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('adminmenu');

        $aclviewablecolumns = AclController::getAclEnabledColumns('adminmenu', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminmenu', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminmenu', 'new');
        }
        ...
        $ids = isset($_SESSION['adminmenu.list.ids'])? $_SESSION['adminmenu.list.ids'] : array();
        ...
        PluginManager::do_action('adminmenu_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminmenu', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminmenu', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminmenu', 'view'));
        ...
        PluginManager::do_action('adminmenu_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.adminmenu.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('adminmenu');
        ...
        $roweditablecolumns = array('NAME', 'ORDERING', 'IS_ENABLED');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminmenu', 'editpeer');
                }
            } else {
        ...
        PluginManager::do_action('adminmenu_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminmenu', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminmenu'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminmenu'));

	    $this->display($smarty, $templatecode);
	}

    protected function getLayoutColumns() {
        return array('NAME', 'IS_ENABLED');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('adminmenu', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.UDID = 0 OR ".TABLE_PREFIX."ADMIN_MENU.UDID IN ('".implode("','", AclController::getExtraUDIDs())."') OR ".TABLE_PREFIX."ADMIN_MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminMenuController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminmenu.list.model'] = clone $model;
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_MENU.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_MENU.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_MENU`.NAME, `'.TABLE_PREFIX.'ADMIN_MENU`.IS_ENABLED, `'.TABLE_PREFIX.'ADMIN_MENU`.ID, `'.TABLE_PREFIX.'ADMIN_MENU`.JSON, `'.TABLE_PREFIX.'ADMIN_MENU`.UUID, `'.TABLE_PREFIX.'ADMIN_MENU`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_MENU.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_MENU.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_MENU.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
