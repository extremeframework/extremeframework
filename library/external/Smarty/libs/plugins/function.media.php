...
function smarty_function_media($params, &$smarty)
{
	...
	$width = isset($params['width'])? $params['width'] : THUMBS_WIDTH;
	$height = isset($params['height'])? $params['height'] : THUMBS_HEIGHT;
    ...
    if (preg_match('/\.(gif|jpg|jpeg|png|swf|flv|svg)/is', $src, $matches)) {
        ...
        if (preg_match('/(gif|jpg|jpeg|png|svg)/is', $extension)) {
            ...
            if ($thumbnail && !$remote) {
                ...
                if ($remote) {
                    $html = '<img '.$class.'src="'.$src.'" width="'.$width.'" height="'.$height.'"/>';
                } else {
                    $html = '<img '.$class.'src="'.UPLOAD_URL.'/'.$src.'" width="'.$width.'" height="'.$height.'"/>';
                }
            }
