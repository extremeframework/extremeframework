...
//        }
        ...
        // VIETTQ - Skip tables that do not match with our specified prefix
        if (TABLE_PREFIX != '' && !preg_match('/^'.TABLE_PREFIX.'/i', $table)) {
            return '';
        }
    ...
    */
    ...
    function getClassNameFromTableName($table)
    {
        ...
        return  $class_prefix.preg_replace('/[^A-Z0-9]/i','_',ucfirst(trim($table)));
    }
    ...
    */
    ...
    function getFileNameFromTableName($table)
    {
        ...
        if (strpos($options['class_location'],'%s') !== false) {
            ...
            preg_replace('/[^A-Z0-9]/i','_',ucfirst($table)));
        } else {
            $outfilename = "{$base}/".preg_replace('/[^A-Z0-9]/i','_',ucfirst($table)).".php";
        }
        return $outfilename;
    ...
    */
    ...
    function generateClasses()
    {
        ...
        foreach($this->tables as $this->table) {
            // VIETTQ - Skip tables that do not match with our specified prefix
            if (TABLE_PREFIX != '' && !preg_match('/^'.TABLE_PREFIX.'/i', trim($this->table))) {
                continue;
            }
