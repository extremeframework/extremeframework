...
class DB_DataObject extends DB_DataObject_Overload
{
    ...
    */
    ...
    function find($n = false)
    {
        ...
        $sql = 'SELECT ' .
            ...
            ' FROM ' . ($quoteIdentifiers ? $DB->quoteIdentifier(TABLE_PREFIX.$this->__table) : TABLE_PREFIX.$this->__table) . " \n" .
            $this->_join . " \n" .
    ...
    function fetch()
    {
        ...
        if (!empty($_DB_DATAOBJECT['CONFIG']['debug'])) {
            $this->debug(TABLE_PREFIX."{$this->__table} DONE", "fetchrow",2);
        }
    ...
    function selectAs($from = null,$format = '%s',$tableName=false)
    {
        ...
        foreach ($from as $k) {
            $this->selectAdd(sprintf("{$s}.{$s} as {$format}",TABLE_PREFIX.$table,$k,$k));
        }
        $this->_query['data_select'] .= "\n";
    }
    /**
    ...
    function insert()
    {
        ...
        if (($key !== false) && !$useNative) {
            ...
            if (!$seq) {
                $keyvalue =  $DB->nextId(TABLE_PREFIX.$this->__table);
            } else {
        ...
        if ($leftq || $useNative) {
            $table = ($quoteIdentifiers ? $DB->quoteIdentifier(TABLE_PREFIX.$this->__table)    : TABLE_PREFIX.$this->__table);

            $r = $this->_query("INSERT ".(isset($this->_insert_ignore) && $this->_insert_ignore? 'IGNORE ' : '')."INTO {$table} ($leftq) VALUES ($rightq) ");
    ...
    function fetchRow($row = null)
    {
        ...
        if (!empty($_DB_DATAOBJECT['CONFIG']['debug'])) {
            $this->debug(TABLE_PREFIX."{$this->__table} $row of {$this->N}", "fetchrow",3);
        }
        if (!$this->__table) {
        ...
        if (!empty($_DB_DATAOBJECT['CONFIG']['debug'])) {
            $this->debug(TABLE_PREFIX."{$this->__table} DONE", "fetchrow", 3);
        }
        return true;
    }
    ...
    function _build_condition($keys, $filter = array(),$negative_filter=array())
    {
        ...
        foreach($keys as $k => $v) {
            ...
            $kSql = $quoteIdentifiers
                ? ( $DB->quoteIdentifier(TABLE_PREFIX.$this->__table) . '.' . $DB->quoteIdentifier($k) )
                : TABLE_PREFIX."{$this->__table}.{$k}";
    ...
    function factory($table = '') {
        ...
        $p = isset($_DB_DATAOBJECT['CONFIG']['class_prefix']) ?
            $_DB_DATAOBJECT['CONFIG']['class_prefix'] : '';
        $class = $p . preg_replace('/[^A-Z0-9]/i','_',ucfirst(preg_replace('/^'.TABLE_PREFIX.'/i', '', $table)));
    ...
    */
    ...
    function joinAdd($obj = false, $joinType='INNER', $joinAs=false, $joinCol=false)
    {
        ...
        if ($joinAs === false) {
            $joinAs = TABLE_PREFIX.$obj->__table;
        }
        ...
        $objTable = $quoteIdentifiers ?
                $DB->quoteIdentifier(TABLE_PREFIX.$obj->__table) :
                TABLE_PREFIX.$obj->__table ;
        ...
        if ($quoteIdentifiers) {
            ...
            $table    = $DB->quoteIdentifier(TABLE_PREFIX.$table);
        ...
        $fullJoinAs = '';
        $addJoinAs  = ($quoteIdentifiers ? $DB->quoteIdentifier(TABLE_PREFIX.$obj->__table) : TABLE_PREFIX.$obj->__table) != $joinAs;
        if ($addJoinAs) {
            ...
            case 'RIGHT': // others??? .. cross, left outer, right outer, natural..?
                ...
                if (is_array($ofield)) {
                    ...
                    if ($table == substr($joinAs, 0, -1) || substr($table, 0, -1) == $joinAs) {
                        $this->_join .= " ON ({$table}.{$ofield}={$joinAs}.{$tfield}) {$appendJoin} ";
                    } else {
                        ...
                        if (preg_match('/(.*)[\:\.]{1}(.*)/i', $tfield, $matches)) {
                            ...
                            $this->_join .= " ON ({$joinAs}.{$ofield}={$_m1}.{$_m2}) {$appendJoin} ";
                        } else {
