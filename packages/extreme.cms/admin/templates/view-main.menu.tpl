<div class="view-main">
    <{plugin key="menu_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_MENU_NAME"}></label>
            </td>
    ...
    <{plugin key="menu_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        menuview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        menuview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                menuview_apply_block_visibility(key, value);
            });
        }

        menuview_apply_block_visibility = function (key, value) {
        ...
        menuview_update_visibility_settings = function () {
            var settings = menuview_get_visibility_settings();

            menuview_apply_visibility_settings(settings);
        }

        $(function() {
            menuview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/menu/fieldSave",
//                        data: td.find(':input').serialize() + "&menu_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
