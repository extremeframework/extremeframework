...
<{assign var='title' value=$smarty.const.L_MENU_ITEM}>
<{include file="top.tpl"}>

<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="menuitemquicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/menuitem/search" method="post" enctype="multipart/form-data">
            <input type="text" name="menuitem_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{$smarty.const.L_SEARCH}>" />
...
<form name="menuitemlist" id="menuitemlistform" class="form-list scope-list" action="<{$smarty.const.APPLICATION_URL}>/menuitem/" method="post">
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/menuitem/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{$smarty.const.L_NEW}> <{$smarty.const.L_MENU_ITEM|strtolower}>"/><{label key="L_NEW"}> <{$smarty.const.L_MENU_ITEM|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="menuitem_delete()"><span class="button-face"><{$smarty.const.L_DELETE}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.menuitem.import)}>
                                            <div class="btn button-general">
                            <a class="button-import scope-main" href="<{$smarty.const.APPLICATION_URL}>/menuitem/import/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-import.png" alt="<{$smarty.const.L_IMPORT}>" /><{$smarty.const.L_IMPORT}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.menuitem.export)}>
                                            <div class="btn button-general">
                            <a class="button-export" href="<{$smarty.const.APPLICATION_URL}>/menuitem/export/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-export.png" alt="<{$smarty.const.L_EXPORT}>" /><{$smarty.const.L_EXPORT}></span></a>
                        </div>
                                            <{/if}>
                        ...
                        <{if isset($smarty.session.acl.adminfilter.edit) }>
                            <div class="custom-filter">
                                ...
                                <span class="custom-filter-footer menuitem-custom-filter-footer hide">
                                    <hr>
                                    <ul>
                                    	<li><a class="scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminfilter/new/preset/MODULE/presetvalue/menuitem"><i class="fa fa-plus-circle"></i> <{label key="L_CREATE_NEW_FILTER"}></a></li>
                                    </ul>
                                </span>
            	        ...
            	        <ul class="dropdown-menu pull-right">
                            <{foreach from=$admin_list_actions item=item}>
                                <{if empty($item->ACL) || isset($smarty.session.acl.menuitem[$item->ACL])}>
                                    <li><a <{if $item->IS_OPEN_IN_NEW_WINDOW}>target="_blank"<{/if}> <{if $item->LINK}>href="<{framework_report_link format=$item->LINK}>"<{/if}> <{if $item->ACTION}>onclick="menuitem_<{$item->ACTION}>(); return false;"<{/if}>><span class="button-face"><{label text=$item->TITLE}></span></a></li>
                                <{/if}>
                            <{/foreach}>
                        </ul>
            	    </div>
...
<!-- Relations -->
    <{php}>
    	$template->assign('copyguidelines',  sprintf(L_GUIDELINES_COPY_RELS, strtolower(L_MENU_ITEM)));
    	$template->assign('approveguidelines', sprintf(L_GUIDELINES_APPROVE_RELS, strtolower(L_MENU_ITEM)));
    	$template->assign('deleteguidelines', sprintf(L_GUIDELINES_DELETE_RELS, strtolower(L_MENU_ITEM), strtolower(L_MENU_ITEM)));
    <{/php}>

    <div id="menuitemcopyrelations" style="display:none" title="<{$smarty.const.L_COPY}> <{$smarty.const.L_MENU_ITEM|strtolower}>">
        <p><{$copyguidelines}></p>
        <ul>
    ...
    <div id="menuitemapproverelations" style="display:none" title="<{$smarty.const.L_APPROVE}> <{$smarty.const.L_MENU_ITEM|strtolower}>">
    ...
    <div id="menuitemdeleterelations" style="display:none" title="<{$smarty.const.L_DELETE}> <{$smarty.const.L_MENU_ITEM|strtolower}>">
...
function menuitem_delete() {
	var dialog = $( "#menuitemdeleterelations" ).dialog({
		resizable: true,
		width: 500,
		modal: false,
		buttons: {
	...
	dialog.parent().appendTo($('#menuitemlistform'));
}

function menuitem_copy() {
	var dialog = $( "#menuitemcopyrelations" ).dialog({
		resizable: true,
		width: 500,
		modal: false,
		buttons: {
...
function menuitem_approve() {
	var dialog = $( "#menuitemapproverelations" ).dialog({
		resizable: true,
		width: 500,
		modal: false,
		buttons: {
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="menuitem-list-count list-count"><{$total}></span> <{label key="L_RECORDS"}>
	</div>
	<div style="clear:both"></div>
<{/if}>
