<div class="view-main">
    <{plugin key="posttype_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_POST_TYPE_NAME"}></label>
            </td>
    ...
    <{plugin key="posttype_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        posttypeview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        posttypeview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                posttypeview_apply_block_visibility(key, value);
            });
        }

        posttypeview_apply_block_visibility = function (key, value) {
        ...
        posttypeview_update_visibility_settings = function () {
            var settings = posttypeview_get_visibility_settings();

            posttypeview_apply_visibility_settings(settings);
        }

        $(function() {
            posttypeview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/posttype/fieldSave",
//                        data: td.find(':input').serialize() + "&posttype_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
