...
<{assign var='title' value=$smarty.const.L_PAGE_GALLERY}>
<{include file="top.tpl"}>

<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="pagegalleryquicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/pagegallery/search" method="post" enctype="multipart/form-data">
            <input type="text" name="pagegallery_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{$smarty.const.L_SEARCH}>" />
...
<{if isset($title) }>
    <h1 class="heading">
        ...
        <span style="margin-left:10px; font-size:12px; font-weight: normal" class="hidden-print">
            <a style="text-decoration: none" class="button-view-refresh scope-main" href="<{$smarty.const.APPLICATION_URL}>/pagegallery"><i class="fa fa-refresh"></i></a>
        </span>
    </h1>
<{/if}>
...
<form name="pagegallerylist" id="pagegallerylistform" class="form-list scope-list" action="<{$smarty.const.APPLICATION_URL}>/pagegallery/" method="post">
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/pagegallery/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{$smarty.const.L_NEW}> <{$smarty.const.L_PAGE_GALLERY|strtolower}>"/><{label key="L_NEW"}> <{$smarty.const.L_PAGE_GALLERY|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="pagegallery_delete()"><span class="button-face"><{$smarty.const.L_DELETE}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.pagegallery.import)}>
                                            <div class="btn button-general">
                            <a class="button-import scope-main" href="<{$smarty.const.APPLICATION_URL}>/pagegallery/import/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-import.png" alt="<{$smarty.const.L_IMPORT}>" /><{$smarty.const.L_IMPORT}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.pagegallery.export)}>
                                            <div class="btn button-general">
                            <a class="button-export" href="<{$smarty.const.APPLICATION_URL}>/pagegallery/export/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-export.png" alt="<{$smarty.const.L_EXPORT}>" /><{$smarty.const.L_EXPORT}></span></a>
                        </div>
                                            <{/if}>
            	        ...
            	        <ul class="dropdown-menu pull-right">
                            <{foreach from=$admin_list_actions item=item}>
                                <{if empty($item->ACL) || isset($smarty.session.acl.pagegallery[$item->ACL])}>
                                    <li><a <{if $item->IS_OPEN_IN_NEW_WINDOW}>target="_blank"<{/if}> <{if $item->LINK}>href="<{framework_report_link format=$item->LINK}>"<{/if}> <{if $item->ACTION}>onclick="pagegallery_<{$item->ACTION}>(); return false;"<{/if}>><span class="button-face"><{label text=$item->TITLE}></span></a></li>
                                <{/if}>
                            <{/foreach}>
                        </ul>
            	    </div>
...
function pagegallery_search() {
	$('#pagegallerylistform').attr('action', '<{$smarty.const.APPLICATION_URL}>/pagegallery/search/');
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="pagegallery-list-count list-count"><{$total}></span> <{label key="L_RECORDS"}>
	</div>
	<div style="clear:both"></div>
<{/if}>
