...
/**
 * Usage: <{module class="WidgetListWidgetPosition" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.widgetposition.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListWidgetPosition extends WidgetPositionController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.widgetposition.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('widgetposition')) {
            $vars = array_keys(get_object_vars(new WidgetPositionModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('widgetposition', $preset, $presetvalue));
