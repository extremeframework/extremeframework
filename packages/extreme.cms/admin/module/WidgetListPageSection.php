...
/**
 * Usage: <{module class="WidgetListPageSection" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.pagesection.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPageSection extends PageSectionController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.pagesection.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('pagesection')) {
            $vars = array_keys(get_object_vars(new PageSectionModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('pagesection', $preset, $presetvalue));
