...
/**
 * Usage: <{module class="WidgetListMenu" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.menu.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListMenu extends MenuController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.menu.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('menu')) {
            $vars = array_keys(get_object_vars(new MenuModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('menu', $preset, $presetvalue));
