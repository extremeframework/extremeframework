...
/**
 * Usage: <{module class="WidgetListPostType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.posttype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPostType extends PostTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.posttype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('posttype')) {
            $vars = array_keys(get_object_vars(new PostTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('posttype', $preset, $presetvalue));
