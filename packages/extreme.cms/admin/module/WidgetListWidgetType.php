...
/**
 * Usage: <{module class="WidgetListWidgetType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.widgettype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListWidgetType extends WidgetTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.widgettype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('widgettype')) {
            $vars = array_keys(get_object_vars(new WidgetTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('widgettype', $preset, $presetvalue));
