...
/**
 * Usage: <{module class="WidgetListPostCategory" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.postcategory.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPostCategory extends PostCategoryController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.postcategory.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('postcategory')) {
            $vars = array_keys(get_object_vars(new PostCategoryModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postcategory', $preset, $presetvalue));
