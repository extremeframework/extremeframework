...
/**
 * Usage: <{module class="WidgetListPostRelation" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.postrelation.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPostRelation extends PostRelationController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.postrelation.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('postrelation')) {
            $vars = array_keys(get_object_vars(new PostRelationModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postrelation', $preset, $presetvalue));
