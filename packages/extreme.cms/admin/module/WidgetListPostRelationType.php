...
/**
 * Usage: <{module class="WidgetListPostRelationType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.postrelationtype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPostRelationType extends PostRelationTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.postrelationtype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('postrelationtype')) {
            $vars = array_keys(get_object_vars(new PostRelationTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postrelationtype', $preset, $presetvalue));
