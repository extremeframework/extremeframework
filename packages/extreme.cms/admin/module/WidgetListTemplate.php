...
/**
 * Usage: <{module class="WidgetListTemplate" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.template.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListTemplate extends TemplateController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.template.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('template')) {
            $vars = array_keys(get_object_vars(new TemplateModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('template', $preset, $presetvalue));
