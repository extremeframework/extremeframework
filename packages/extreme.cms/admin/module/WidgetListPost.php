...
/**
 * Usage: <{module class="WidgetListPost" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.post.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPost extends PostController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.post.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('post')) {
            $vars = array_keys(get_object_vars(new PostModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('post', $preset, $presetvalue));
