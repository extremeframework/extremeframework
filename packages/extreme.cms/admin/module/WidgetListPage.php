...
/**
 * Usage: <{module class="WidgetListPage" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.page.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPage extends PageController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.page.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('page')) {
            $vars = array_keys(get_object_vars(new PageModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('page', $preset, $presetvalue));
