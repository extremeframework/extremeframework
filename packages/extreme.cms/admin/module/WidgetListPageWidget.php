...
/**
 * Usage: <{module class="WidgetListPageWidget" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.pagewidget.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPageWidget extends PageWidgetController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.pagewidget.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('pagewidget')) {
            $vars = array_keys(get_object_vars(new PageWidgetModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('pagewidget', $preset, $presetvalue));
