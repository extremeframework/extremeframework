...
/**
 * Usage: <{module class="WidgetListPostSection" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.postsection.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPostSection extends PostSectionController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.postsection.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('postsection')) {
            $vars = array_keys(get_object_vars(new PostSectionModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postsection', $preset, $presetvalue));
