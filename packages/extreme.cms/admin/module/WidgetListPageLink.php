...
/**
 * Usage: <{module class="WidgetListPageLink" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.pagelink.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPageLink extends PageLinkController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.pagelink.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('pagelink')) {
            $vars = array_keys(get_object_vars(new PageLinkModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('pagelink', $preset, $presetvalue));
