...
/**
 * Usage: <{module class="WidgetListPageLinkType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.pagelinktype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPageLinkType extends PageLinkTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.pagelinktype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('pagelinktype')) {
            $vars = array_keys(get_object_vars(new PageLinkTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('pagelinktype', $preset, $presetvalue));
