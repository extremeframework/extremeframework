...
/**
 * Usage: <{module class="WidgetListPageGallery" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.pagegallery.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPageGallery extends PageGalleryController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.pagegallery.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('pagegallery')) {
            $vars = array_keys(get_object_vars(new PageGalleryModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('pagegallery', $preset, $presetvalue));
