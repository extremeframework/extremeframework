...
/**
 * Usage: <{module class="WidgetListPostGallery" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.postgallery.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPostGallery extends PostGalleryController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.postgallery.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('postgallery')) {
            $vars = array_keys(get_object_vars(new PostGalleryModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postgallery', $preset, $presetvalue));
