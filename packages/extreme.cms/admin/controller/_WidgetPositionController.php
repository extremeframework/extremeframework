...
class _WidgetPositionController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_WIDGET_POSITION_NAME);
           return false;
       }
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."WIDGET_POSITION.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
