...
class _PostSectionController extends __AppController
{
    ...
    public function __construct() {
        ...
        PluginManager::do_action('postsection_init');
    }
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('postsection', $model, $errors)) {
            return false;
        }
    ...
    private function handleFileUploads(&$model) {
	    if (isset($_FILES) && !empty($_FILES)) {
            foreach ($_FILES as $column => $file) {
                if (preg_match('/^postsection_formdata_(.*)/', $column, $matches)) {
                    $column = $matches[1];
                }
                ...
                if (!empty($file['name'])) {
                    ...
                    $rel = 'postsection/'.strtolower(str_replace('_', '-', $column));
                    ...
                    if (in_array(strtolower($extension), array('.gif', '.jpg', '.jpeg', '.png'))) {
                        ...
                        if (!($width > 0 || $height > 0) && AdminImageConfigHelper::hasImageConfig('postsection', $column)) {
                            $config = AdminImageConfigHelper::getImageConfig('postsection', $column);
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^postsection_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^postsection_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^postsection_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('postsection');

        AclController::checkPermission('postsection', 'list');

		ContextStack::register(APPLICATION_URL.'/postsection/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."POST_SECTION.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('postsection_before_delete', $_model);
        }

		$model = new PostSectionModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('postsection', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."POST_SECTION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('postsection', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['postsectionlist_selection_selectall']) && $_REQUEST['postsectionlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['postsection.list.model']) && is_object($_SESSION['postsection.list.model'])) {
					$model = clone $_SESSION['postsection.list.model'];
				} else {
		        	$model = new PostSectionModel();
				}
    			...
    			$selection = isset($_REQUEST['postsectionlist_selection'])? $_REQUEST['postsectionlist_selection'] : '';
			}
        }
        ...
        if ($module != 'postsection' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('postsection', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PostSectionModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('postsection', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/postsection/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/postsection/edit/'.$id);
		$this->_edit($id);
	}

    protected function onSaveSuccess($model) {
        PostController::sync_JSON_POST_SECTIONS($model->ID_POST);
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'postsection_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'postsection_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('postsection');
        $customfieldvalues = array();

        $columns2edit = array('UUID', 'TITLE', 'ID_POST', 'TAG_LINE', 'IMAGE', 'BACKGROUND_IMAGE', 'BACKGROUND_COLOR', 'VIDEO', 'CONTENT', 'LINK_PATH', 'ORDERING');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'postsection_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'TITLE', 'ID_POST', 'TAG_LINE', 'IMAGE', 'BACKGROUND_IMAGE', 'BACKGROUND_COLOR', 'VIDEO', 'CONTENT', 'LINK_PATH', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('postsection'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'postsection_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'postsection_multiformdata_deleteditems'])? $_REQUEST[$prefix.'postsection_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('postsection_before_save', $model);

    		if ($model->UUID) {
				
				$old = new PostSectionModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('postsection_before_update', $model, $old);

        		if (!AclController::hasPermission('postsection', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."POST_SECTION.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."POST_SECTION.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."POST_SECTION.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('postsection_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('postsection', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.postsection.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('postsection', 'import');

        if (isset($_FILES['postsection_importfile'])) {
        	if (!$_FILES['postsection_importfile']['error']) {
	            $filepath = $_FILES['postsection_importfile']['tmp_name'];
        		$original = $_FILES['postsection_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/postsection/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('TITLE','ID_POST','TAG_LINE','IMAGE','BACKGROUND_IMAGE','BACKGROUND_COLOR','VIDEO','CONTENT','LINK_PATH','ORDERING');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['postsection.list.model']) && is_object($_SESSION['postsection.list.model'])) {
			$model = clone $_SESSION['postsection.list.model'];
		} else {
        	$model = new PostSectionModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."POST_SECTION.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'postsection_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/postsection_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/postsection_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('TITLE','ID_POST','TAG_LINE','IMAGE','BACKGROUND_IMAGE','BACKGROUND_COLOR','VIDEO','CONTENT','LINK_PATH','ORDERING');
        $filename = 'postsection_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PostSection');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.postsection.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.postsection.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.postsection.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.postsection.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.postsection.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.postsection.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.postsection.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.postsection.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['postsection.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['postsection.customview'])? $_SESSION['postsection.customview'] : '';
    ...
    private function _list() {
        $filtercolumns = $this->getCustomFilterColumns('postsection', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('postsection', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('postsection');
        ...
        $_SESSION['postsection.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postsection'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('postsection', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('postsection', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('postsection', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.postsection.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.postsection.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('postsection');

        $aclviewablecolumns = AclController::getAclEnabledColumns('postsection', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('postsection', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('postsection', 'new');
        }
        ...
        $ids = isset($_SESSION['postsection.list.ids'])? $_SESSION['postsection.list.ids'] : array();
        ...
        PluginManager::do_action('postsection_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('postsection', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('postsection', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('postsection', 'view'));
        ...
        PluginManager::do_action('postsection_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.postsection.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('postsection');
        ...
        $roweditablecolumns = array('TITLE', 'ID_POST', 'BACKGROUND_COLOR', 'LINK_PATH', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('postsection', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('postsection', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('postsection_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('postsection', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('postsection'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('postsection'));

	    $this->display($smarty, $templatecode);
	}

    private function getLayoutColumns() {
        return array('TITLE', 'ID_POST', 'TAG_LINE', 'IMAGE', 'BACKGROUND_IMAGE', 'BACKGROUND_COLOR', 'VIDEO', 'CONTENT', 'LINK_PATH');
    }
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    PostSectionController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['postsection.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'TITLE':
                        $model->whereAdd(TABLE_PREFIX."POST_SECTION.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'ID_POST':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."POST_SECTION.ID_POST IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."POST_SECTION.ID_POST = '$value'");
                        }

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."POST_SECTION.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."POST_SECTION.$key IS NULL OR ".TABLE_PREFIX.".POST_SECTION.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."POST_SECTION.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."POST_SECTION.ID_POST IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."POST_SECTION.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_SECTION.TITLE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_SECTION.TAG_LINE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_SECTION.CONTENT LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_SECTION.LINK_PATH LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_SECTION.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    private function _import($templatecode = 'import.postsection.tpl') {
    ...
    private function _importxls($filepath, &$error) {
        ...
        $columns2import = array('TITLE','ID_POST','TAG_LINE','IMAGE','BACKGROUND_IMAGE','BACKGROUND_COLOR','VIDEO','CONTENT','LINK_PATH','ORDERING');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('postsection_before_import', $model, $rawdata);
