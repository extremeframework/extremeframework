...
class _PostController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ID_POST_TYPE', $columns2check) && trim($model->ID_POST_TYPE) == '') {
           $errors['id-post-type'] = sprintf(L_VALIDATION_NOT_EMPTY, L_POST_TYPE);
           return false;
       }
       ...
       if (in_array('EXTERNAL_LINK', $columns2check) && !empty($model->EXTERNAL_LINK) && !preg_match('/^(http|https|ftp):\/\/(([A-Z0-9][A-Z0-9_-]*)(\.?[A-Z0-9][A-Z0-9_-]*)+)(:(\d+))?/i', $model->EXTERNAL_LINK)) {
           $errors['external-link'] = sprintf(L_VALIDATION_URL, L_EXTERNAL_LINK);
           return false;
       }
       if (in_array('NBR_VIEWS', $columns2check) && !empty($model->NBR_VIEWS) && !is_numeric($model->NBR_VIEWS)) {
           $errors['nbr-views'] = sprintf(L_VALIDATION_NUMBER, L_NBR_VIEWS);
           return false;
       }
    ...
    private function _view($id, $details = null, $templatecode = 'view.post.tpl') {
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.post.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('post');
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."POST.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
    ...
    private function _import($templatecode = 'import.post.tpl') {
