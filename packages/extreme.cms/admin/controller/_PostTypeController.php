...
class _PostTypeController extends __AppController
{
    var $module = 'posttype';
    var $type = 'controller';
    ...
    public function __construct() {
        ...
        PluginManager::do_action('posttype_init');
    }

    protected function checkConstraint($model, &$errors, $columns2check) {
        
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(_t('L_VALIDATION_NOT_EMPTY'), _t('L_POST_TYPE_NAME'));
           return false;
       }
       if (in_array('CODE', $columns2check) && trim($model->CODE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('posttype', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^posttype_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^posttype_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^posttype_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('posttype');

        AclController::checkPermission('posttype', 'list');

		ContextStack::register(APPLICATION_URL.'/posttype/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."POST_TYPE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('posttype_before_delete', $_model);
        }

		$model = new PostTypeModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('posttype', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."POST_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('posttype', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['posttypelist_selection_selectall']) && $_REQUEST['posttypelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['posttype.list.model']) && is_object($_SESSION['posttype.list.model'])) {
					$model = clone $_SESSION['posttype.list.model'];
				} else {
		        	$model = new PostTypeModel();
				}
    			...
    			$selection = isset($_REQUEST['posttypelist_selection'])? $_REQUEST['posttypelist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'post': 
                            (new PostController())->delete('ID_POST_TYPE', $_ids);
                            break;
        ...
        if ($module != 'posttype' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('posttype', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PostTypeModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('posttype', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/posttype/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/posttype/edit/'.$id);
		$this->_edit($id);
	}
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'posttype_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'posttype_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('posttype');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'posttype_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'CODE');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('posttype'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'posttype_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
				...
				$old = new PostTypeModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('posttype_before_update', $model, $old);

        		if (!AclController::hasPermission('posttype', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."POST_TYPE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."POST_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."POST_TYPE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('posttype_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('posttype', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.posttype.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('posttype', 'import');

        if (isset($_FILES['posttype_importfile'])) {
        	if (!$_FILES['posttype_importfile']['error']) {
	            $filepath = $_FILES['posttype_importfile']['tmp_name'];
        		$original = $_FILES['posttype_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/posttype/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['posttype.list.model']) && is_object($_SESSION['posttype.list.model'])) {
			$model = clone $_SESSION['posttype.list.model'];
		} else {
        	$model = new PostTypeModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."POST_TYPE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'posttype_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/posttype_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/posttype_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE');
        $filename = 'posttype_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PostType');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.posttype.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.posttype.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.posttype.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.posttype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.posttype.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.posttype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.posttype.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.posttype.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['posttype.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['posttype.customview'])? $_SESSION['posttype.customview'] : '';
    ...
    protected function _list() {
        $filtercolumns = $this->getCustomFilterColumns('posttype', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('posttype', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('posttype');
        ...
        $_SESSION['posttype.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('posttype'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('posttype', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('posttype', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('posttype', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.posttype.tpl');
    }

    protected function _view($id, $details = null, $templatecode = 'view.posttype.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('posttype');

        $aclviewablecolumns = AclController::getAclEnabledColumns('posttype', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('posttype', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('posttype', 'new');
        }
        ...
        $ids = isset($_SESSION['posttype.list.ids'])? $_SESSION['posttype.list.ids'] : array();
        ...
        PluginManager::do_action('posttype_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('posttype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('posttype', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('posttype', 'view'));
        ...
        PluginManager::do_action('posttype_viewed', $details);
	}

    protected function _edit($id, $details = null, $templatecode = 'edit.posttype.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('posttype');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('posttype', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('posttype_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('posttype', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('posttype_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('posttype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('posttype'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('posttype'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    PostTypeController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['posttype.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."POST_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."POST_TYPE.$key IS NULL OR ".TABLE_PREFIX.".POST_TYPE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."POST_TYPE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."POST_TYPE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_TYPE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_TYPE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    protected function _import($templatecode = 'import.posttype.tpl') {
    ...
    protected function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('posttype_before_import', $model, $rawdata);
