...
class _MenuController extends __AppController
{
    var $module = 'menu';
    var $type = 'controller';
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_MENU_NAME);
           return false;
       }
       if (in_array('CODE', $columns2check) && trim($model->CODE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('menu', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^menu_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^menu_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^menu_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('menu');

        AclController::checkPermission('menu', 'list');

		ContextStack::register(APPLICATION_URL.'/menu/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."MENU.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('menu_before_delete', $_model);
        }

		$model = new MenuModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('menu', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('menu', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['menulist_selection_selectall']) && $_REQUEST['menulist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['menu.list.model']) && is_object($_SESSION['menu.list.model'])) {
					$model = clone $_SESSION['menu.list.model'];
				} else {
		        	$model = new MenuModel();
				}
    			...
    			$selection = isset($_REQUEST['menulist_selection'])? $_REQUEST['menulist_selection'] : '';
			}
        }
        ...
        if ($module != 'menu' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('menu', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new MenuModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('menu', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/menu/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/menu/edit/'.$id);
		$this->_edit($id);
	}
    ...
    static function sync_JSON_MENU_ITEMS($id) {
        ...
        MenuController::update_json($id, 'MENU_ITEM', $items);
    }
    ...
    private function getTempCreateId() {
        ...
        if (!isset($_SESSION['menu.tmpid'])) {
            ...
            $model->UUID = $_SESSION['menu.tmpid'];
        ...
        if ($create) {
            ...
            $_SESSION['menu.tmpid'] = $model->UUID;
        }
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'menu_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'menu_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('menu');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'menu_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'menu_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'menu_multiformdata_deleteditems'])? $_REQUEST[$prefix.'menu_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('menu_before_save', $model);
    		...
    		if ($model->UUID) {
                ...
                PluginManager::do_action('menu_before_update', $model, $old);

        		if (!AclController::hasPermission('menu', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."MENU.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."MENU.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."MENU.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('menu_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('menu', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.menu.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
		    ...
		    ContextStack::replace(APPLICATION_URL.'/menu/view/'.$model->UUID);
    ...
    public function importAction() {
        ...
        AclController::checkPermission('menu', 'import');

        if (isset($_FILES['menu_importfile'])) {
        	if (!$_FILES['menu_importfile']['error']) {
	            $filepath = $_FILES['menu_importfile']['tmp_name'];
        		$original = $_FILES['menu_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/menu/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('NAME','CODE','CLASS');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['menu.list.model']) && is_object($_SESSION['menu.list.model'])) {
			$model = clone $_SESSION['menu.list.model'];
		} else {
        	$model = new MenuModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."MENU.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/menu_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/menu_'.date('Ymd').'.zip';
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.menu.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.menu.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.menu.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.menu.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.menu.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.menu.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.menu.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.menu.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['menu.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['menu.customview'])? $_SESSION['menu.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('menu');
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('menu'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('menu', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('menu', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('menu', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.menu.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('menu', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('menu', 'new');
        }
        ...
        $ids = isset($_SESSION['menu.list.ids'])? $_SESSION['menu.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('menu', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('menu', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('menu', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.menu.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('NAME', 'CODE', 'CLASS');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('menu', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('menu_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('menu', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('menu', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('menu'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('menu'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    MenuController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['menu.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."MENU.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."MENU.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."MENU.$key IS NULL OR ".TABLE_PREFIX.".MENU.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."MENU.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."MENU.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."MENU.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."MENU.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."MENU.CLASS LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."MENU.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
    ...
    private function _importxls($filepath, &$error) {
        ...
        $columns2import = array('NAME','CODE','CLASS');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('menu_before_import', $model, $rawdata);
