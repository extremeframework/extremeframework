...
class _TemplateController extends __AppController
{
    var $module = 'template';
    var $type = 'controller';
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_TEMPLATE_NAME);
           return false;
       }
       ...
       if (in_array('MODULE', $columns2check) && trim($model->MODULE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('template', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^template_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^template_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^template_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('template');

        AclController::checkPermission('template', 'list');

		ContextStack::register(APPLICATION_URL.'/template/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."TEMPLATE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('template_before_delete', $_model);
        }

		$model = new TemplateModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('template', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."TEMPLATE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('template', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['templatelist_selection_selectall']) && $_REQUEST['templatelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['template.list.model']) && is_object($_SESSION['template.list.model'])) {
					$model = clone $_SESSION['template.list.model'];
				} else {
		        	$model = new TemplateModel();
				}
    			...
    			$selection = isset($_REQUEST['templatelist_selection'])? $_REQUEST['templatelist_selection'] : '';
			}
        }
        ...
        if ($module != 'template' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('template', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new TemplateModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('template', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/template/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/template/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'template_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'template_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('template');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'template_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'template_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'template_multiformdata_deleteditems'])? $_REQUEST[$prefix.'template_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('template_before_save', $model);

    		if ($model->UUID) {
				
				$old = new TemplateModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('template_before_update', $model, $old);

        		if (!AclController::hasPermission('template', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."TEMPLATE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."TEMPLATE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."TEMPLATE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('template_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('template', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.template.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('template', 'import');

        if (isset($_FILES['template_importfile'])) {
        	if (!$_FILES['template_importfile']['error']) {
	            $filepath = $_FILES['template_importfile']['tmp_name'];
        		$original = $_FILES['template_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/template/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('NAME','CODE','MODULE');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['template.list.model']) && is_object($_SESSION['template.list.model'])) {
			$model = clone $_SESSION['template.list.model'];
		} else {
        	$model = new TemplateModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."TEMPLATE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/template_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/template_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','MODULE');
        $filename = 'template_'.date('Ymd').'.xls';
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.template.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.template.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.template.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.template.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.template.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.template.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.template.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.template.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['template.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['template.customview'])? $_SESSION['template.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('template');
        ...
        $_SESSION['template.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('template'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('template', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('template', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('template', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.template.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('template', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('template', 'new');
        }
        ...
        $ids = isset($_SESSION['template.list.ids'])? $_SESSION['template.list.ids'] : array();
        ...
        PluginManager::do_action('template_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('template', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('template', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('template', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.template.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('NAME', 'CODE', 'MODULE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('template', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('template_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('template', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('template_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('template', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('template'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('template'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    TemplateController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['template.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."TEMPLATE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");

                        break;

                    case 'MODULE':
                        $model->whereAdd(TABLE_PREFIX."TEMPLATE.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."TEMPLATE.$key IS NULL OR ".TABLE_PREFIX.".TEMPLATE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."TEMPLATE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'TEMPLATE`.NAME, `'.TABLE_PREFIX.'TEMPLATE`.CODE, `'.TABLE_PREFIX.'TEMPLATE`.MODULE, `'.TABLE_PREFIX.'TEMPLATE`.ID, `'.TABLE_PREFIX.'TEMPLATE`.JSON, `'.TABLE_PREFIX.'TEMPLATE`.UUID, `'.TABLE_PREFIX.'TEMPLATE`.WFID');
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."TEMPLATE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."TEMPLATE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."TEMPLATE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."TEMPLATE.MODULE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."TEMPLATE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
    ...
    private function _importxls($filepath, &$error) {
        ...
        $columns2import = array('NAME','CODE','MODULE');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('template_before_import', $model, $rawdata);
