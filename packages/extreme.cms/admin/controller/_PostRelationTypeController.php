...
class _PostRelationTypeController extends __AppController
{
    var $module = 'postrelationtype';
    var $type = 'controller';

    public function __construct() {
        parent::__construct();

        PluginManager::do_action('postrelationtype_init');
    }

    private function checkConstraint($model, &$errors, $columns2check) {
        
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(_t('L_VALIDATION_NOT_EMPTY'), _t('L_POST_RELATION_TYPE_NAME'));
           return false;
       }
       if (in_array('CODE', $columns2check) && trim($model->CODE) == '') {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('postrelationtype', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^postrelationtype_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^postrelationtype_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^postrelationtype_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('postrelationtype');

        AclController::checkPermission('postrelationtype', 'list');

		ContextStack::register(APPLICATION_URL.'/postrelationtype/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."POST_RELATION_TYPE.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'postrelationtype');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new PostRelationTypeModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('postrelationtype_before_delete', $_model);
        }

		$model = new PostRelationTypeModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('postrelationtype', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('postrelationtype_deleted', $_model);
        }

        NotificationHelper::notifyChange('postrelationtype', 'delete');
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('postrelationtype', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['postrelationtypelist_selection_selectall']) && $_REQUEST['postrelationtypelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['postrelationtype.list.model']) && is_object($_SESSION['postrelationtype.list.model'])) {
					$model = clone $_SESSION['postrelationtype.list.model'];
				} else {
		        	$model = new PostRelationTypeModel();
				}
    			...
    			$selection = isset($_REQUEST['postrelationtypelist_selection'])? $_REQUEST['postrelationtypelist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'postrelation': 
                            (new PostRelationController())->delete('ID_POST_RELATION_TYPE', $_ids);
                            break;
        ...
        if ($module != 'postrelationtype' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('postrelationtype', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PostRelationTypeModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('postrelationtype', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/postrelationtype/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/postrelationtype/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'postrelationtype_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'postrelationtype_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('postrelationtype');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'postrelationtype_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'CODE');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('postrelationtype'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'postrelationtype_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'postrelationtype_multiformdata_deleteditems'])? $_REQUEST[$prefix.'postrelationtype_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('postrelationtype_before_save', $model);

    		if ($model->UUID) {
				
				$old = new PostRelationTypeModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('postrelationtype_before_update', $model, $old);

        		if (!AclController::hasPermission('postrelationtype', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."POST_RELATION_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('postrelationtype_updated', $model, $old);
                NotificationHelper::notifyChange('postrelationtype', 'update');
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('postrelationtype_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('postrelationtype_created', $model);
    		    NotificationHelper::notifyChange('postrelationtype', 'insert');
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('postrelationtype');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('postrelationtype', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.postrelationtype.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('postrelationtype', 'import');

        if (isset($_FILES['postrelationtype_importfile'])) {
        	if (!$_FILES['postrelationtype_importfile']['error']) {
	            $filepath = $_FILES['postrelationtype_importfile']['tmp_name'];
        		$original = $_FILES['postrelationtype_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/postrelationtype/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['postrelationtype.list.model']) && is_object($_SESSION['postrelationtype.list.model'])) {
			$model = clone $_SESSION['postrelationtype.list.model'];
		} else {
        	$model = new PostRelationTypeModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'postrelationtype_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/postrelationtype_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/postrelationtype_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE');
        $filename = 'postrelationtype_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PostRelationType');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.postrelationtype.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.postrelationtype.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.postrelationtype.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.postrelationtype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.postrelationtype.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.postrelationtype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.postrelationtype.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.postrelationtype.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['postrelationtype.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['postrelationtype.customview'])? $_SESSION['postrelationtype.customview'] : '';
    ...
    private function _list() {
        $filtercolumns = $this->getCustomFilterColumns('postrelationtype', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('postrelationtype', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('postrelationtype');
        ...
        $_SESSION['postrelationtype.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('postrelationtype'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('postrelationtype', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('postrelationtype', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('postrelationtype', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.postrelationtype.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.postrelationtype.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('postrelationtype');

        $aclviewablecolumns = AclController::getAclEnabledColumns('postrelationtype', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('postrelationtype', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('postrelationtype', 'new');
        }
        ...
        $ids = isset($_SESSION['postrelationtype.list.ids'])? $_SESSION['postrelationtype.list.ids'] : array();
        ...
        PluginManager::do_action('postrelationtype_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('postrelationtype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('postrelationtype', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('postrelationtype', 'view'));
        ...
        PluginManager::do_action('postrelationtype_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.postrelationtype.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('postrelationtype');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('postrelationtype', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('postrelationtype_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('postrelationtype', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('postrelationtype_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('postrelationtype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('postrelationtype'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('postrelationtype'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    PostRelationTypeController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['postrelationtype.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.$key IS NULL OR ".TABLE_PREFIX.".POST_RELATION_TYPE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."POST_RELATION_TYPE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."POST_RELATION_TYPE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_RELATION_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_RELATION_TYPE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."POST_RELATION_TYPE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    private function _import($templatecode = 'import.postrelationtype.tpl') {
    ...
    private function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('postrelationtype_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('postrelationtype_imported', $model);
            NotificationHelper::notifyChange('postrelationtype', 'insert');
		}
