...
class _MenuItemController extends __AppController
{
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        NotificationHelper::notifyChange('menuitem', 'delete');
    }
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
                ...
                NotificationHelper::notifyChange('menuitem', 'update');
            } else {
                $model->ID = null;
    		    ...
    		    NotificationHelper::notifyChange('menuitem', 'insert');
            }
    ...
    private function _view($id, $details = null, $templatecode = 'view.menuitem.tpl') {
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.menuitem.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('menuitem');
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('menuitem', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."MENU_ITEM.UDID = 0 OR ".TABLE_PREFIX."MENU_ITEM.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    private function _import($templatecode = 'import.menuitem.tpl') {
