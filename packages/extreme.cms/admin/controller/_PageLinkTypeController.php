...
class _PageLinkTypeController extends __AppController
{
    var $module = 'pagelinktype';
    var $type = 'controller';

    public function __construct() {
        parent::__construct();

        PluginManager::do_action('pagelinktype_init');
    }
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('pagelinktype', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^pagelinktype_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^pagelinktype_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^pagelinktype_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('pagelinktype');

        AclController::checkPermission('pagelinktype', 'list');

		ContextStack::register(APPLICATION_URL.'/pagelinktype/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."PAGE_LINK_TYPE.$field $operator '$value')";
                }
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$smarty->assign('module', 'pagelinktype');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('pagelinktype_before_delete', $_model);
        }

		$model = new PageLinkTypeModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('pagelinktype', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('pagelinktype_deleted', $_model);
        }

        NotificationHelper::notifyChange('pagelinktype', 'delete');
    }
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('pagelinktype', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['pagelinktypelist_selection_selectall']) && $_REQUEST['pagelinktypelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['pagelinktype.list.model']) && is_object($_SESSION['pagelinktype.list.model'])) {
					$model = clone $_SESSION['pagelinktype.list.model'];
				} else {
		        	$model = new PageLinkTypeModel();
				}
    			...
    			$selection = isset($_REQUEST['pagelinktypelist_selection'])? $_REQUEST['pagelinktypelist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'pagelink': 
                            (new PageLinkController())->delete('ID_PAGE_LINK_TYPE', $_ids);
                            break;
        ...
        if ($module != 'pagelinktype' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('pagelinktype', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PageLinkTypeModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('pagelinktype', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/pagelinktype/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/pagelinktype/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'pagelinktype_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'pagelinktype_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('pagelinktype');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'pagelinktype_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        $columns2edit = array('UUID', 'NAME', 'CODE', 'ORDERING');
        $columns2edit = array_merge($columns2edit, CustomFieldHelper::getCustomFieldColumns('pagelinktype'));

        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'pagelinktype_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
				...
				$old = new PageLinkTypeModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('pagelinktype_before_update', $model, $old);

        		if (!AclController::hasPermission('pagelinktype', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."PAGE_LINK_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
    		    ...
    		    PluginManager::do_action('pagelinktype_updated', $model, $old);
                NotificationHelper::notifyChange('pagelinktype', 'update');
            } else {
                $model->ID = null;
                
                $this->onBeforeInsert($model);
                PluginManager::do_action('pagelinktype_before_create', $model);

    		    $model->insert();
    		    $model->_isnew = true;

    		    $this->onInsertSuccess($model);
    		    PluginManager::do_action('pagelinktype_created', $model);
    		    NotificationHelper::notifyChange('pagelinktype', 'insert');
            }

            $this->onSaveSuccess($model);
        }

        DraftHelper::clearAllDrafts('pagelinktype');
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('pagelinktype', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.pagelinktype.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('pagelinktype', 'import');

        if (isset($_FILES['pagelinktype_importfile'])) {
        	if (!$_FILES['pagelinktype_importfile']['error']) {
	            $filepath = $_FILES['pagelinktype_importfile']['tmp_name'];
        		$original = $_FILES['pagelinktype_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/pagelinktype/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['pagelinktype.list.model']) && is_object($_SESSION['pagelinktype.list.model'])) {
			$model = clone $_SESSION['pagelinktype.list.model'];
		} else {
        	$model = new PageLinkTypeModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'pagelinktype_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/pagelinktype_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/pagelinktype_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','ORDERING');
        $filename = 'pagelinktype_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PageLinkType');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.pagelinktype.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.pagelinktype.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.pagelinktype.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.pagelinktype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.pagelinktype.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.pagelinktype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.pagelinktype.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.pagelinktype.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['pagelinktype.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['pagelinktype.customview'])? $_SESSION['pagelinktype.customview'] : '';
    ...
    private function _list() {
        $filtercolumns = $this->getCustomFilterColumns('pagelinktype', $filter);

        $aclviewablecolumns = AclController::getAclEnabledColumns('pagelinktype', 'view');

        if (empty($aclviewablecolumns) || (!isset($aclviewablecolumns['*']) && !in_array(true, $aclviewablecolumns))) {
            $this->pagenotfound('No view available');
        }

        $excludedcolumns = AclController::getSystemExcludedColumns('pagelinktype');
        ...
        $_SESSION['pagelinktype.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('pagelinktype'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('pagelinktype', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('pagelinktype', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('pagelinktype', 'view'));
		

        $templatetype = !empty($this->templatetype)? $this->templatetype : 'list';
	    $this->display($smarty, $templatetype.'.pagelinktype.tpl');
    }

    private function _view($id, $details = null, $templatecode = 'view.pagelinktype.tpl') {
        $filtercolumns = $this->getCustomFilterColumns('pagelinktype');

        $aclviewablecolumns = AclController::getAclEnabledColumns('pagelinktype', 'view');

        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('pagelinktype', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('pagelinktype', 'new');
        }
        ...
        $ids = isset($_SESSION['pagelinktype.list.ids'])? $_SESSION['pagelinktype.list.ids'] : array();
        ...
        PluginManager::do_action('pagelinktype_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('pagelinktype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('pagelinktype', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('pagelinktype', 'view'));
        ...
        PluginManager::do_action('pagelinktype_viewed', $details);
	}

    private function _edit($id, $details = null, $templatecode = 'edit.pagelinktype.tpl', $restoredraft = true) {
        $filtercolumns = $this->getCustomFilterColumns('pagelinktype');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('pagelinktype', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('pagelinktype_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('pagelinktype', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('pagelinktype_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('pagelinktype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('pagelinktype'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('pagelinktype'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    PageLinkTypeController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['pagelinktype.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.$key IS NULL OR ".TABLE_PREFIX.".PAGE_LINK_TYPE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."PAGE_LINK_TYPE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'PAGE_LINK_TYPE`.NAME, `'.TABLE_PREFIX.'PAGE_LINK_TYPE`.CODE, `'.TABLE_PREFIX.'PAGE_LINK_TYPE`.ID, `'.TABLE_PREFIX.'PAGE_LINK_TYPE`.JSON, `'.TABLE_PREFIX.'PAGE_LINK_TYPE`.UUID, `'.TABLE_PREFIX.'PAGE_LINK_TYPE`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."PAGE_LINK_TYPE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAGE_LINK_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAGE_LINK_TYPE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAGE_LINK_TYPE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";

            $model->whereAdd(implode(' OR ', $conds));
        }
    }

    private function _import($templatecode = 'import.pagelinktype.tpl') {
    ...
    private function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('pagelinktype_before_import', $model, $rawdata);
            ...
            PluginManager::do_action('pagelinktype_imported', $model);
            NotificationHelper::notifyChange('pagelinktype', 'insert');
		}
