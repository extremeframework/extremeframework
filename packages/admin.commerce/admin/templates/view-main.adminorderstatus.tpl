<div class="view-main">
    <{plugin key="adminorderstatus_view_top" args=$details}>
        ...
        <{if !isset($columns2hide) || !in_array('NAME', $columns2hide) }>
    		<td class="form-field form-field-label column-name">
	            		        <label class="attribute-name"><{label key="L_ADMIN_ORDER_STATUS_NAME"}></label>
            </td>
    ...
    <{plugin key="adminorderstatus_view_bottom" args=$details}>

    <script type="text/javascript">
        ///////////////////////////////////////////////////////////////////////////////
        // VISIBILITY REFLECTION
        adminorderstatusview_get_visibility_settings = function (model) {
            var settings = {};
        ...
        adminorderstatusview_apply_visibility_settings = function (settings) {
            $.each(settings, function(key, value) {
                adminorderstatusview_apply_block_visibility(key, value);
            });
        }

        adminorderstatusview_apply_block_visibility = function (key, value) {
        ...
        adminorderstatusview_update_visibility_settings = function () {
            var settings = adminorderstatusview_get_visibility_settings();

            adminorderstatusview_apply_visibility_settings(settings);
        }

        $(function() {
            adminorderstatusview_update_visibility_settings();
        });
...
//                        type: "post",
//                        dataType: "json",
//                        url: "<{$smarty.const.APPLICATION_URL}>/adminorderstatus/fieldSave",
//                        data: td.find(':input').serialize() + "&adminorderstatus_formdata_UUID=<{$details->UUID}>"
//                    }).done(function(data) {
//                        if (data.success) {
//                            if (input.is("select")) {
