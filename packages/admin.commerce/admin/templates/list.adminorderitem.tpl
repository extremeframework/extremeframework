...
<{assign var='title' value=$smarty.const.L_ADMIN_ORDER_ITEM}>
<{include file="top.tpl"}>

<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="adminorderitemquicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/adminorderitem/search" method="post" enctype="multipart/form-data">
            <input type="text" name="adminorderitem_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{$smarty.const.L_SEARCH}>" />
...
<{if isset($title) }>
    <h1 class="heading">
        ...
        <span style="margin-left:10px; font-size:12px; font-weight: normal" class="hidden-print">
            <a style="text-decoration: none" class="button-view-refresh scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminorderitem"><i class="fa fa-refresh"></i></a>
        </span>
    </h1>
<{/if}>
...
<form name="adminorderitemlist" id="adminorderitemlistform" class="form-list scope-list" action="<{$smarty.const.APPLICATION_URL}>/adminorderitem/" method="post">
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminorderitem/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{$smarty.const.L_NEW}> <{$smarty.const.L_ADMIN_ORDER_ITEM|strtolower}>"/><{label key="L_NEW"}> <{$smarty.const.L_ADMIN_ORDER_ITEM|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                                            ...
                                            <div class="btn button-general">
                            <a href="#" onclick="adminorderitem_delete()"><span class="button-face"><{$smarty.const.L_DELETE}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.adminorderitem.import)}>
                                            <div class="btn button-general">
                            <a class="button-import scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminorderitem/import/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-import.png" alt="<{$smarty.const.L_IMPORT}>" /><{$smarty.const.L_IMPORT}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.adminorderitem.export)}>
                                            <div class="btn button-general">
                            <a class="button-export" href="<{$smarty.const.APPLICATION_URL}>/adminorderitem/export/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-export.png" alt="<{$smarty.const.L_EXPORT}>" /><{$smarty.const.L_EXPORT}></span></a>
                        </div>
                                            <{/if}>
                        ...
                        <{if isset($smarty.session.acl.adminfilter.edit) }>
                            <div class="custom-filter">
                                ...
                                <span class="custom-filter-footer adminorderitem-custom-filter-footer hide">
                                    <hr>
                                    <ul>
                                    	<li><a class="scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminfilter/new/preset/MODULE/presetvalue/adminorderitem"><i class="fa fa-plus-circle"></i> <{label key="L_CREATE_NEW_FILTER"}></a></li>
                                    </ul>
                                </span>
            	        ...
            	        <ul class="dropdown-menu pull-right">
                            <{foreach from=$admin_list_actions item=item}>
                                <{if empty($item->ACL) || isset($smarty.session.acl.adminorderitem[$item->ACL])}>
                                    <li><a <{if $item->IS_OPEN_IN_NEW_WINDOW}>target="_blank"<{/if}> <{if $item->LINK}>href="<{framework_report_link format=$item->LINK}>"<{/if}> <{if $item->ACTION}>onclick="adminorderitem_<{$item->ACTION}>(); return false;"<{/if}>><span class="button-face"><{label text=$item->TITLE}></span></a></li>
                                <{/if}>
                            <{/foreach}>
                        </ul>
            	    </div>
...
function adminorderitem_search() {
	$('#adminorderitemlistform').attr('action', '<{$smarty.const.APPLICATION_URL}>/adminorderitem/search/');
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="adminorderitem-list-count list-count"><{$total}></span> <{label key="L_RECORDS"}>
	</div>
	<div style="clear:both"></div>
<{/if}>
