...
<{assign var='title' value=_t('L_ADMIN_ORDER_STATUS')}>
<{include file="top.tpl"}>

<!-- Quick search -->
    <div class="quicksearch hidden-print">
        <form id="adminorderstatusquicksearch" class="form-quicksearch scope-list" action="<{$smarty.const.APPLICATION_URL}>/adminorderstatus/search" method="post" enctype="multipart/form-data">
            <input type="text" name="adminorderstatus_searchdata___QUICKSEARCH__" value="<{if isset($searchdata.__QUICKSEARCH__)}><{$searchdata.__QUICKSEARCH__}><{/if}>" size="25" placeholder="<{_t('L_SEARCH', true)}>" />
...
<{if isset($title) }>
    <h1 class="heading">
        ...
        <span style="margin-left:10px; font-size:12px; font-weight: normal" class="hidden-print">
            <a style="text-decoration: none" class="button-view-refresh scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminorderstatus"><i class="fa fa-refresh"></i></a>
        </span>
    </h1>
<{/if}>
        ...
        <ul class="subtabs">
            ...
            <div class="buttons">
            		        ...
            		        <div class="btn button-general">
            		            <a class="button-new scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminorderstatus/new/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-add.png" alt="<{_t('L_NEW', true)}> <{_t('L_ADMIN_ORDER_STATUS', true)|strtolower}>"/><{_t('L_NEW')}> <{_t('L_ADMIN_ORDER_STATUS')|strtolower}></span></a>
            		        </div>
                                    		        <{/if}>
        	    ...
        	    <{/if}>
                            ...
                            <a href="#" onclick="adminorderstatus_delete()"><span class="button-face"><{_t('L_DELETE')}></span></a>
                                            ...
                                            <div class="btn button-general">
                            <a class="button-import scope-main" href="<{$smarty.const.APPLICATION_URL}>/adminorderstatus/import/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-import.png" alt="<{_t('L_IMPORT', true)}>" /><{_t('L_IMPORT')}></span></a>
                        </div>
                                            <{/if}>
                                                                                <{if isset($smarty.session.acl.adminorderstatus.export)}>
                                            <div class="btn button-general">
                            <a class="button-export" href="<{$smarty.const.APPLICATION_URL}>/adminorderstatus/export/"><span class="button-face"><img class="button-icon" src="<{$smarty.const.APPLICATION_URL}>/images/button-icon-export.png" alt="<{_t('L_EXPORT', true)}>" /><{_t('L_EXPORT')}></span></a>
                        </div>
                                            <{/if}>
...
<!-- Relations -->
    <{php}>
    	$template->assign('copyguidelines',  sprintf(L_GUIDELINES_COPY_RELS, strtolower(L_ADMIN_ORDER_STATUS)));
    	$template->assign('approveguidelines', sprintf(L_GUIDELINES_APPROVE_RELS, strtolower(L_ADMIN_ORDER_STATUS)));
    	$template->assign('deleteguidelines', sprintf(L_GUIDELINES_DELETE_RELS, strtolower(L_ADMIN_ORDER_STATUS), strtolower(L_ADMIN_ORDER_STATUS)));
    <{/php}>

    <div id="adminorderstatuscopyrelations" style="display:none" title="<{_t('L_COPY', true)}> <{_t('L_ADMIN_ORDER_STATUS', true)|strtolower}>">
        <p><{$copyguidelines}></p>
        <ul>
                            <{if Framework::hasModule('AdminOrder')}>
                    <li style="padding:5px 0 5px 0"><input type="checkbox" name="copyrelations[]" value="adminorder" /> <{_t('L_COPY_ALSO')}> <{_t('L_ADMIN_ORDER')|strtolower}></li>
                <{/if}>
                    </ul>
    </div>

    <div id="adminorderstatusapproverelations" style="display:none" title="<{_t('L_APPROVE', true)}> <{_t('L_ADMIN_ORDER_STATUS', true)|strtolower}>">
        <p><{$approveguidelines}></p>
        <ul>
                            <{if Framework::hasModule('AdminOrder')}>
                    <li style="padding:5px 0 5px 0"><input type="checkbox" name="approverelations[]" value="adminorder" /> <{_t('L_APPROVE_ALSO')}> <{_t('L_ADMIN_ORDER')|strtolower}></li>
                <{/if}>
                    </ul>
    </div>

    <div id="adminorderstatusdeleterelations" style="display:none" title="<{_t('L_DELETE', true)}> <{_t('L_ADMIN_ORDER_STATUS', true)|strtolower}>">
        <p><{$deleteguidelines}></p>
        <ul>
                            <{if Framework::hasModule('AdminOrder')}>
                    <li style="padding:5px 0 5px 0"><input type="checkbox" name="deleterelations[]" value="adminorder" /> <{_t('L_DELETE_ALSO')}> <{_t('L_ADMIN_ORDER')|strtolower}></li>
                <{/if}>
                    </ul>
    </div>
...
function adminorderstatus_search() {
	$('#adminorderstatuslistform').attr('action', '<{$smarty.const.APPLICATION_URL}>/adminorderstatus/search/');
...
<{if $pagination || $total > 10}>
	...
	<div style="float:left">
		<span class="adminorderstatus-list-count list-count"><{$total}></span> <{_t('L_RECORDS')}>
	</div>
	<div style="clear:both"></div>
<{/if}>
