...
/**
 * Usage: <{module class="WidgetListPaymentType" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.paymenttype.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListPaymentType extends PaymentTypeController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.paymenttype.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('paymenttype')) {
            $vars = array_keys(get_object_vars(new PaymentTypeModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('paymenttype', $preset, $presetvalue));
