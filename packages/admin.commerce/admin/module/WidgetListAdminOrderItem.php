...
/**
 * Usage: <{module class="WidgetListAdminOrderItem" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminorderitem.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminOrderItem extends AdminOrderItemController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminorderitem.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminorderitem')) {
            $vars = array_keys(get_object_vars(new AdminOrderItemModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminorderitem', $preset, $presetvalue));
