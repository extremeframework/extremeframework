...
/**
 * Usage: <{module class="WidgetListAdminOrder" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminorder.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminOrder extends AdminOrderController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminorder.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminorder')) {
            $vars = array_keys(get_object_vars(new AdminOrderModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminorder', $preset, $presetvalue));
