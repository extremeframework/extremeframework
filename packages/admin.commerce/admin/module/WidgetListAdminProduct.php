...
/**
 * Usage: <{module class="WidgetListAdminProduct" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminproduct.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminProduct extends AdminProductController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminproduct.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminproduct')) {
            $vars = array_keys(get_object_vars(new AdminProductModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminproduct', $preset, $presetvalue));
