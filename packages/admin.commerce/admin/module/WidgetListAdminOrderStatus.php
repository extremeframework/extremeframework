...
/**
 * Usage: <{module class="WidgetListAdminOrderStatus" method="<overriding_method>" COLUMN=? REFTABLE___REFCOLUMN=? where="<overriding_condition>" template='widgetlist.adminorderstatus.tpl'}>
 */

defined('APPLICATION_DIR') OR exit();

class WidgetListAdminOrderStatus extends AdminOrderStatusController {
    function fetch($params) {
        ...
        $template = isset($params['template'])? $params['template'] : 'widgetlist.adminorderstatus.tpl';
        $limit = isset($params['limit'])? $params['limit'] : 10;
        $readonly = isset($params['readonly'])? $params['readonly'] : false;

        if (AclController::hasPermission('adminorderstatus')) {
            $vars = array_keys(get_object_vars(new AdminOrderStatusModel()));
    		...
    		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminorderstatus', $preset, $presetvalue));
