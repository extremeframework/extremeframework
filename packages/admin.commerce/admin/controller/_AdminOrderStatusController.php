...
class _AdminOrderStatusController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_ADMIN_ORDER_STATUS_NAME);
           return false;
       }
       ...
       if (in_array('CODE', $columns2check)) {
           $_model = new AdminOrderStatusModel();
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('adminorderstatus', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminorderstatus_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminorderstatus_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminorderstatus_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminorderstatus');

        AclController::checkPermission('adminorderstatus', 'list');

		ContextStack::register(APPLICATION_URL.'/adminorderstatus/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_ORDER_STATUS.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminorderstatus_before_delete', $_model);
        }

		$model = new AdminOrderStatusModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminorderstatus', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminorderstatus', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminorderstatuslist_selection_selectall']) && $_REQUEST['adminorderstatuslist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminorderstatus.list.model']) && is_object($_SESSION['adminorderstatus.list.model'])) {
					$model = clone $_SESSION['adminorderstatus.list.model'];
				} else {
		        	$model = new AdminOrderStatusModel();
				}
    			...
    			$selection = isset($_REQUEST['adminorderstatuslist_selection'])? $_REQUEST['adminorderstatuslist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminorderstatus' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminorderstatus', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminOrderStatusModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminorderstatus', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminorderstatus/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminorderstatus/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorderstatus_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminorderstatus_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminorderstatus');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorderstatus_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorderstatus_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
				...
				$old = new AdminOrderStatusModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminorderstatus_before_update', $model, $old);

        		if (!AclController::hasPermission('adminorderstatus', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_ORDER_STATUS.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('adminorderstatus_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminorderstatus', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminorderstatus.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminorderstatus', 'import');

        if (isset($_FILES['adminorderstatus_importfile'])) {
        	if (!$_FILES['adminorderstatus_importfile']['error']) {
	            $filepath = $_FILES['adminorderstatus_importfile']['tmp_name'];
        		$original = $_FILES['adminorderstatus_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminorderstatus/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminorderstatus.list.model']) && is_object($_SESSION['adminorderstatus.list.model'])) {
			$model = clone $_SESSION['adminorderstatus.list.model'];
		} else {
        	$model = new AdminOrderStatusModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminorderstatus_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminorderstatus_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminorderstatus_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','ORDERING');
        $filename = 'adminorderstatus_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminOrderStatus');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminorderstatus.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminorderstatus.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminorderstatus.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminorderstatus.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminorderstatus.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminorderstatus.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminorderstatus.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminorderstatus.tpl');
    }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminorderstatus.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminorderstatus.customview'])? $_SESSION['adminorderstatus.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminorderstatus');
        ...
        $_SESSION['adminorderstatus.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminorderstatus'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminorderstatus', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminorderstatus', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminorderstatus', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.adminorderstatus.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminorderstatus', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminorderstatus', 'new');
        }
        ...
        $ids = isset($_SESSION['adminorderstatus.list.ids'])? $_SESSION['adminorderstatus.list.ids'] : array();
        ...
        PluginManager::do_action('adminorderstatus_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminorderstatus', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminorderstatus', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminorderstatus', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.adminorderstatus.tpl', $restoredraft = true) {
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminorderstatus', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('adminorderstatus_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminorderstatus', $model->UUID, $model);
            }
        ...
        PluginManager::do_action('adminorderstatus_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminorderstatus', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminorderstatus'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminorderstatus'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminOrderStatusController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminorderstatus.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_ORDER_STATUS.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_STATUS.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'ADMIN_ORDER_STATUS`.NAME, `'.TABLE_PREFIX.'ADMIN_ORDER_STATUS`.CODE, `'.TABLE_PREFIX.'ADMIN_ORDER_STATUS`.ID, `'.TABLE_PREFIX.'ADMIN_ORDER_STATUS`.JSON, `'.TABLE_PREFIX.'ADMIN_ORDER_STATUS`.UUID, `'.TABLE_PREFIX.'ADMIN_ORDER_STATUS`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_ORDER_STATUS.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER_STATUS.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER_STATUS.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER_STATUS.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
    ...
    private function _importxls($filepath, &$error) {
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminorderstatus_before_import', $model, $rawdata);
