...
class _AdminOrderItemController extends __AppController
{
    ...
    protected function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('QUANTITY', $columns2check) && !empty($model->QUANTITY) && !is_numeric($model->QUANTITY)) {
           $errors['quantity'] = sprintf(_t('L_VALIDATION_NUMBER'), _t('Quantity'));
           return false;
       }
       if (in_array('PRICE', $columns2check) && !empty($model->PRICE) && !is_numeric($model->PRICE)) {
           $errors['price'] = sprintf(_t('L_VALIDATION_NUMBER'), _t('Price'));
           return false;
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('adminorderitem', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminorderitem_formdata_(.*)/', $name, $matches)) {
    ...
    protected function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminorderitem_searchdata_(.*)/', $name, $matches)) {
    ...
    protected function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminorderitem_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('adminorderitem');

        AclController::checkPermission('adminorderitem', 'list');

		ContextStack::register(APPLICATION_URL.'/adminorderitem/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_ORDER_ITEM.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
        ...
        $columns2return = array('ID' => 'id', 'QUANTITY' => 'name');
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
		...
		$_model = new AdminOrderItemModel();
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminorderitem_before_delete', $_model);
        }

		$model = new AdminOrderItemModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminorderitem', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminorderitem', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminorderitemlist_selection_selectall']) && $_REQUEST['adminorderitemlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminorderitem.list.model']) && is_object($_SESSION['adminorderitem.list.model'])) {
					$model = clone $_SESSION['adminorderitem.list.model'];
				} else {
		        	$model = new AdminOrderItemModel();
				}
    			...
    			$selection = isset($_REQUEST['adminorderitemlist_selection'])? $_REQUEST['adminorderitemlist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminorderitem' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminorderitem', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminOrderItemModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminorderitem', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminorderitem/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminorderitem/edit/'.$id);
		$this->_edit($id);
	}

    protected function onSaveSuccess($model) {
        AdminOrderController::sync_JSON_ADMIN_ORDER_ITEMS($model->ID_ADMIN_ORDER);
    ...
    protected function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorderitem_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminorderitem_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function form2model($prefix = null, &$columns2check = null) {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorderitem_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    protected function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorderitem_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminorderitem_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminorderitem_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            ...
            PluginManager::do_action('adminorderitem_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminOrderItemModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminorderitem_before_update', $model, $old);

        		if (!AclController::hasPermission('adminorderitem', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_ORDER_ITEM.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('adminorderitem_before_create', $model);
    ...
    protected function bind2refobject(&$model, $refobject = null) {
        if ($refobject != null) {
            ...
            if ($refclass == 'AdminOrderModel' && empty($model->ID_ADMIN_ORDER)) {
                $model->ID_ADMIN_ORDER = $refobject->ID;
            }
            if ($refclass == 'AdminProductModel' && empty($model->ID_ADMIN_PRODUCT)) {
                $model->ID_ADMIN_PRODUCT = $refobject->ID;
            }
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminorderitem', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminorderitem.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminorderitem', 'import');

        if (isset($_FILES['adminorderitem_importfile'])) {
        	if (!$_FILES['adminorderitem_importfile']['error']) {
	            $filepath = $_FILES['adminorderitem_importfile']['tmp_name'];
        		$original = $_FILES['adminorderitem_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminorderitem/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('ID_ADMIN_ORDER','ID_ADMIN_PRODUCT','QUANTITY','PRICE');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminorderitem.list.model']) && is_object($_SESSION['adminorderitem.list.model'])) {
			$model = clone $_SESSION['adminorderitem.list.model'];
		} else {
        	$model = new AdminOrderItemModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminorderitem_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminorderitem_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminorderitem_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('ID_ADMIN_ORDER','ID_ADMIN_PRODUCT','QUANTITY','PRICE');
        $filename = 'adminorderitem_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminOrderItem');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminorderitem.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminorderitem.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminorderitem.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminorderitem.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminorderitem.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminorderitem.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminorderitem.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminorderitem.tpl');
    }
    ...
    protected function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminorderitem.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminorderitem.customview'])? $_SESSION['adminorderitem.customview'] : '';
    ...
    protected function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminorderitem');
        ...
        $_SESSION['adminorderitem.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminorderitem'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminorderitem', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminorderitem', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminorderitem', 'view'));
    ...
    protected function _view($id, $details = null, $templatecode = 'view.adminorderitem.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminorderitem', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminorderitem', 'new');
        }
        ...
        $ids = isset($_SESSION['adminorderitem.list.ids'])? $_SESSION['adminorderitem.list.ids'] : array();
        ...
        PluginManager::do_action('adminorderitem_before_view', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminorderitem', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminorderitem', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminorderitem', 'view'));
    ...
    protected function _edit($id, $details = null, $templatecode = 'edit.adminorderitem.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('ID_ADMIN_ORDER', 'ID_ADMIN_PRODUCT', 'QUANTITY', 'PRICE');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminorderitem', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminorderitem', $model->UUID, $model);
            }
        ...
        if ($details->UUID) {
            if (isset($details->ID_ADMIN_ORDER) && $details->ID_ADMIN_ORDER) {
                $acleditablecolumns['ID_ADMIN_ORDER'] = false;

                $roweditablecolumns = array_diff($roweditablecolumns, array('ID_ADMIN_ORDER'));
            }
        }
        ...
        PluginManager::do_action('adminorderitem_before_edit', $details);
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminorderitem', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminorderitem'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminorderitem'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminOrderItemController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminorderitem.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'ID_ADMIN_ORDER':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.ID_ADMIN_ORDER IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.ID_ADMIN_ORDER = '$value'");
                        }

                        break;

                    case 'ID_ADMIN_PRODUCT':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.ID_ADMIN_PRODUCT IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.ID_ADMIN_PRODUCT = '$value'");
                        }

                        break;

                    case 'QUANTITY__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.QUANTITY >= '".$this->field_sanitize('QUANTITY', $value)."'");

                        break;

                    case 'QUANTITY__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.QUANTITY IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER_ITEM.QUANTITY <= '".$this->field_sanitize('QUANTITY', $value)."')");

                        break;

                    case 'PRICE__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.PRICE >= '".$this->field_sanitize('PRICE', $value)."'");

                        break;

                    case 'PRICE__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.PRICE IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER_ITEM.PRICE <= '".$this->field_sanitize('PRICE', $value)."')");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_ORDER_ITEM.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER_ITEM.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function getParentWiseAclCheckString() {
        ...
        if (!empty($ids)) {
            $where = TABLE_PREFIX."ADMIN_ORDER_ITEM.ID_ADMIN_ORDER IN (".implode(',', $ids).")";
        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_ORDER_ITEM.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER_ITEM.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
    ...
    protected function _importxls($filepath, &$error) {
        ...
        $columns2import = array('ID_ADMIN_ORDER','ID_ADMIN_PRODUCT','QUANTITY','PRICE');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminorderitem_before_import', $model, $rawdata);
        ...
        if (isset($valuecache[$refcolumn][$reflabel])) {
            ...
            switch ($refcolumn) {
                case 'ID_ADMIN_ORDER':
                    ...
                    $model->CUSTOMER_FIRST_NAME = $reflabel;
                    if ($model->find(1)) {
                        $value = $model->ID;
                    } else {
                        $model->insert();
            ...
            if (!empty($refvalue)) {
                switch ($refcolumn) {
                    case 'ID_ADMIN_ORDER':
                        ...
                        $model->ID = $refvalue;
                        $model->find();
                        $label = $model->fetch()? $model->CUSTOMER_FIRST_NAME : $refvalue;
                        break;
