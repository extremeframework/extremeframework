...
class _PaymentTypeController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('NAME', $columns2check) && trim($model->NAME) == '') {
           $errors['name'] = sprintf(L_VALIDATION_NOT_EMPTY, L_PAYMENT_TYPE_NAME);
           return false;
       }
       ...
       if (in_array('ORDERING', $columns2check) && !empty($model->ORDERING) && !is_numeric($model->ORDERING)) {
        ...
        if (!CustomFieldHelper::checkCustomFieldConstraint('paymenttype', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^paymenttype_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^paymenttype_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^paymenttype_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        ContextStack::check2redirect('paymenttype');

        AclController::checkPermission('paymenttype', 'list');

		ContextStack::register(APPLICATION_URL.'/paymenttype/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."PAYMENT_TYPE.$field $operator '$value')";
                }
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('paymenttype_before_delete', $_model);
        }

		$model = new PaymentTypeModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('paymenttype', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('paymenttype', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['paymenttypelist_selection_selectall']) && $_REQUEST['paymenttypelist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['paymenttype.list.model']) && is_object($_SESSION['paymenttype.list.model'])) {
					$model = clone $_SESSION['paymenttype.list.model'];
				} else {
		        	$model = new PaymentTypeModel();
				}
    			...
    			$selection = isset($_REQUEST['paymenttypelist_selection'])? $_REQUEST['paymenttypelist_selection'] : '';
			}
        }
		...
		if (!empty($selection)) {
            ...
            if (!empty($relations)) {
                foreach ($relations as $module) {
                    switch ($module) {
                        case 'adminorder': 
                            (new AdminOrderController())->delete('ID_PAYMENT_TYPE', $_ids);
                            break;
        ...
        if ($module != 'paymenttype' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('paymenttype', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new PaymentTypeModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('paymenttype', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/paymenttype/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/paymenttype/edit/'.$id);
		$this->_edit($id);
	}
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'paymenttype_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'paymenttype_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('paymenttype');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'paymenttype_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'paymenttype_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    protected function save($models = array()) {
        ...
        foreach ($models as $model) {
    		...
    		if ($model->UUID) {
				...
				$old = new PaymentTypeModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('paymenttype_before_update', $model, $old);

        		if (!AclController::hasPermission('paymenttype', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."PAYMENT_TYPE.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
                ...
                PluginManager::do_action('paymenttype_before_create', $model);
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('paymenttype', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.paymenttype.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
    ...
    public function importAction() {
        ...
        AclController::checkPermission('paymenttype', 'import');

        if (isset($_FILES['paymenttype_importfile'])) {
        	if (!$_FILES['paymenttype_importfile']['error']) {
	            $filepath = $_FILES['paymenttype_importfile']['tmp_name'];
        		$original = $_FILES['paymenttype_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/paymenttype/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('NAME','CODE','IS_ENABLED','INSTRUCTION','ORDERING','NOTE');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['paymenttype.list.model']) && is_object($_SESSION['paymenttype.list.model'])) {
			$model = clone $_SESSION['paymenttype.list.model'];
		} else {
        	$model = new PaymentTypeModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'paymenttype_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/paymenttype_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/paymenttype_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('NAME','CODE','IS_ENABLED','INSTRUCTION','ORDERING','NOTE');
        $filename = 'paymenttype_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('PaymentType');
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.paymenttype.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.paymenttype.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.paymenttype.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.paymenttype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.paymenttype.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.paymenttype.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.paymenttype.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.paymenttype.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('NAME', 'CODE', 'IS_ENABLED', 'ORDERING');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['paymenttype.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['paymenttype.customview'])? $_SESSION['paymenttype.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('paymenttype');
        ...
        $_SESSION['paymenttype.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('paymenttype'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('paymenttype', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('paymenttype', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('paymenttype', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.paymenttype.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('paymenttype', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('paymenttype', 'new');
        }
        ...
        $ids = isset($_SESSION['paymenttype.list.ids'])? $_SESSION['paymenttype.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('paymenttype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('paymenttype', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('paymenttype', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.paymenttype.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('NAME', 'CODE', 'IS_ENABLED', 'ORDERING');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('paymenttype', 'editpeer');
                }
            } else {
                // Set default values here
                
                $this->onInitialization($model);
                PluginManager::do_action('paymenttype_new', $model);
            }
            
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('paymenttype', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('paymenttype', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('paymenttype'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('paymenttype'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
        ...
        $item = null;
     	if ($model->fetch()) {
		    PaymentTypeController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['paymenttype.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'NAME':
                        $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.$key IS NULL OR ".TABLE_PREFIX.".PAYMENT_TYPE.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."PAYMENT_TYPE.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function initViewModel(&$model, $join = false) {
        $model->selectAdd();
        $model->selectAdd('`'.TABLE_PREFIX.'PAYMENT_TYPE`.NAME, `'.TABLE_PREFIX.'PAYMENT_TYPE`.CODE, `'.TABLE_PREFIX.'PAYMENT_TYPE`.IS_ENABLED, `'.TABLE_PREFIX.'PAYMENT_TYPE`.INSTRUCTION, `'.TABLE_PREFIX.'PAYMENT_TYPE`.NOTE, `'.TABLE_PREFIX.'PAYMENT_TYPE`.ID, `'.TABLE_PREFIX.'PAYMENT_TYPE`.JSON, `'.TABLE_PREFIX.'PAYMENT_TYPE`.UUID, `'.TABLE_PREFIX.'PAYMENT_TYPE`.WFID');
    }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."PAYMENT_TYPE.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAYMENT_TYPE.NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAYMENT_TYPE.CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAYMENT_TYPE.INSTRUCTION LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAYMENT_TYPE.NOTE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."PAYMENT_TYPE.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
    ...
    private function _importxls($filepath, &$error) {
        ...
        $columns2import = array('NAME','CODE','IS_ENABLED','INSTRUCTION','ORDERING','NOTE');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('paymenttype_before_import', $model, $rawdata);
