...
class _AdminOrderController extends __AppController
{
    ...
    private function checkConstraint($model, &$errors, $columns2check) {
       ...
       if (in_array('ID_PAYMENT_TYPE', $columns2check) && trim($model->ID_PAYMENT_TYPE) == '') {
           $errors['id-payment-type'] = sprintf(L_VALIDATION_NOT_EMPTY, L_PAYMENT_TYPE);
           return false;
       }
       if (in_array('CUSTOMER_EMAIL', $columns2check) && !empty($model->CUSTOMER_EMAIL) && !preg_match('/^([.0-9a-z_-]+)@(([0-9a-z-]+\.)+[0-9a-z]{2,4})$/i', $model->CUSTOMER_EMAIL)) {
           $errors['customer-email'] = sprintf(L_VALIDATION_EMAIL, L_CUSTOMER_EMAIL);
           return false;
       }
       if (in_array('CUSTOMER_PHONE', $columns2check) && !empty($model->CUSTOMER_PHONE) && !preg_match('/^[0-9 +-\.\(\)]*$/i', $model->CUSTOMER_PHONE)) {
           $errors['customer-phone'] = sprintf(L_VALIDATION_PHONE, L_CUSTOMER_PHONE);
           return false;
       }
       if (in_array('TOTAL_VALUE', $columns2check) && !empty($model->TOTAL_VALUE) && !is_numeric($model->TOTAL_VALUE)) {
           $errors['total-value'] = sprintf(L_VALIDATION_NUMBER, L_TOTAL_VALUE);
           return false;
       }
       if (in_array('COUPON_DISCOUNT', $columns2check) && !empty($model->COUPON_DISCOUNT) && !is_numeric($model->COUPON_DISCOUNT)) {
           $errors['coupon-discount'] = sprintf(L_VALIDATION_NUMBER, L_COUPON_DISCOUNT);
           return false;
       }


        if (!CustomFieldHelper::checkCustomFieldConstraint('adminorder', $model, $errors)) {
            return false;
        }
    ...
    public function getFormData() {
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^adminorder_formdata_(.*)/', $name, $matches)) {
    ...
    private function getSearchFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminorder_searchdata_(.*)/', $name, $matches)) {
    ...
    private function getFilterFormData() {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^adminorder_filterdata_(.*)/', $name, $matches)) {
                $searchdata[$matches[1]] = trim($value);
            }
		}
    ...
    public function listAction() {
        ...
        LicenseController::enforceLicenseCheck('adminorder');

        ContextStack::check2redirect('adminorder');

        AclController::checkPermission('adminorder', 'list');

		ContextStack::register(APPLICATION_URL.'/adminorder/list/');
        ...
        if (!empty($id) && Framework::hasModule('AdminFilter')) {
            ...
            if ($afm->fetch()) {
                ...
                foreach ($items as $item) {
                    ...
                    $conditions[$condition][] = "(".TABLE_PREFIX."ADMIN_ORDER.$field $operator '$value')";
                }
    ...
    public function jsonSearchByTermAction() {
        ...
        $columns2return = array('ID' => 'id', 'CUSTOMER_FIRST_NAME' => 'name');

        $term = isset($_REQUEST['term'])? $_REQUEST['term'] : '';

		$model = new AdminOrderModel();
		if (!empty($term)) {
            $model->whereAdd("REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR CUSTOMER_FIRST_NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR CUSTOMER_LAST_NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR CUSTOMER_ZIP_CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%' OR COUPON_CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($term))."%'");
		}
		$model->find();
    ...
    public function liveSearchAction($liveselect = false) {
		...
		$model->orderBy('CUSTOMER_FIRST_NAME');
		$model->limit(0, 10);
		$model->find();

		$rows = array();
     	while ($model->fetch()) {
     		$rows[] = array('id' => $model->UUID, 'eid' => $model->ID, 'title' => $model->CUSTOMER_FIRST_NAME);
		}
    ...
    public function delete($target_column, $target_values, &$_ids = null) {
        ...
        foreach ($_models as $_model) {
            ...
            PluginManager::do_action('adminorder_before_delete', $_model);
        }

		$model = new AdminOrderModel();
		$model->whereAdd($target_column.' = "'.implode('" or '.$target_column.' = "', $target_values).'"');

		if (!AclController::hasPermission('adminorder', 'deletepeer')) {
		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
    ...
    public function deleteAction() {
        ...
        AclController::checkPermission('adminorder', 'delete');
	    ...
	    if (!empty($id)) {
			...
			$selectall = isset($_REQUEST['adminorderlist_selection_selectall']) && $_REQUEST['adminorderlist_selection_selectall'];

			if ($selectall) {
				if (isset($_SESSION['adminorder.list.model']) && is_object($_SESSION['adminorder.list.model'])) {
					$model = clone $_SESSION['adminorder.list.model'];
				} else {
		        	$model = new AdminOrderModel();
				}
    			...
    			$selection = isset($_REQUEST['adminorderlist_selection'])? $_REQUEST['adminorderlist_selection'] : '';
			}
        }
        ...
        if ($module != 'adminorder' || in_array($action, array('list', 'page'))) {
		    ContextStack::back(0);
        } else {
		    ContextStack::back(1);
        }
    ...
    public function newAction() {
        ...
        AclController::checkPermission('adminorder', 'edit');

		ContextStack::register(APPLICATION_URL.'/adminorder/new/');
    ...
    public function viewAction() {
        ...
        AclController::checkPermission('adminorder', 'view');
        ...
        if (!empty($id)) {
            ...
            $filters = FilterHelper::getProperFilters(new AdminOrderModel(), $_REQUEST);
        }
        ...
        if (!empty($templatecode)) {
            $adminview = AdminViewHelper::getAdminView('adminorder', $templatecode);
		    ...
		    ContextStack::register(APPLICATION_URL.'/adminorder/view/'.$id);
    		$this->_view($id);
        }
	}
    ...
    public function editAction() {
		...
		ContextStack::register(APPLICATION_URL.'/adminorder/edit/'.$id);
		$this->_edit($id);
	}
    ...
    static function sync_JSON_ADMIN_ORDER_ITEMS($id) {
        $model = new AdminOrderItemModel();
        $model->ID_ADMIN_ORDER = $id;
        ...
        AdminOrderController::update_json($id, 'ADMIN_ORDER_ITEM', $items);
    }
    ...
    private function getTempCreateId() {
        ...
        if (!isset($_SESSION['adminorder.tmpid'])) {
            ...
            $model->UUID = $_SESSION['adminorder.tmpid'];
        ...
        if ($create) {
            ...
            $_SESSION['adminorder.tmpid'] = $model->UUID;
        }
    ...
    private function formmode($prefix = null) {
		...
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorder_formdata_(.+)/', $name)) {
                return 'single'; // the single mode has a higher priority
            }

            if (!$multiple && preg_match('/^'.$prefix.'adminorder_multiformdata_(.+)/', $name)) {
                $multiple = true;
            }
        }
    ...
    protected function field_sanitize($column, $value) {
        ...
        } elseif ($column == 'PAYMENT_DATE' && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/', $value)) {
            $format = DATE_FORMAT;
            $format = str_ireplace(array('yy', 'mm', 'dd'), array('Y', 'm', 'd'), $format);
        ...
        } elseif ($column == 'REFUND_DATE' && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/', $value)) {
            $format = DATE_FORMAT;
            $format = str_ireplace(array('yy', 'mm', 'dd'), array('Y', 'm', 'd'), $format);
        ...
        } elseif ($column == 'PROCESSED_DATE' && !preg_match('/^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/', $value)) {
            $format = DATE_FORMAT;
            $format = str_ireplace(array('yy', 'mm', 'dd'), array('Y', 'm', 'd'), $format);
    ...
    private function form2model($prefix = null, &$columns2check = null) {
        $customfieldcolumns = CustomFieldHelper::getCustomFieldColumns('adminorder');
		...
		foreach ($raw as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorder_formdata_(.*)/', $name, $matches)) {
                $matched = true;
                $key = $matches[1];
    ...
    private function form2models($prefix = null, &$columns2check = null) {
        ...
        $rows = array();
		foreach ($_REQUEST as $name => $value) {
            if (preg_match('/^'.$prefix.'adminorder_multiformdata_(.+)/', $name, $matches)) {
                $column = $matches[1];
    ...
    private function saveform($prefix = null, $refobject = null) {
        ...
        if ($formmode == 'multiple') {
            ...
            $deleteditems = isset($_REQUEST[$prefix.'adminorder_multiformdata_deleteditems'])? $_REQUEST[$prefix.'adminorder_multiformdata_deleteditems'] : '';
    ...
    protected function save($models = array(), $refobject = null) {
        ...
        foreach ($models as $model) {
            CustomFieldHelper::updateCustomFieldValues('adminorder', $model);
            
            $this->bind2refobject($model, $refobject);
            $this->onBeforeSave($model);
            PluginManager::do_action('adminorder_before_save', $model);

    		if ($model->UUID) {
				
				$old = new AdminOrderModel();
				$old->UUID = $model->UUID;
				$old->find();
				$old->fetch();

                $this->onBeforeUpdate($model, $old);
                PluginManager::do_action('adminorder_before_update', $model, $old);

        		if (!AclController::hasPermission('adminorder', 'editpeer')) {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.UUID = '".$model->UUID."' AND ".TABLE_PREFIX."ADMIN_ORDER.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		} else {
        		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.UUID = '".$model->UUID."'");
        		    $model->update(DB_DATAOBJECT_WHEREADD_ONLY);
        		}
        ...
        DraftHelper::clearAllDrafts('adminorder');
    ...
    private function bind2refobject(&$model, $refobject = null) {
        if ($refobject != null) {
            ...
            if ($refclass == 'PaymentTypeModel' && empty($model->ID_PAYMENT_TYPE)) {
                $model->ID_PAYMENT_TYPE = $refobject->CODE;
            }
            if ($refclass == 'AdminOrderStatusModel' && empty($model->ID_ADMIN_ORDER_STATUS)) {
                $model->ID_ADMIN_ORDER_STATUS = $refobject->CODE;
            }
    ...
    public function saveDraftAction() {
        ...
        DraftHelper::saveDraft('adminorder', isset($formdata['UUID'])? $formdata['UUID'] : '', $formdata);
    }
    ...
    public function saveAction($datacheck = true, $addmore = false) {
        ...
        if (!empty($errors)) {
	        ...
	        if ($formmode == 'single') {
	            $this->_edit(null, $this->form2model(), "$viewtype.adminorder.tpl");
			} else {
				ContextStack::back(0);
			}
        } else {
		    ...
		    ContextStack::replace(APPLICATION_URL.'/adminorder/view/'.$model->UUID);
    ...
    public function importAction() {
        ...
        AclController::checkPermission('adminorder', 'import');

        if (isset($_FILES['adminorder_importfile'])) {
        	if (!$_FILES['adminorder_importfile']['error']) {
	            $filepath = $_FILES['adminorder_importfile']['tmp_name'];
        		$original = $_FILES['adminorder_importfile']['name'];
    		...
    		ContextStack::register(APPLICATION_URL.'/adminorder/import/');
    		$this->_import();
        }
    }
    ...
    public function exportAction() {
        ...
        $columns2export = array('CUSTOMER_ID_COUNTRY','CUSTOMER_FIRST_NAME','CUSTOMER_LAST_NAME','CUSTOMER_ADDRESS','CUSTOMER_CITY','CUSTOMER_STATE','CUSTOMER_ZIP_CODE','CUSTOMER_EMAIL','CUSTOMER_PHONE','TOTAL_VALUE','COUPON_CODE','COUPON_DISCOUNT','ID_PAYMENT_TYPE','CREATION_DATE','PAYMENT_DATE','PAYMENT_REF','REFUND_DATE','REFUND_REF','PROCESSED_DATE','ID_ADMIN_ORDER_STATUS','NOTE');
		...
		if (empty($preset) && empty($presetvalue) && isset($_SESSION['adminorder.list.model']) && is_object($_SESSION['adminorder.list.model'])) {
			$model = clone $_SESSION['adminorder.list.model'];
		} else {
        	$model = new AdminOrderModel();
		}
        ...
        if (in_array($preset, $vars)) {
            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.$preset = '$presetvalue'");
		...
		if (!$splitsize || $model->N < $splitsize) {
	        $filename = 'adminorder_'.date('Ymd').'.csv';
	        ...
	        while ($model->fetch()) {
	        	$filepath = UPLOAD_DIR.'/adminorder_'.date('Ymd').'_part_'.str_pad($filenumber, $pad_length, '0', STR_PAD_LEFT).'.csv';
	        ...
	        $zipfile = UPLOAD_DIR.'/adminorder_'.date('Ymd').'.zip';
    ...
    public function exportXlsAction() {
        ...
        $columns2export = array('CUSTOMER_ID_COUNTRY','CUSTOMER_FIRST_NAME','CUSTOMER_LAST_NAME','CUSTOMER_ADDRESS','CUSTOMER_CITY','CUSTOMER_STATE','CUSTOMER_ZIP_CODE','CUSTOMER_EMAIL','CUSTOMER_PHONE','TOTAL_VALUE','COUPON_CODE','COUPON_DISCOUNT','ID_PAYMENT_TYPE','CREATION_DATE','PAYMENT_DATE','PAYMENT_REF','REFUND_DATE','REFUND_REF','PROCESSED_DATE','ID_ADMIN_ORDER_STATUS','NOTE');
        $filename = 'adminorder_'.date('Ymd').'.xls';
		...
		$worksheet =& $workbook->addWorksheet('AdminOrder');
    ...
    public function quickCreateAction() {
		...
		$this->_edit(0, null, 'quick-create.adminorder.tpl', false);
    }
    ...
    public function preCreateAction() {
		...
		$this->_edit(0, null, 'pre-create.adminorder.tpl', false);
    }
    ...
    public function rowNewAction() {
		...
		$this->_edit(0, null, 'row-edit.adminorder.tpl', false);
    }
    ...
    public function rowEditAction() {
		...
		$this->_edit($id, null, 'row-edit.adminorder.tpl', false);
    }
    ...
    public function rowExpandedEditAction() {
		...
		$this->_edit($id, null, 'row-expanded-edit.adminorder.tpl', false);
    }
    ...
    public function rowViewAction() {
		...
		$this->_view($id, null, 'row-view.adminorder.tpl');
    }
    ...
    public function rowSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-edit.adminorder.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-view.adminorder.tpl');
        }
    }
    ...
    public function rowExpandedSaveAction() {
        ...
        if (!empty($errors)) {
            ...
            $this->_edit(null, $this->form2model(), 'row-expanded-edit.adminorder.tpl', false);
        } else {
            $model = $this->saveform();

            $this->_view($model->UUID, null, 'row-expanded.adminorder.tpl');
        }
    }
    ...
    public function rowExpandedViewAction() {
		...
		$this->_view($id, null, 'row-expanded.adminorder.tpl');
    }
    ...
    protected function getCustomFilterColumns($module, &$filter = null) {
        if (!Framework::hasModule('AdminFilter')) {
            return array('CUSTOMER_FIRST_NAME', 'CUSTOMER_LAST_NAME', 'CUSTOMER_ZIP_CODE', 'TOTAL_VALUE', 'COUPON_CODE', 'ID_PAYMENT_TYPE', 'ID_ADMIN_ORDER_STATUS');
        }
    ...
    private function initCustomView(&$customview, &$customtemplate) {
        ...
        if (isset($_REQUEST['customview'])) {
            $_SESSION['adminorder.customview'] = $_REQUEST['customview'];
        }

        $customview = isset($_SESSION['adminorder.customview'])? $_SESSION['adminorder.customview'] : '';
    ...
    private function _list() {
        ...
        $excludedcolumns = AclController::getSystemExcludedColumns('adminorder');
        ...
        $_SESSION['adminorder.list.ids'] = $ids;
		...
		$smarty->assign('customfields', CustomFieldHelper::getListCustomFields('adminorder'));
		...
		$smarty->assign('admin_list_actions', AdminActionHelper::getAdminActions('adminorder', 'list'));
		$smarty->assign('admin_listitem_actions', AdminActionHelper::getAdminActions('adminorder', 'listitem'));
		$smarty->assign('customview', $customview);
		$smarty->assign('customtemplate', $customtemplate);
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminorder', 'view'));
    ...
    private function _view($id, $details = null, $templatecode = 'view.adminorder.tpl') {
        ...
        if ($id || !empty($details) && $details->UUID) {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminorder', 'edit');
        } else {
            $acleditablecolumns = AclController::getAclEnabledColumns('adminorder', 'new');
        }
        ...
        $ids = isset($_SESSION['adminorder.list.ids'])? $_SESSION['adminorder.list.ids'] : array();
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminorder', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		...
		$smarty->assign('admin_view_actions', AdminActionHelper::getAdminActions('adminorder', 'view'));
		$smarty->assign('admin_view_options', AdminViewHelper::getAdminViews('adminorder', 'view'));
    ...
    private function _edit($id, $details = null, $templatecode = 'edit.adminorder.tpl', $restoredraft = true) {
        ...
        $roweditablecolumns = array('CUSTOMER_ID_COUNTRY', 'CUSTOMER_FIRST_NAME', 'CUSTOMER_LAST_NAME', 'CUSTOMER_CITY', 'CUSTOMER_STATE', 'CUSTOMER_ZIP_CODE', 'CUSTOMER_EMAIL', 'CUSTOMER_PHONE', 'TOTAL_VALUE', 'COUPON_CODE', 'COUPON_DISCOUNT', 'ID_PAYMENT_TYPE', 'CREATION_DATE', 'PAYMENT_DATE', 'PAYMENT_REF', 'REFUND_DATE', 'REFUND_REF', 'PROCESSED_DATE', 'ID_ADMIN_ORDER_STATUS');
        ...
        if (empty($details)) {
            ...
            if ($id) {
                ...
                if ($model->GUID != $_SESSION['user']->ID) {
             	    AclController::checkPermission('adminorder', 'editpeer');
                }
            } else {
            ...
            if ($restoredraft) {
                DraftHelper::tryRestoreDraft('adminorder', $model->UUID, $model);
            }
		...
		$smarty->assign('customfields', CustomFieldHelper::getObjectCustomFields('adminorder', $details));
		$smarty->assign('excludedcolumns', $excludedcolumns);
		$smarty->assign('columntooltips', FieldHelper::getColumnTooltips('adminorder'));
		$smarty->assign('roweditablecolumns', $roweditablecolumns);
        $smarty->assign('fielddependencies', FieldHelper::getFieldDependencies('adminorder'));
    ...
    public function getItem($id_or_filters, $join = false, $check_acl = true, $additional_select_fields = '') {
		...
		if ($check_acl && !AclController::hasPermission('adminorder', 'viewpeer')) {
		    // UDID: 0 - public
		    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.UDID = 0 OR ".TABLE_PREFIX."ADMIN_ORDER.GUID = '".(isset($_SESSION['user'])? $_SESSION['user']->ID : null)."'");
		}
        ...
        $item = null;
     	if ($model->fetch()) {
		    AdminOrderController::setupAdditionalData($model);
    ...
    public function getList($join = false, $filters = array(), $aftersearchfilters = array(), $orderby = 'ID DESC', $limit = 0, $page = 0, &$pagination = null, $checkguid = true, $additional_wheres = array()) {
		...
		$_SESSION['adminorder.list.model'] = clone $model;
        ...
        foreach($filters as $key => $value) {
            ...
            if ($value != '') {
                switch ($key) {
                    ...
                    case 'CUSTOMER_ID_COUNTRY':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_ID_COUNTRY IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_ID_COUNTRY = '$value'");
                        }

                        break;

                    case 'TOTAL_VALUE__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.TOTAL_VALUE >= '".$this->field_sanitize('TOTAL_VALUE', $value)."'");

                        break;

                    case 'TOTAL_VALUE__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.TOTAL_VALUE IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER.TOTAL_VALUE <= '".$this->field_sanitize('TOTAL_VALUE', $value)."')");

                        break;

                    case 'COUPON_DISCOUNT__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.COUPON_DISCOUNT >= '".$this->field_sanitize('COUPON_DISCOUNT', $value)."'");

                        break;

                    case 'COUPON_DISCOUNT__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.COUPON_DISCOUNT IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER.COUPON_DISCOUNT <= '".$this->field_sanitize('COUPON_DISCOUNT', $value)."')");

                        break;

                    case 'ID_PAYMENT_TYPE':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.ID_PAYMENT_TYPE IS NULL");
                        } else {
                    ...
                    case 'PAYMENT_DATE__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.PAYMENT_DATE >= '".$this->field_sanitize('PAYMENT_DATE', $value)."'");

                        break;

                    case 'PAYMENT_DATE__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.PAYMENT_DATE IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER.PAYMENT_DATE <= '".$this->field_sanitize('PAYMENT_DATE', $value)."')");

                        break;

                    case 'REFUND_DATE__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.REFUND_DATE >= '".$this->field_sanitize('REFUND_DATE', $value)."'");

                        break;

                    case 'REFUND_DATE__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.REFUND_DATE IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER.REFUND_DATE <= '".$this->field_sanitize('REFUND_DATE', $value)."')");

                        break;

                    case 'PROCESSED_DATE__FROM':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.PROCESSED_DATE >= '".$this->field_sanitize('PROCESSED_DATE', $value)."'");

                        break;

                    case 'PROCESSED_DATE__TO':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.PROCESSED_DATE IS NULL OR ".TABLE_PREFIX."ADMIN_ORDER.PROCESSED_DATE <= '".$this->field_sanitize('PROCESSED_DATE', $value)."')");

                        break;

                    case 'ID_ADMIN_ORDER_STATUS':
                        if ($value == '__NULL__') {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.ID_ADMIN_ORDER_STATUS IS NULL");
                        } else {
                            $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.ID_ADMIN_ORDER_STATUS = '$value'");
                        }

                        break;

                    case 'WFID':
                        $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.WFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($value))."%'");
                    ...
                    default:
                        if (preg_match('/^custom.*/i', $key)) {
                            ...
                            if (property_exists($model, $key)) {
                                if ($value == '__EMPTY__') {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.$key IS NULL OR ".TABLE_PREFIX.".ADMIN_ORDER.$key = ''");
                                } else {
                                    $model->whereAdd(TABLE_PREFIX."ADMIN_ORDER.$key = '".$model->escape($value)."'");
                                }
                            }
                        }
    ...
    protected function setupQuickSearchModel(&$model, $searchstring) {
        ...
        foreach ($keywords as $keyword) {
            ...
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.REFID LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_FIRST_NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_LAST_NAME LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_ADDRESS LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_CITY LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_STATE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.CUSTOMER_ZIP_CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.COUPON_CODE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.PAYMENT_REF LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.REFUND_REF LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.NOTE LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
            $conds[] = TABLE_PREFIX."ADMIN_ORDER.JSON LIKE '%".$model->escape(StringHelper::htmlspecialchars($keyword))."%'";
    ...
    private function _importxls($filepath, &$error) {
        ...
        $columns2import = array('CUSTOMER_ID_COUNTRY','CUSTOMER_FIRST_NAME','CUSTOMER_LAST_NAME','CUSTOMER_ADDRESS','CUSTOMER_CITY','CUSTOMER_STATE','CUSTOMER_ZIP_CODE','CUSTOMER_EMAIL','CUSTOMER_PHONE','TOTAL_VALUE','COUPON_CODE','COUPON_DISCOUNT','ID_PAYMENT_TYPE','CREATION_DATE','PAYMENT_DATE','PAYMENT_REF','REFUND_DATE','REFUND_REF','PROCESSED_DATE','ID_ADMIN_ORDER_STATUS','NOTE');
		...
		for ($i = 2; $i <= $rowcount; $i++) {
            ...
            PluginManager::do_action('adminorder_before_import', $model, $rawdata);
